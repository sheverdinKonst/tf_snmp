...
/* information about write hw access */
static APP_DEMO_CPSS_HW_ACCESS_INFO_STC  writeAccessInfo;
/* information about read hw access */
static APP_DEMO_CPSS_HW_ACCESS_INFO_STC  readAccessInfo;
/* pointer to write hw access db */
static APP_DEMO_CPSS_HW_ACCESS_DB_STC    *writeAccessDbPtr;
/* pointer to read hw access db */
static APP_DEMO_CPSS_HW_ACCESS_DB_STC    *readAccessDbPtr;
 ...
 *     APP_DEMO_CPSS_LOG_MODE_FILE_E                       - writes the log into a file
 *
 *  Comments:
 *      None.
 *
...
/* a pointer to file to store the logs in */
static int prvOsLogFilePointer = 0;
...
static GT_STATUS appDemoUpdateHwAccessDbState
(
    IN      GT_BOOL                             enable,
...
)
{
    ...
    {
        if (prvOsLogFilePointer >= 0)
        {
            va_end(args);
#ifdef CMD_LUA_CLI
            cmdFSwrite(prvOsLogFilePointer,buffer,osStrlen(buffer));
#endif
        }
    }
}
...
{
    /*if we change from save in file mode to print on screen mode*/
    if ((prvOsLogMode == APP_DEMO_CPSS_LOG_MODE_FILE_E) && (mode == APP_DEMO_CPSS_LOG_MODE_PRINT_E)
        && (prvOsLogFilePointer >= 0))
    {
#ifdef CMD_LUA_CLI
        cmdFSclose(prvOsLogFilePointer);
#endif
        prvOsLogFilePointer = -1;
        return GT_OK;
    }

    prvOsLogMode = mode;

    /*if we want to save the log in file with specific name*/
    if (mode == APP_DEMO_CPSS_LOG_MODE_FILE_E && name != NULL)
    {
#ifdef CMD_LUA_CLI
        prvOsLogFilePointer = cmdFSopen(name,CMD_FS_O_WRONLY|CMD_FS_O_CREAT|CMD_FS_O_TRUNC);
#endif
        if (prvOsLogFilePointer < 0)
        {
            osPrintf("failed to open file: [%s] \n",name);
            return GT_BAD_PTR;
        }
        return GT_OK;
    }

    /*if we want to save the log in file with defult name*/
    if (mode == APP_DEMO_CPSS_LOG_MODE_FILE_E && name == NULL)
    {
#ifdef CMD_LUA_CLI
        prvOsLogFilePointer = cmdFSopen(
        #if (defined _WIN32)
            "c:\\temp\\dtemp\\cpssLog.txt",CMD_FS_O_WRONLY|CMD_FS_O_CREAT|CMD_FS_O_TRUNC);  /*win32*/
        #else
                "testLog.txt",CMD_FS_O_WRONLY|CMD_FS_O_CREAT|CMD_FS_O_TRUNC);                 /*linux*/
        #endif
#endif
        if (prvOsLogFilePointer < 0)
        {
...
{
    if ((prvOsLogMode == APP_DEMO_CPSS_LOG_MODE_FILE_E) && (prvOsLogFilePointer >= 0))
    {
#ifdef CMD_LUA_CLI
        cmdFSclose(prvOsLogFilePointer);
#endif
...
)
{
    ...
    for (devIdx = SYSTEM_DEV_NUM_MAC(0); devIdx < SYSTEM_DEV_NUM_MAC(numOfDev); devIdx++)
    {
            ...
            || (value == 0))
        {
            ...
            if ((appDemoPpConfigList[devIdx].devFamily == CPSS_PP_FAMILY_DXCH_LION3_E)
                || (appDemoPpConfigList[devIdx].devFamily == CPSS_PP_FAMILY_DXCH_BOBCAT2_E)
                || (appDemoPpConfigList[devIdx].devFamily == CPSS_PP_FAMILY_DXCH_BOBCAT3_E)
                )
            {
        ...
        if(PRV_CPSS_PP_MAC(appDemoPpConfigList[devIdx].devNum)->devFamily == CPSS_PP_FAMILY_DXCH_BOBCAT2_E ||
           PRV_CPSS_PP_MAC(appDemoPpConfigList[devIdx].devNum)->devFamily == CPSS_PP_FAMILY_DXCH_BOBCAT3_E )
        {
            appDemoPpConfigList[devIdx].devSupportSystemReset  = GT_TRUE;
        }
        else
...
)
{
    ...
    APP_DEMO_CPSS_HW_ACCESS_DB_STC      **dbPtrPtr;
    APP_DEMO_CPSS_HW_ACCESS_INFO_STC    *infoDbPtr;
    #ifdef ASIC_SIMULATION
    FILE  *filePtr;
    ...
    switch (outputMode)
    {
        ...
        case APP_DEMO_TRACE_OUTPUT_MODE_DB_E:
            ...
            if (accessType == APP_DEMO_TRACE_HW_ACCESS_TYPE_READ_E)
            {
                dbPtrPtr = &readAccessDbPtr;
                infoDbPtr = &readAccessInfo;
            }
            else  /*write and trace in the same db */
            {
                dbPtrPtr = &writeAccessDbPtr;
                infoDbPtr = &writeAccessInfo;
            }
...
)
{
    if (enable == GT_TRUE)
    {
        ...
        if (dbInfoPtr->curState == GT_FALSE)
        {
            if (*dbPtrPtr == NULL)
            {
    ...
    else
    {
        ...
        if (dbInfoPtr->curState == GT_TRUE)
        {
            /* disable HW Access DB */
            dbInfoPtr->curState = GT_FALSE;
        }
        else
    ...
    switch (accessType)
    {
        case APP_DEMO_TRACE_HW_ACCESS_TYPE_READ_E:
            ...
            if (appDemoTraceOutputMode == APP_DEMO_TRACE_OUTPUT_MODE_DB_E)
            {
                ...
                rc = appDemoUpdateHwAccessDbState(enable,
                                                  &readAccessDbPtr,
                                                  &readAccessInfo);
                if (rc != GT_OK)
                {
...
)
{
    ...
    if ((writeAccessInfo.curState == GT_TRUE) ||
        (readAccessInfo.curState == GT_TRUE))
    {
        /* at least one of the access DB is enabled */
        return GT_BAD_STATE;
    }

    if (mode == APP_DEMO_TRACE_OUTPUT_MODE_DB_E)
    {
        if (traceReadHwAccess == GT_TRUE)
        {
            /* update Read HW Access DB */
            rc = appDemoUpdateHwAccessDbState(GT_TRUE,
                                              &readAccessDbPtr,
                                              &readAccessInfo);
            if (rc != GT_OK)
            {
...
)
{
    ...
    {
        case APP_DEMO_TRACE_HW_ACCESS_TYPE_READ_E:
            infoDb = readAccessInfo;
            accessDbPtr = readAccessDbPtr;
            break;
        case APP_DEMO_TRACE_HW_ACCESS_TYPE_WRITE_E:
            infoDb = writeAccessInfo;
            accessDbPtr = writeAccessDbPtr;
            break;
        default:
            return GT_BAD_PARAM;
    }
...
)
{
    ...
    {
        case APP_DEMO_TRACE_HW_ACCESS_TYPE_READ_E:
            if (index<readAccessInfo.curSize)
            {
                dbLine->accessParamsBmp=readAccessDbPtr[index].accessParamsBmp;
                dbLine->data=readAccessDbPtr[index].data;
                dbLine->addr=readAccessDbPtr[index].addr;
                return GT_OK;
            }
             break;
        ...
        case APP_DEMO_TRACE_HW_ACCESS_TYPE_WRITE_E:
        case APP_DEMO_TRACE_HW_ACCESS_TYPE_WRITE_DELAY_E:
            if (index<writeAccessInfo.curSize)
            {
                dbLine->accessParamsBmp=writeAccessDbPtr[index].accessParamsBmp;
                dbLine->data=writeAccessDbPtr[index].data;
                dbLine->addr=writeAccessDbPtr[index].addr;
                return GT_OK;
            }
           break;
        default:
...
#ifdef  INCLUDE_UTF
    if(appDemoFdbUpdateLockMtx)
    {
        /*osPrintSync("cpssResetSystem : delete FDB mutex (no thread should be in it at this time) \n");*/
        cpssOsMutexDelete(appDemoFdbUpdateLockMtx);
        appDemoFdbUpdateLockMtx = 0;
    }
#endif /*INCLUDE_UTF*/
