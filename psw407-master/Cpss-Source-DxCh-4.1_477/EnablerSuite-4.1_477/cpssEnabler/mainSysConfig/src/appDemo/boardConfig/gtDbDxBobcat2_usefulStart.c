...
)
{
    ...
    rc = cpssDxChPortPhysicalPortMapSet(dev,sizeof(bc2TmEnableMap)/sizeof(bc2TmEnableMap[0]),
                                            &bc2TmEnableMap[0]);
    if (rc != GT_OK)
    {
        return rc;
...
)
{
    ...
    rc = cpssDxChPortPhysicalPortMapSet(dev, sizeof(bc2Map)/sizeof(bc2Map[0]),
                                             &bc2Map[0]);
    if (rc != GT_OK)
    {
        return rc;
...
GT_STATUS pa175Mhz_84Gpbs(GT_VOID)
{
    ...
    for (i = 0; i < sizeof(highSpeedPortListConfArr)/sizeof(highSpeedPortListConfArr[0]); i++)
    {
        ...
        rc = prvCpssDxChPortDynamicPATxQHighSpeedPortGet(dev,&getList.size,&getList.list[0],NULL);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = prvCpssDxChPortDynamicPATxQHighSpeedPortDumpGet(dev,&dqList[0], &llList[0]);
        if (rc != GT_OK)
        {
            return rc;
        }
