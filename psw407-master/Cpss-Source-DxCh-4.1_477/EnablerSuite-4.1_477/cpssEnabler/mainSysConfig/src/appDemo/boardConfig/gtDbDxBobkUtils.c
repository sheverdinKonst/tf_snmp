...
extern GT_STATUS prvCpssDxChCaelumTmPortResourseConfigGet
(
    IN  GT_U8                                        devNum,
...
)
{
    ...
    CPSS_PORT_SPEED_ENT speed;
    CPSS_PORT_INTERFACE_MODE_ENT   ifMode;
    PRV_CPSS_DXCH_CAELUM_PORT_TM_RESOURCE_STC    resource;
    ...
    for (portNum = 0 ; portNum < 256; portNum++)
    {
        ...
        if (speed != CPSS_PORT_SPEED_NA_E)
        {
            ...
            rc = prvCpssDxChCaelumTmPortResourseConfigGet(dev, portNum,/*OUT*/ &resource);
            if(rc != GT_OK)
            {
                return rc;
            }
