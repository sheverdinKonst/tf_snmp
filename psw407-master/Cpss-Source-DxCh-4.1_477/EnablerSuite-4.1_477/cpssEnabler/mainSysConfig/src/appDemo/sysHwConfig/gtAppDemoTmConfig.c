...
)
{
    ...
    for (i = 0; i < appDemoPpConfigPtr->portsMapArrLen && tmPorts < *tmPortsArrSizePtr; i++) 
    {
        if (!portsMappingPtr[i].tmEnable)
        {
            continue;
        }
...
)
{
    ...
    for (i = 0; i < portsNum; i++) 
    {
        ...
        rc = cpssTmPortCreate(dev, tmPort, CPSS_TM_PORT_BW_1G_E, &portParams, 1, 1, 1, queuesNum); 
...
)
{
    ...
    for (i = 0; i < portsNum; i++) 
    {
        ...
        total_queues += MIN(cNodesNum*bNodesNum*aNodesNum*queuesNum, tmTreeParams.maxQueues-1);

        if (total_queues > tmTreeParams.maxQueues-1)
        {
    ...
    for (i = 0; i < queue_id_pool_size; i++) 
    {
		rc = cpssDxChTmGlueDropQueueProfileIdSet(0, queue_id_pool[i], 0);
...
*/
    ...
    {
        cNodesNum = 2; /* per port */
        bNodesNum = 6; /* per c node */
        aNodesNum = 8; /* per b node */
        queuesNum = 4; /* per a node */
    }
...
)
{
	...
	for (i = 1; i < (aNodes * 8); i++)
	{
		...
		rc = cpssTmShapingProfileCreate(0, CPSS_TM_LEVEL_Q_E, &profileShaping, &qProfileShapingInd);
		if (rc != GT_OK)
			return rc;
	}
...
)
{
	...
	switch (mode)
	{
		case 0:
		{
			cpssOsPrintf("\n ***CPSS_TM_STRESS_12_PORTS * 31 * 1 * 11 * 4 , Q no drop, Port drop 10G global only \n ");
			...
			port_elig_fun=   CPSS_TM_ELIG_N_PP_E;
			c_node_elig_fun= CPSS_TM_ELIG_N_PP_E;
			b_node_elig_fun= CPSS_TM_ELIG_N_PP_E;
			a_node_elig_fun= CPSS_TM_ELIG_N_PP_E;
		...
		case 1:
		{
			cpssOsPrintf("\n ***CPSS_TM_STRESS_12_PORTS * 1 * 1 * 1 * (31*11*4) , Q no drop, Port drop 10G global only \n ");
			cpssOsPrintf("               Queue priority = q_index % 4  A, B, C ,Port  have propagated priority\n ");
...
/*case 1:*/
		case 3:
		{
			cpssOsPrintf("\n ***CPSS_TM_STRESS_12_PORTS * 31 * 1 * 11 * 4 , Q no drop\n ");
		...
		case 4:
		{
			cpssOsPrintf("\n ***CPSS_TM_STRESS_12_PORTS * 1 * 1 * 1 * (31*11*4) , Q no drop \n ");
			cpssOsPrintf("               all Queues , A, B, C ,Ports  have  equal priority 0 \n ");
			cpssOsPrintf("               NO DROP ON PORT LEVEL, NO SHAPING ON PORT LEVEL !!!\n ");
			portParams.numOfChildren=1;
			cParams.numOfChildren=1;
 ...
 /*   queueParams.dropProfileInd = queueDropProfileId; */
    ...
    {
        /*create port*/
        rc = cpssTmAsymPortCreate(dev, i, CPSS_TM_PORT_BW_10G_E, &portParams);
        ...
        {
            /*create c node*/
            rc = cpssTmCnodeToPortCreate(dev, i, &cParams, &cNodeInd);
            CPSS_ENABLER_DBG_TRACE_RC_MAC("cpssTmCnodeToPortCreate", rc);
            if (rc != GT_OK)
            {
                return rc;
...
extern GT_STATUS appDemoTmInitStressConfiguration
(
    IN  GT_U8   dev,
	IN  GT_U8   param
);
...
)
{
	...
	cpssOsPrintf("\n appDemoTmInitStressConfiguration input param :: %d\n", param);
    rc = appDemoTmStressScenarioInit(dev, param);
    CPSS_ENABLER_DBG_TRACE_RC_MAC("appDemoTmInitStressConfiguration", rc);
	if (rc == GT_OK)
	{
		cpssOsPrintf("\n appDemoTmInitStressConfiguration dev=%d , param=%d: successful  \n", dev, param);
	}
	else
	{
		cpssOsPrintf("\n             appDemoTmInitStressConfiguration dev=%d , param=%d: failed  rc=%d \n",dev,param,rc);
	}
	return rc;
}
...
)
{
    ...
    cpssOsPrintf("\n ***********CPSS_TM_2  + ports 128::191  -> 4 queues per port transparent\n ");
...
{
    GT_STATUS   rc;         /* return code */
    rc = appDemoTmTxFifoScenarioInit(0);
