                      ...
                      GT_U32 thresholdRatio)
{
	...
	switch (dropMode)
	{
		case CPSS_TM_DROP_MODE_CB_TD_CA_WRED_E:
			profile.cbTdThresholdBytes = cbMBytesThreshold * 1024 * 1024 * thresholdRatio /100;  /* make it big - to have no influence*/
			...
			for (i=0; i < 3 ; i++)
			{
				...
				profile.caWredDp.caWredTdMaxThreshold[i] = caMBytesThreshold * 1024 * 1024 * thresholdRatio /100;
 			}
			break;
		case CPSS_TM_DROP_MODE_CB_TD_CA_TD_E:
			...
			for (i=0; i < 3 ; i++)
			{
				profile.caTdDp.caTdThreshold[i]= caMBytesThreshold * 1024 * 1024 * thresholdRatio /100;
			}
			break;
		case CPSS_TM_DROP_MODE_CB_TD_ONLY_E:
			profile.cbTdThresholdBytes = cbMBytesThreshold * 1024 * 1024 * thresholdRatio /100;
			break;
		default:break;
	}
...
static GT_STATUS appDemoDropProfilePrint(CPSS_TM_LEVEL_ENT level, GT_U32 cos, GT_U32 profileInd, CPSS_TM_DROP_PROFILE_PARAMS_STC *profilePtr)
{
    ...
    {
    case CPSS_TM_DROP_MODE_CB_TD_CA_WRED_E:
        sprintf(modeStr,"ColorBlind TD + WRED Color Aware");
    ...
    case CPSS_TM_DROP_MODE_CB_TD_CA_TD_E:
        sprintf(modeStr,"ColorBlind TD + Color Aware TD");
        osPrintf("\nDrop Profile: index: %d, level: %s, cos: %d, mode: %s\n",
                profileInd,
                tmTreelevelNamesArr[level],
                cos,
...
set TD profiles and WRED curves
	...
	TDProfileCreate 0,2,500,500,0,0,100
	TDProfileCreate 0,0,500,500,1,0,100
	TDProfileCreate 0,0,500,500,1,7,100
	TDProfileCreate 0,0,500,500,2,0,100
	TDProfileCreate 0,0,500,500,3,0,100
	TDProfileCreate 0,0,500,500,4,0,100
	TDProfileCreate 0,0,500,500,4,7,100
	TDProfileCreate 0,0,500,500,4,14,100
...
/* configure PFC TM glue layer */
	rc = cpssDxChTmGluePfcResponseModeSet(0,CPSS_DXCH_TM_GLUE_PFC_RESPONSE_MODE_TM_E);
	if(rc != GT_OK)
		return rc;
...
GT_STATUS tmB0RegSet()
{
    GT_STATUS rc = GT_OK;

	rc = cpssDrvPpHwRegisterWrite(0, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, 0x27000724, 0x2e);
	if( GT_OK != rc)
	   return rc;

	rc = cpssDrvPpHwRegisterWrite(0, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, 0x2700012C, 0x20);
	if( GT_OK != rc)
	   return rc;
	rc = cpssDrvPpHwRegisterWrite(0, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, 0x26003920, 0x3b);
	if( GT_OK != rc)
	   return rc;
	rc = cpssDrvPpHwRegisterWrite(0, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, 0x2600392C, 0x3e8fa);
	if( GT_OK != rc)
	   return rc;
	rc = cpssDrvPpHwRegisterWrite(0, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, 0x26003938, 0x1f0000);
	if( GT_OK != rc)
	   return rc;
	rc = cpssDrvPpHwRegisterWrite(0, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, 0x26003930, 0xa0201);
	if( GT_OK != rc)
	   return rc;
	rc = cpssDrvPpHwRegisterWrite(0, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, 0x40000A00, 0x1ffff);
	if( GT_OK != rc)
	   return rc;
	rc = cpssDrvPpHwRegisterWrite(0, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, 0x3D0A0004, 0x81);
	if( GT_OK != rc)
	   return rc;
	rc = cpssDrvPpHwRegisterWrite(0, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, 0x4000100C, 0x401);
	if( GT_OK != rc)
	   return rc;
	rc = cpssDrvPpHwRegisterWrite(0, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, 0x40001480, 0x0f);
	if( GT_OK != rc)
	   return rc;
	rc = cpssDrvPpHwRegisterWrite(0, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, 0x40000248, 0xe02);
	if( GT_OK != rc)
	   return rc;

	rc = cpssDrvPpHwRegisterWrite(0, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, 0x40001904, 0x4040404);
	if( GT_OK != rc)
	   return rc;
	rc = cpssDrvPpHwRegisterWrite(0, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, 0x40001a0c, 3);
	if( GT_OK != rc)
	   return rc;
