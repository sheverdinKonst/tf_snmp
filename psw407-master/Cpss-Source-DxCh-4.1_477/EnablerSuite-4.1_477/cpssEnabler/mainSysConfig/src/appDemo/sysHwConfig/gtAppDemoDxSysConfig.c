    ...
    switch(PRV_CPSS_DXCH_PP_MAC(dev)->portGroupsExtraInfo.fdbMode)
    {
        ...
        case PRV_CPSS_DXCH_MULTI_PORT_GROUP_FDB_MODE_NON_MULTI_PORT_GROUP_DEVICE_E:
            if((PRV_CPSS_PP_MAC(dev)->devFamily == CPSS_PP_FAMILY_DXCH_LION_E)||
               ...
               (PRV_CPSS_PP_MAC(dev)->devFamily == CPSS_PP_FAMILY_DXCH_LION3_E))
            {
                actionMode = CPSS_FDB_ACTION_AGE_WITHOUT_REMOVAL_E;
            }
            break;
...
)
{
    ...
    if(PRV_CPSS_DXCH_PP_MAC(cpssPpPhase2Info.newDevNum)->hwInfo.eArch.supported == GT_TRUE)
    {
        ...
        numOfEports =
            PRV_CPSS_DXCH_MAX_PORT_NUMBER_MAC(cpssPpPhase2Info.newDevNum);
...
)
{
    ...
    cpssPpPhase1InfoPtr->enableEarchSupport = appCpssPpPhase1ParamsPtr->enableEarchSupport;
...
{
    /* IP Unicast */
    ppLogicalInfoPtr->maxNumOfVirtualRouters = ppConfigPtr->maxNumOfVirtualRouters;
    ppLogicalInfoPtr->maxNumOfIpNextHop = ppConfigPtr->maxNumOfIpNextHop;

    /* IPv4 Unicast */
    ppLogicalInfoPtr->maxNumOfIpv4Prefixes = ppConfigPtr->maxNumOfIpv4Prefixes;

    /* IPv4/IPv6 Multicast */
    ppLogicalInfoPtr->maxNumOfIpv4McEntries = ppConfigPtr->maxNumOfIpv4McEntries;
    ppLogicalInfoPtr->maxNumOfMll = ppConfigPtr->maxNumOfMll;

    /* IPv6 Unicast */
    ppLogicalInfoPtr->maxNumOfIpv6Prefixes = ppConfigPtr->maxNumOfIpv6Prefixes;

    /* Tunnels */
    ppLogicalInfoPtr->maxNumOfTunnelEntries = ppConfigPtr->maxNumOfTunnelEntries;

    /* ipv4 tunnel termination */
    ppLogicalInfoPtr->maxNumOfIpv4TunnelTerms = ppConfigPtr->maxNumOfIpv4TunnelTerms;
...
)
{
    ...
    rc = cpssDxChTrunkInit(dev, sysConfigParamsPtr->numOfTrunks ,
...
)
{
    ...
    for(port = 0 ; port < 256 /*support 256 ports in bc2*/; port++)
    {
        /* set the port in bmp */
    ...
    {
        /* read the 12 registers */
        for(ii = 0 ; ii < 6 ; ii++)
        {
           /*<MG_IP> MG_IP/MG Registers/Address Decoding/Base Address %n*/
            rc = prvCpssDrvHwPpReadRegister(devNum,mgDecodingAddress + ii*8 + 0,&mgDecodingValue[2*ii + 0]);
            ...
            rc = prvCpssDrvHwPpReadRegister(devNum,mgDecodingAddress + ii*8 + 4,&mgDecodingValue[2*ii + 1]);
            CPSS_ENABLER_DBG_TRACE_RC_MAC("prvCpssDrvHwPpReadRegister", rc);
            if (rc != GT_OK)
            {
                return rc;
            ...
            rc = prvCpssDrvHwPpReadRegister(devNum,mgWindowControlAddress + ii*4 ,&mgWindowControlValue[ii]);
            CPSS_ENABLER_DBG_TRACE_RC_MAC("prvCpssDrvHwPpReadRegister", rc);
            if (rc != GT_OK)
            {
                return rc;
    ...
    if(doMgDecodingRestore)
    {
        ...
        */
        ...
        for(ii = 0 ; ii < 6 ; ii++)
        {
            ...
            /*<MG_IP> MG_IP/MG Registers/Address Decoding/Window Control Register%n*/
            address = mgWindowControlAddress + ii*4;

            rc = cpssDxChDiagRegWrite(deviceBaseAddrOnPex,
                    ifChannel,
                    CPSS_DIAG_PP_REG_INTERNAL_E,
                    address,
                    mgWindowControlValue[ii],
                    doByteSwap);
            CPSS_ENABLER_DBG_TRACE_RC_MAC("cpssDxChDiagRegWrite", rc);
            if (rc != GT_OK)
            {
