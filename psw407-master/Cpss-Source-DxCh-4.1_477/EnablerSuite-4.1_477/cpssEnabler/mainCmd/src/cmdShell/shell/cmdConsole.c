...
*
*******************************************************************************/
GT_STATUS cmdTakeGlobalStdio(cmdStreamPTR IOStream)
{
...
)
{
...
#ifdef  CMD_LUA_CLI
    if (((IOStream->flags & STREAM_FLAG_C_FIRSTSHELL) != 0)
    ...
    {
#ifdef  CMD_LUA_CLI
        if ((IOStream->flags & STREAM_FLAG_C_FIRSTSHELL) && 
            (inputBuffer[0] != 0))
        {
        ...
        IOStream->flags &= ~STREAM_FLAG_C_FIRSTSHELL;
#ifdef  CMD_LUA_CLI
