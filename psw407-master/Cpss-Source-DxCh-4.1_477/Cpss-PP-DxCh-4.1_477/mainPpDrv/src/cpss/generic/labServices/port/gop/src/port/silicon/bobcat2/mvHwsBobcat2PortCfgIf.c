...
)
{
	...
	if (HWS_DEV_IS_BOBCAT2_A0(devNum))
	{
		...
		case _40GBase_KR:
		case _40GBase_SR_LR:
			mvUnitExtInfoGet(devNum, MMPCS_UNIT, (GT_U8)curPortParams->portPcsNumber, &unitAddr, &unitIndex);
	...
	CHECK_STATUS(mvHwsBobcat2PortFixAlign90Ext(devNum, portGroup, phyPortNum, portMode, optAlgoMask));
...
)
{
	...
	{
		return mvHwsBobcat2PortRxAutoTuneSetExt(devNum, portGroup, phyPortNum, portMode,
								portMode, portTuningMode, results);
	}
	else
	{
		return mvHwsBobcat2PortTxAutoTuneSetExt(devNum, portGroup, phyPortNum, portMode,
						portTuningMode, optAlgoMask);
	}
}
