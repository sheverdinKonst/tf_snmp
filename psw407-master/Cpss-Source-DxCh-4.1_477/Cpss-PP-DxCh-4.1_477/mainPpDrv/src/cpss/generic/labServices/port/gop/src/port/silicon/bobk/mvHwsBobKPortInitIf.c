...
)
{
	...
	if (serdesInit == GT_TRUE)
	{
		...
		case SGMII:
		case QSGMII:
			CHECK_STATUS(mvHwsSerdesManualTxConfig(devNum, portGroup, MV_HWS_SERDES_NUM(curLanesList[0]),
								                   HWS_DEV_SERDES_TYPE(devNum), 0x17, GT_TRUE, 0x3, 0x3, GT_FALSE /* set slew rate to fastest */));
			break;
		default:
			break;
		}
...
)
{
	...
	CHECK_STATUS(mvHwsPortStopCfg(devNum, portGroup, phyPortNum, portMode, action, &(curLanesList[0])));
