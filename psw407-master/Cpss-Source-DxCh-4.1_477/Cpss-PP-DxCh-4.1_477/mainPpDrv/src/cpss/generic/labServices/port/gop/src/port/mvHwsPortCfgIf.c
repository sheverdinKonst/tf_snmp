...
GT_BOOL skipPcsUnreset = GT_TRUE;
...
*
* DESCRIPTION:
*       Stop Tx and Rx training (mainly used by AP process)
*
* INPUTS:
*       devNum    - system device number
*       portGroup - port group (core) number
...
*
*******************************************************************************/
GT_STATUS mvHwsPortAutoTuneStopExt
(
	GT_BOOL skipPcsUnresetFlag
)
{
	if (skipPcsUnresetFlag == GT_TRUE)
	{
		return GT_OK;
	}
...
)
{
	...
	/* TBD: This function is in uses for Lion2/Hooper.
	It shouldn't be used for BC2/AC3 (i.e. GOP_REV_28NM and above)!!
	Need to add AutoTune algorithm for 28nm Serdes revision. */
	...
	if (hwsDevFunc[HWS_DEV_SILICON_TYPE(devNum)].portAutoTuneStopFunc != NULL)
		if (hwsDevFunc[HWS_DEV_SILICON_TYPE(devNum)].portAutoTuneStopFunc(skipPcsUnreset) == GT_OK)
			return GT_OK;
