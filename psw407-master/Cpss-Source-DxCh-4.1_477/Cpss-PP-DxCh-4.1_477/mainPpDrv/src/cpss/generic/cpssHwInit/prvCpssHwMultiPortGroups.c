...
)
{
    ...
    if(regValue == 1)
    {
        ...
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_ONE_PARAM_FORMAT_MAC(regValue));
    }
...
)
{
    ...
    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum,portGroupsBmp,portGroupId)
    {
        ...
        {
            rc = portGroupBusyWait(devNum,portGroupId,additionalRegDup,selfClearBit,onlyCheck);
            if(rc != GT_OK)
            {
                return rc;
            }
...
)
{
    ...
    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum,portGroupsBmp,portGroupId)
    {
        ...
        if (isAdditionalRegDupNeeded == GT_TRUE)
        {
            rc = portGroupBusyWaitByMask(devNum,portGroupId,additionalRegDup,regMask,onlyCheck);
            if(rc != GT_OK)
            {
                return rc;
            }
...
)
{
    ...
    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum,portGroupsBmp,portGroupId)
    {
        ...
        if(isAdditionalRegDupNeeded == GT_TRUE)
        {
            rc = prvCpssDrvHwPpPortGroupReadRegister(devNum,portGroupId,additionalRegDup,&coreValue);
            if(rc != GT_OK)
            {
                return rc;
            }
