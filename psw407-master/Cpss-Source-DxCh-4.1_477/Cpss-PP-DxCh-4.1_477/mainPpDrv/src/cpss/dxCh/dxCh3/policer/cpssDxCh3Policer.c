...
)
{
    ...
    rc = prvCpssDrvHwPpPortGroupSetRegField(devNum, portGroupId, plrRegAddr,
    ...
    {
        rc = prvCpssDrvHwPpPortGroupSetRegField(devNum, portGroupId, pclRegAddr,
...
)
{
    ...
    retStatus = prvCpssDrvHwPpPortGroupGetRegField(devNum, portGroupId,plrRegAddr,
                                                   plrRegOffset, 1, &regValue);
    if(GT_OK == retStatus)
    {
...
)
{
    ...
    return prvCpssDrvHwPpSetRegField(devNum, regAddr, 0, numOfBits, mruSize);
}
...
)
{
    ...
    return prvCpssDrvHwPpGetRegField(devNum, regAddr, 0, numOfBits, mruSizePtr);
}
...
)
{
    ...
    if(PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_MAC(devNum) == GT_TRUE)
    {
        ...
        retStatus = prvCpssDrvHwPpWriteRam(devNum, regAddr, 8 , hwFormatArray);
        if (retStatus != GT_OK)
        {
            return retStatus;
        }
...
)
{
    ...
    switch(meterModeVal)
    {
        ...
        case 5:
            ...
            break;
        default:
            return GT_BAD_STATE; /* never should be reached*/
    }
...
)
{
    ...
    else
    {
        ...
        retStatus = prvCpssDrvHwPpPortGroupReadRam(devNum, portGroupId, regAddr,
                                                   5, &hwDataArr[0]);
        hwData = &hwDataArr[0];
    }
...
)
{
    ...
    {
        /* convert entry */
        rc = prvCpssDxChPolicerBillingCntrToHwConvert(devNum, billingCntrPtr,
...
)
{
    ...
    GT_U32      billingCntrModeHwVal;   /* Billing counter mode hw value */
    GT_U32      portGroupId;            /* port group id */
    GT_U32      portGroupIdTmp;         /* port group id */
    ...
    if (PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
    {
        ...
        /* loop on all port groups to get the green counter snapshot valid bit */
        PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum, portGroupsBmp,
                                                          portGroupIdTmp)
        {
            retStatus = prvCpssDrvHwPpPortGroupGetRegField(devNum, portGroupIdTmp,
...
)
{
    ...
    retStatus = prvCpssDrvHwPpReadRam(devNum, regAddr, 1, hwDataArr);
...
)
{
    ...
    GT_U32      ram0Size;       /* RAM#0 size, used for device with
                                   3 memories like xCat2, Lion3 and Bobcat2 */
    ...
    switch (PRV_CPSS_PP_MAC(devNum)->devFamily)
    {
        ...
        case CPSS_PP_FAMILY_DXCH_BOBCAT3_E:
                /*
                    ...
                    Memory 2 : 0x0100 (256)
                */
             ram0Size = 0x3c00; CPSS_TBD_BOOKMARK_BOBCAT3
            break;
        case CPSS_PP_FAMILY_DXCH_BOBCAT2_E:
            ...
            break;
        default: /* Lion3 - need to update to 8K after simulation alignment */
    ...
    if(PRV_CPSS_SIP_5_15_CHECK_MAC(devNum))
    {
        ...
        retStatus = prvCpssDrvHwPpSetRegField(devNum, regAddr, 0, 24, iplr0MemSize);
        if (retStatus != GT_OK)
        {
            return retStatus;
        }
...
)
{
    ...
    return prvCpssDrvHwPpSetRegField(devNum, regAddr, 5, 1, BOOL2BIT_MAC(enable));
}
...
)
{
    ...
    rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, regOffset, length, plrIndexMask);
...
)
{
    ...
    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, regOffset, length, plrIndexMaskPtr);
...
)
{
    ...
    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, 5, 1, &regValue);
    if(GT_OK != rc)
    {
        return rc;
    }
...
)
{
    ...
    rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 16, 1, value);
...
)
{
    ...
    regAddr =
        PRV_CPSS_DXCH_DEV_REGS_MAC(devNum)->bridgeRegs.bridgeGlobalConfigRegArray[2];
    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, 16, 1, &value);
    if (rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 0, 17, threshold);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 0, 17, mask);
...
)
{
    ...
    regAddr =
        PRV_DXCH_REG1_UNIT_PLR_MAC(devNum, stage).meteringAddressingModeCfg0;
    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, 0, 17, thresholdPtr);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    regAddr =
        PRV_DXCH_REG1_UNIT_PLR_MAC(devNum, stage).meteringAddressingModeCfg1;
    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, 0, 17, &mask);
    if (rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 0, 17, hwRate);
...
)
{
    ...
    regAddr = (bucketIndex == 0)
        ? PRV_DXCH_REG1_UNIT_PLR_MAC(devNum, stage).mef10_3_bucket0MaxRateArr[entryIndex]
        : PRV_DXCH_REG1_UNIT_PLR_MAC(devNum, stage).mef10_3_bucket1MaxRateArr[entryIndex];
    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, 0, 17, &maxHwRate);
    if (rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, offset, 3, priority);
...
)
{
    ...
    regAddr = PRV_DXCH_REG1_UNIT_PLR_MAC(devNum, stage).qosProfile2PriorityMapArr[entryIndex];
    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, offset, 3, priorityPtr);
