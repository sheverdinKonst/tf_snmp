...
*
* APPLICABLE DEVICES:
*        DxCh1; DxCh1_Diamond; DxCh2; DxCh3; xCat; xCat3; Lion; Lion2; Lion3; xCat2; Bobcat2.
*
...
)
{
    ...
    {/* ensure last packets passed pipe */
        ...
        {
            rc = prvCpssDxChCaelumPortRegAddrGet(devNum, portNum, PRV_CAELUM_REG_FIELDS_TXDMA_DESC_CREDITS_SCDMA_E, &regAddr, &fieldOffset, &fieldLen);
            if(rc != GT_OK)
            {
                return rc;
            }

            rc = prvCpssHwPpPortGroupGetRegField(devNum, portGroupId, regAddr, fieldOffset, fieldLen, &fifoThresholdCounters);
            if(rc != GT_OK)
            {
                return rc;
            }

            rc = prvCpssDxChCaelumPortRegAddrGet(devNum, portNum, PRV_CAELUM_REG_FIELDS_TXQ_DQ_TXDMA_PORT_CREDIT_COUNTER_E, &regAddr, &fieldOffset, &fieldLen);
            if(rc != GT_OK)
            {
                return rc;
            }
            ...
            for(timeout = 100; timeout > 0; timeout--)
            {
                rc = prvCpssHwPpPortGroupGetRegField(devNum, portGroupId, regAddr, fieldOffset, fieldLen, &txdmaPortCreditsCounters);
...
)
{
    ...
    if (dMode == CPSS_PORT_HALF_DUPLEX_E)
    {
        if ( (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_LION_E)  ||
             ...
             (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_LION3_E) ||
...
)
{
    ...
    if(fcAutoNegRegAddr != PRV_CPSS_SW_PTR_ENTRY_UNUSED)
    {
        ...
        regDataArray[PRV_CPSS_PORT_GE_E].fieldOffset = 11;
        regDataArray[PRV_CPSS_PORT_GE_E].fieldLength = 1;
    }
    ...
    {
        regDataArray[PRV_CPSS_PORT_GE_E].regAddr = fcAutoNegRegAddr;
        regDataArray[PRV_CPSS_PORT_GE_E].fieldData = pauseAdvValue;
...
*
*       Note: In Bobcat2/Lion3 CPSS_PORT_PERIODIC_FLOW_CONTROL_XON_ONLY_E
...
)
{
    ...
    if(PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_MAC(devNum) == GT_TRUE)
    {
        /* Reset PTP unit after port state is disabled. RESET and UNRESET of PTP should be
          done after MAC RESET. The code below use MAC RESET/UNRESET. */
...
)
{
    ...
    return GT_FAIL;/* error ... no valid ports */
}
...
)
{
        ...
        || (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_XCAT3_E))
    {
        ...
        /* patch for Lion3 (until supported by 'HW services')*/
        /* override the 'base addresses' that are set within hwsLion2IfInit() for the devices */
        if(CPSS_PP_FAMILY_DXCH_LION3_E == PRV_CPSS_PP_MAC(devNum)->devFamily)
        {
            /* next defines comes from :
                <pss/generic/port/portHwConfig/src/port/silicon/mvHwsLion2If.c> */
            ...
            regAddr = LION2_GIG_BASE;
            convertRegAddrToNewUnitsBaseAddr(devNum,&regAddr);
			CPSS_LOG_INFORMATION_MAC("Calling: mvUnitInfoSet(devType[%d], unitId[%d], baseAddr[%d], unitIndex[%d])", hwsDeviceSpecInfo[devNum].devType, GEMAC_UNIT, regAddr, GIG_IND_OFFSET);
            mvUnitInfoSet(hwsDeviceSpecInfo[devNum].devType, GEMAC_UNIT, regAddr, GIG_IND_OFFSET);
            regAddr = LION2_MMPCS_BASE;
            convertRegAddrToNewUnitsBaseAddr(devNum,&regAddr);
			CPSS_LOG_INFORMATION_MAC("Calling: mvUnitInfoSet(devType[%d], unitId[%d], baseAddr[%d], unitIndex[%d])", hwsDeviceSpecInfo[devNum].devType, MMPCS_UNIT, regAddr, MMPCS_IND_OFFSET);
            mvUnitInfoSet(hwsDeviceSpecInfo[devNum].devType, MMPCS_UNIT, regAddr, MMPCS_IND_OFFSET);
            regAddr = LION2_XPCS_BASE;
            convertRegAddrToNewUnitsBaseAddr(devNum,&regAddr);
			CPSS_LOG_INFORMATION_MAC("Calling: mvUnitInfoSet(devType[%d], unitId[%d], baseAddr[%d], unitIndex[%d])", hwsDeviceSpecInfo[devNum].devType, XPCS_UNIT, regAddr, XPCS_IND_OFFSET);
            mvUnitInfoSet(hwsDeviceSpecInfo[devNum].devType, XPCS_UNIT, regAddr, XPCS_IND_OFFSET);
            regAddr = LION2_MMPCS_BASE;
            convertRegAddrToNewUnitsBaseAddr(devNum,&regAddr);
			CPSS_LOG_INFORMATION_MAC("Calling: mvUnitInfoSet(devType[%d], unitId[%d], baseAddr[%d], unitIndex[%d])", hwsDeviceSpecInfo[devNum].devType, HGLMAC_UNIT, regAddr, MMPCS_IND_OFFSET);
            mvUnitInfoSet(hwsDeviceSpecInfo[devNum].devType, HGLMAC_UNIT, regAddr, MMPCS_IND_OFFSET);
            regAddr = LION2_XLG_BASE;
            convertRegAddrToNewUnitsBaseAddr(devNum,&regAddr);
			CPSS_LOG_INFORMATION_MAC("Calling: mvUnitInfoSet(devType[%d], unitId[%d], baseAddr[%d], unitIndex[%d])", hwsDeviceSpecInfo[devNum].devType, XLGMAC_UNIT, regAddr, MSM_IND_OFFSET);
            mvUnitInfoSet(hwsDeviceSpecInfo[devNum].devType, XLGMAC_UNIT, regAddr, MSM_IND_OFFSET);
            regAddr = LION2_SERDES_BASE;
            convertRegAddrToNewUnitsBaseAddr(devNum,&regAddr);
			CPSS_LOG_INFORMATION_MAC("Calling: mvUnitInfoSet(devType[%d], unitId[%d], baseAddr[%d], unitIndex[%d])", hwsDeviceSpecInfo[devNum].devType, SERDES_UNIT, regAddr, SERDES_IND_OFFSET);
            mvUnitInfoSet(hwsDeviceSpecInfo[devNum].devType, SERDES_UNIT, regAddr, SERDES_IND_OFFSET);
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum,
                                            CPSS_CH1_E      | CPSS_CH1_DIAMOND_E    | CPSS_CH2_E    |  CPSS_CH3_E      | CPSS_LION3_E |  
