...
extern GT_U32 prvDxChLion3UnitsBaseAddr  [PRV_CPSS_DXCH_UNIT_LAST_E + 1];  /* Lion2 unit base address array */
...
extern GT_U32 prvDxChBobcat3UnitsBaseAddr[PRV_CPSS_DXCH_UNIT_LAST_E + 1];  /* Bobk unit base address array */
...
/* filled in runtime */
static PRV_CPSS_DXCH_UNIT_ENT prvDxChLion3UnitIdPer8MSBitsBaseAddrArr[NUM_ENTRIES_FOR_8_MSBITS_BASE_ADDR_CNS] =
{
    0,
};
/* filled in runtime */
...
#define REG_NAME_AND_SIZE_FOR_2_UNITS_MAC(devNum, unitName1,unitName2 , regName ,startIndexUnit1,startIndexUnit2,numOfPorts, \
        saveLastNumOfPorts,saveToIndex) \
...
*       saveLastNumOfPorts - number of ports from the end of the unitStart1Ptr[]
*                           that need to be saved before overridden by unitStart2Ptr[]
*       saveToIndex - the index in unitStart1Ptr[] to save the 'last ports'
...
static void regUnitPerPortAddrUpdate(
    ...
    IN GT_U32    numOfPorts,
    IN GT_U32    saveLastNumOfPorts,
    IN GT_U32    saveToIndex
)
{
    GT_U32  ii,jj,kk;
    GT_U32  mm;
    ...
    if(saveLastNumOfPorts > 0)
        /* save last indexes to other place to will not be totally deleted */
        for(mm = 0; mm < saveLastNumOfPorts; mm++)
        {
            unitStart1Ptr[saveToIndex + mm] = unitStart1Ptr[numOfPorts - saveLastNumOfPorts + mm];
        }
...
)
{
    ...
    switch(prvCpssDrvPpConfig[devNum]->devFamily)
    {
        ...
        case CPSS_PP_FAMILY_DXCH_LION3_E:
            baseAddr = prvDxChLion3UnitsBaseAddr[unitId];
            break;
...
)
{
    ...
    {/*start of unit EQ */
        ...
        if(!isSip5_20)/*register is not used any way from sip5 , as feature removed */
        {/*start of unit logicalTargetMapTable */
            regsAddrPtr->EQ.logicalTargetMapTable.logicalTargetDeviceMapConfig = 0x0000a004;
...
{
    PRV_CPSS_DXCH_PP_REGS_ADDR_VER1_STC *regsAddrPtr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum);
    GT_U32  numPhyPorts = PRV_CPSS_DXCH_MAX_PHY_PORT_NUMBER_MAC(devNum);
    ...
    {/*start of unit EGF_eft */
        ...
        {/*start of unit egrFilterConfigs */
            ...
            {/*10120+4*n*/
                GT_U32    n;
                GT_U32    offset = isSip5_20 ? 0x10200 : 0x10120;
                GT_U32    numRegs = isSip5_20 ? 256 : (8*numWordsForBitPerPort);
                for(n = 0 ; n < numRegs ; n++) {
            ...
            {/*10220+4*n*/
                GT_U32    n;
                GT_U32    offset = isSip5_20 ? 0x10600 : 0x10220;
                for(n = 0 ; n < (2*numWordsForBitPerPort) ; n++) {
...
{
    PRV_CPSS_DXCH_PP_REGS_ADDR_VER1_STC *regsAddrPtr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum);
    GT_U32  lastRxDmaIndex = (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_BOBCAT3_E) ? 12 : 72;
...
)
{
    ...
    regUnitPerPortAddrUpdate(
        REG_NAME_AND_SIZE_FOR_2_UNITS_MAC(devNum, rxDMA[0] , rxDMA[currentUnitIndex] , singleChannelDMAConfigs.SCDMAConfig0 ,
            portIn_unit_0 , portInCurrentUnit,numOfPorts,saveLastNumOfPorts,saveToIndex)
        );
    regUnitPerPortAddrUpdate(
...
{
    GT_U32  ii,jj;
    GT_U32 supportDualDataPath = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.multiDataPath.supportMultiDataPath;
    ...
    for(jj = 0 ; jj < MAX_DP_CNS ; jj++)
    {
        ...
        */
        ...
        if(ii != 0)/* have multi data path , set 'global' ports */
        {
...
)
{
    ...
    if(PRV_CPSS_SIP_5_15_CHECK_MAC(devNum)) /* bobk*/
    {
        ...
        num_rxDMANextUpdatesFIFOMaxFillLevel    = 3     +1;/*1 less then bc2*/
        num_rxDMANextUpdatesFIFOFillLevel       = 3     +1;/*1 less then bc2*/
        num_SCDMAInProgRegs                     = 147   +1;/*much more then bc2*/
    }
    else /* bobcat2 */
    {
...
)
{
    IN GT_U32    saveLastNumOfPorts=0;/*number of ports from the end of the unitStart1Ptr[]
                                  that need to be saved before overridden by unitStart2Ptr[]*/
    IN GT_U32    saveToIndex=0;/*the index in unitStart1Ptr[] to save the 'last ports'
                                  valid when saveLastNumOfPorts != 0*/
...
)
{
    ...
    switch(prvCpssDrvPpConfig[devNum]->devFamily)
    {
        case CPSS_PP_FAMILY_DXCH_LION2_E:
            ...
            break;
        case CPSS_PP_FAMILY_DXCH_LION3_E:
            unitIdPtr = prvDxChLion3UnitIdPer8MSBitsBaseAddrArr;
            break;
        case CPSS_PP_FAMILY_DXCH_BOBCAT2_E:
            forceBobk_lbl:
...
)
{
    ...
    {
        /* unitBasePtr = prvDxChLion3UnitsBaseAddr; */
        unitIdPtr = prvDxChLion3UnitIdPer8MSBitsBaseAddrArr;
        supportTM = GT_FALSE;
    }
    else
    ...
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_BOBCAT2_E)
    {
        ...
        {
            unitBasePtr = prvDxChBobkUnitsBaseAddr;
        }
        else
        {
            unitBasePtr = prvDxChBobcat2UnitsBaseAddr;
        }
        */
        unitIdPtr = prvDxChBobcat2UnitIdPer8MSBitsBaseAddrArr;
    }
