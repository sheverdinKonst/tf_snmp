...
)
{
    ...
    */
    ...
    if(prvCpssFamilyInfoArray[devFamily] == NULL)
    {
        ...
        if(CPSS_PP_FAMILY_DXCH_LION3_E > devFamily)
        {
...
)
{
         ...
         (tempSystemRecoveryInfo.systemRecoveryProcess == CPSS_SYSTEM_RECOVERY_PROCESS_HA_E) )
    {
        ...
        if ( (useFuQueue == GT_TRUE) && (tempSystemRecoveryInfo.systemRecoveryMode.continuousFuMessages == GT_FALSE) )
        {
            ...
            rc = prvCpssDxChHaFuqNonContinuesMsgModeHandle(devNum);
            if (rc != GT_OK)
            {
        ...
        if(tempSystemRecoveryInfo.systemRecoveryMode.continuousAuMessages == GT_FALSE)
        {
            ...
            rc = prvCpssDxChHaAuqNonContinuesMsgModeHandle(devNum);
            if (rc != GT_OK)
            {
                systemRecoveryInfo.systemRecoveryState = tempSystemRecoveryInfo.systemRecoveryState;
                return rc;
...
)
{
    ...
    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, startBit, 1, &regValue);
    if(GT_OK != rc)
    {
        return rc;
    }
...
)
{
    ...
    switch(table)
    {
        ...
        case CPSS_DXCH_CFG_TABLE_QOS_PROFILE_E:
            ...
            {
                CPSS_TBD_BOOKMARK
                /* the EQ supports 1K , but the PLR , HA still not .*/
                numEntries = PRV_CPSS_DXCH2_QOS_PROFILE_NUM_MAX_CNS;/*128*/
            }
...
)
{
    ...
    rc = prvCpssDrvHwPpPortGroupGetRegField(devNum, portGroupId, regAddr ,0 ,fieldOffset ,&value);
    if(rc != GT_OK)
    {
        return rc;
    }
