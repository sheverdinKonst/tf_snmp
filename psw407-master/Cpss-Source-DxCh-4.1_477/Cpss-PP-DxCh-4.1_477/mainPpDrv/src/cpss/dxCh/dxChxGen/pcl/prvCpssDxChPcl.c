...
/* conversion table for lion3 Common.                                         */
...
/* for lion3 -- fmtFldLion2_INGRESS_EXT_IPV4_PORT_VLAN_QOS */
static CPSS_FORMAT_CONVERT_FIELD_INFO_STC
...
/* for lion3 -- fmtFldLion2_INGRESS_ULTRA_IPV6_PORT_VLAN_QOS */
static CPSS_FORMAT_CONVERT_FIELD_INFO_STC
...
/* for lion3 -- fmtFldLion2_INGRESS_STD_IPV4_ROUTED_ACL_QOS */
static CPSS_FORMAT_CONVERT_FIELD_INFO_STC
...
/* for lion3 -- fmtFldLion2_INGRESS_ULTRA_IPV6_ROUTED_ACL_QOS */
static CPSS_FORMAT_CONVERT_FIELD_INFO_STC
...
/* lion3 : ipcl : start bits for the ports bmp parts */
static GT_U32  lion3IngressPortsBmpRuleStartBitsArr[3] = {0,11,192};
/* lion3 : epcl : start bits for the ports bmp parts */
...
*
*******************************************************************************/
static GT_VOID prvCpssDxChPclActionSizeAndAllignmentGet
(
    IN  GT_U8                         devNum,
    OUT GT_U32                        *sizePtr,
...
*
* DESCRIPTION:
*       Converts TCAM Ingress Action from SW to HW format - Lion3
*
* APPLICABLE DEVICES:
*        Bobcat2; Caelum; Bobcat3.
*
...
)
{
    ...
    {
        /* in Lion3 the CPU code is also 'drop code' so relevant for drops too. */
        rc = prvCpssDxChNetIfCpuToDsaCode(
            swActionPtr->mirror.cpuCode, &dsaCpuCode);
        if (rc != GT_OK)
        {
...
*
* DESCRIPTION:
*       Converts TCAM Egress Action from SW to HW format - Lion3
*
* APPLICABLE DEVICES:
*        Bobcat2; Caelum; Bobcat3.
*
...
*
* DESCRIPTION:
*       Converts TCAM Ingress Action from HW to SW format - Lion3
*
* APPLICABLE DEVICES:
*        Bobcat2; Caelum; Bobcat3.
*
...
*
* DESCRIPTION:
*       Converts TCAM Egress Action from HW to SW format - Lion3
*
* APPLICABLE DEVICES:
*        Bobcat2; Caelum; Bobcat3.
*
        ...
        case CPSS_INTERFACE_PORT_E:
            if (nonLocalAccess == GT_FALSE)
            {
                if (PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
                {
                    ...
                    *portGroupIdPtr = CPSS_PORT_GROUP_UNAWARE_MODE_CNS;
                    localPort = (GT_U8)interfaceInfoPtr->devPort.portNum;
                }
                else
                {
            ...
            else
            {
                ...
                else
                {
                    if (PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
                    {
                        /* no conversion needed and we can write to all port groups the same value */
                        *portGroupIdPtr = CPSS_PORT_GROUP_UNAWARE_MODE_CNS;
                        localPort = (GT_U8)(interfaceInfoPtr->devPort.portNum & (portsPerDev-1));
                    }
                    else
                    {
...
)
{
    ...
    if(PRV_CPSS_DXCH_PP_MAC(devNum)->diagInfo.tcamParityCalcEnable)
    {
        ...
        /* Read Parity_Y table */
        rc = prvCpssDrvHwPpPortGroupReadRegister(devNum, portGroupId, (regAddr + (ruleIndex * 4)), &yParity);
        if (rc != GT_OK)
        {
            return rc;
        }
