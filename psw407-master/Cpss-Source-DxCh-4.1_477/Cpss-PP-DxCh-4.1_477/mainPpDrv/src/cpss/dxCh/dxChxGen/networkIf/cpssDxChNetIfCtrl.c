    ...
    switch(dsaInfoPtr->dsaType)
    {
        ...
        case CPSS_DXCH_NET_DSA_CMD_FORWARD_E:
            ...
            if(dsaInfoPtr->commonParams.dsaTagType == CPSS_DXCH_NET_DSA_4_WORD_TYPE_ENT)
            {
                ...
                if(U32_GET_FIELD_MAC(wordsPtr[1],12,1))/* use vidx */
                {
...
)
{
    ...
    if(dsaTagType != CPSS_DXCH_NET_DSA_1_WORD_TYPE_ENT)
    {
        if(toCpuPtr->srcIsTrunk == GT_FALSE)
        {
            ...
            if (((hwPort >= BIT_6) && (dsaTagType == CPSS_DXCH_NET_DSA_2_WORD_TYPE_ENT)) ||
                ((hwPort >= BIT_8) && (dsaTagType == CPSS_DXCH_NET_DSA_4_WORD_TYPE_ENT)))
            {
                return GT_BAD_PARAM;
            }
...
)
{
    ...
    if(dsaInfoPtr->commonParams.dsaTagType == CPSS_DXCH_NET_DSA_4_WORD_TYPE_ENT)
    {
        /* SrcDev is extended to 12 bits
         ...
         U32_SET_FIELD_NO_CLEAR_MASKED_MAC(wordsPtr[2],14,7,
                         (forwardPtr->srcHwDev >> 5));
    }
...
)
{
    ...
    return prvCpssDrvHwPpSetRegField(devNum,regAddr,bitIndex,1,BOOL2BIT_MAC(enable));
}
...
)
{
    ...
    if(PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_MAC(devNum) == GT_TRUE)
    {
        ...
        return prvCpssDrvHwPpSetRegField(devNum, regAddr, bitNum, 1, BOOL2BIT_MAC(enable));
    }
    else
    {
...
)
{
    ...
    if(PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_MAC(devNum) == GT_TRUE)
    {
        ...
        rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, bitNum, 1, &hwData);
...
)
{
    ...
    return prvCpssDrvHwPpSetRegField(devNum,regAddr,12,1,BOOL2BIT_MAC(enable));
}
...
)
{
    ...
    rc = prvCpssDrvHwPpGetRegField(devNum,regAddr,12,1,&data);
...
)
{
    ...
    rc = prvCpssDrvHwPpSetRegField(devNum,regAddr,startBit,numBits,hwData);
...
)
{
    ...
    rc = prvCpssDrvHwPpGetRegField(devNum,regAddr,startBit,numBits,&hwData);
...
)
{
    ...
    */
        ...
        CPSS_PP_FAMILY_DXCH_XCAT3_E == PRV_CPSS_PP_MAC(devNum)->devFamily)
    {
        ...
        return prvCpssDrvHwPpPortGroupSetRegField(devNum, portGroupId, regAddr, 8, 2, regData);
    }
    else
    {
...
)
{
    ...
    */
    ...
    else
    {
        ...
        /* relevant for bits 8:15 - so need to do shift of 8 bits */
        rc = prvCpssDrvHwPpPortGroupGetRegField(devNum, portGroupId,regAddr, queue + 8, 1, &regData);
