...
)
{
    ...
    if(0 == PRV_CPSS_DXCH_PP_HW_INFO_TXQ_REV_1_OR_ABOVE_MAC(devNum))
    {
        ...
        rc =  prvCpssDrvHwPpPortGroupGetRegField(devNum, portGroupId,regAddr, 22, 2, &hwValue);
        if(rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    status = prvCpssDrvHwPpPortGroupReadRegBitMask(devNum, portGroupId,regAddr, 0x30f0, &value);
    if (status != GT_OK)
    {
        return status;
    }
...
)
{
    ...
    /* In Lion/Lion2/Lion3 families fields shift one bit right (>>1)*/
    ...
    if((!PRV_CPSS_SIP_5_CHECK_MAC(devNum)) &&
       (CPSS_PP_FAMILY_DXCH_XCAT3_E != PRV_CPSS_PP_MAC(devNum)->devFamily))
    {
        status = prvCpssDrvHwPpPortGroupWriteRegBitMask(devNum, portGroupId,regAddr, mask, value);
        if (status != GT_OK)
        {
            return status;
        }
...
)
{
   ...
   if ((PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_LION_E)  ||
    ...
    status = prvCpssDrvHwPpPortGroupReadRegBitMask(devNum, portGroupId,regAddr, mask, &hwValue);
    if (status != GT_OK)
    {
        return status;
    }
...
)
{
    ...
    return prvCpssDrvHwPpSetRegField(devNum, regAddr, 0, 8, value);
}
...
)
{
    ...
    return prvCpssDrvHwPpSetRegField(devNum, regAddr,
        PRV_CPSS_SIP_5_20_CHECK_MAC(devNum) ? 10 : 9,
        2, value);
}
...
)
{
    ...
    return prvCpssDrvHwPpSetRegField(devNum, regAddr, 1,
...
)
{
    ...
    return prvCpssDrvHwPpGetRegField(devNum, regAddr, 1,
        PRV_CPSS_SIP_5_20_CHECK_MAC(devNum) ? 9 : 8,
        physicalPortBasePtr);
}
