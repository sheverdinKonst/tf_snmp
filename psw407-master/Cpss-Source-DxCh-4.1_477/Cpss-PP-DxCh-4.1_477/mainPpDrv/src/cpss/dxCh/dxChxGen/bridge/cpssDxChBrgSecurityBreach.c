...
)
{
    ...
    switch (cntrCfgPtr->dropCntMode)
    {
        case CPSS_BRG_SECUR_BREACH_DROP_COUNT_PORT_E:
            ...
            rc = prvCpssDrvHwPpSetRegField(dev, regAddr, fieldOffset, 1, 0);
            if (rc != GT_OK)
                return rc;
            ...
            */
...

            {
                ...
                rc = prvCpssDrvHwPpPortGroupSetRegField(dev, portGroupId,regAddr, fieldOffset, fieldLength, localPort);
                if(rc != GT_OK)
                {
                    return rc;
                }
        ...
        case CPSS_BRG_SECUR_BREACH_DROP_COUNT_VLAN_E:
            ...
            rc = prvCpssDrvHwPpSetRegField(dev, regAddr, fieldOffset, 1, 1);
            if (rc != GT_OK)
                return rc;
            ...
            rc = prvCpssDrvHwPpSetRegField(dev, regAddr, fieldOffset, fieldLength,
                                       cntrCfgPtr->vlan);
            break;
...
)
{
    ...
    rc = prvCpssDrvHwPpPortGroupGetRegField(dev, portGroupId,regAddr, 1, fieldOffset, &value);
    if (rc != GT_OK)
        return rc;
    ...
    if(PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_MAC(dev) == GT_TRUE)
    {
        ...
        rc = prvCpssDrvHwPpPortGroupGetRegField(dev, portGroupId,regAddr, 0, 13, &value);
        if (rc != GT_OK)
            return rc;
...
)
{
    ...
    return prvCpssDrvHwPpSetRegField(dev, regAddr, fieldOffset, 1, value);
}
...
)
{
    ...
    rc = prvCpssDrvHwPpGetRegField(dev, regAddr, fieldOffset, 1, &value);
...
)
{
    ...
    return prvCpssDrvHwPpSetRegField(dev, regAddr, setBit, numOfBits, value);
}
...
)
{
    ...
    rc = prvCpssDrvHwPpGetRegField(dev, regAddr, setBit, numOfBits, &value);
    if(rc != GT_OK)
    {
        return rc;
    }
