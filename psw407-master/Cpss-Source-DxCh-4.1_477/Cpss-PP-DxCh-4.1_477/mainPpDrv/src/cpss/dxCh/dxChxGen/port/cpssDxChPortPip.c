...
)
{
    ...
    CPSS_API_LOCK_MAC(0,0);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, priority, counterPtr));

    rc = internal_cpssDxChPortPipGlobalDropCounterGet(devNum, priority, counterPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, priority, counterPtr));
    CPSS_API_UNLOCK_MAC(0,0);
