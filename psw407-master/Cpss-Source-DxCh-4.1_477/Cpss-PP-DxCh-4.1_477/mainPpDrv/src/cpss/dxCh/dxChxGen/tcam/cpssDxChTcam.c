...
)
{
    ...
    GT_U32    regAddr;      /* register address  */
    GT_U32  numBitsJump;    /* number of bits for storing group ID */
    GT_U32  ii;             /* index */
    ...
    else if (numOfBlocks == 6) {
        numBitsJump = 3 * numOfBlocks;
        for(ii = 0 ; ii < numOfBlocks; ii++)
        {
            rc = prvCpssHwPpSetRegField(devNum,regAddr,ii*3,3,floorInfoArr[ii].group);
            if (rc != GT_OK)
            {
                return rc;
            }

            rc = prvCpssHwPpSetRegField(devNum,regAddr,numBitsJump + ii*2,2,floorInfoArr[ii].hitNum);
            if (rc != GT_OK)
            {
                return rc;
            }
...
)
{
    ...
    else if (PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.numBanksForHitNumGranularity == 6) {
        numBitsJump = 3 * PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.numBanksForHitNumGranularity;
        for(ii = 0 ;
            ii < PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.numBanksForHitNumGranularity;
            ii++)
        {
            rc = prvCpssHwPpGetRegField(devNum,regAddr,ii*3,3,&floorInfoArr[ii].group);
            if (rc != GT_OK)
            {
                return rc;
            }

            rc = prvCpssHwPpGetRegField(devNum,regAddr,numBitsJump + ii*2,2,&floorInfoArr[ii].hitNum);
            if (rc != GT_OK)
            {
                return rc;
            }
