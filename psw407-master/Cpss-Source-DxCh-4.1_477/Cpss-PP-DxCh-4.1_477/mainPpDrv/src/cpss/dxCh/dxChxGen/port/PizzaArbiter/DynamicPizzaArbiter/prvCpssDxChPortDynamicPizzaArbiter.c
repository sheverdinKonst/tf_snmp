...
static GT_U32                                     prv_highSpeedPortArr             [CPSS_DXCH_PA_UNIT_MAX_E][PRV_CPSS_DXCH_PORT_HIGH_SPEED_PORT_NUM_CNS];
...
)
{
    ...
    {
        GT_U32 txqPort;
        rc = bobcat2PizzaArbiterUnitDrvWorkConservingModeSet(devNum,CPSS_DXCH_PA_UNIT_TXQ_E,paWsPtr->txqWorkConservingModeOn);
        if (rc != GT_OK)
        {
            return rc;
        }
        ...
        {
            rc = prvCpssDxChPortDynamicPizzaArbiterIfTxQPortWorkConservingModeSet(devNum,txqPort,paWsPtr->txqWorkConservingModeOn);
            if (rc != GT_OK)
            {
                return rc;
            }
...
)
{
    ...
    rc = prvCpssDxChPortPhysicalPortMapShadowDBPossiblePortNumGet(devNum,/*OUT*/&maxPhysPortNum);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    for (physPortNum = 0 ; physPortNum < maxPhysPortNum; physPortNum++)
    {
        ...
        if (GT_TRUE == isPortConfigured)
        {
            ...
            {
                return GT_FAIL; /* according to Speed DB port is configured , but mapping is invalide , inconsistent data */
            }
            /* 30G WA */
...
)
{
    ...
    if(NULL != paWsPtr->prv_unit2PizzaAlgoFunArr[unitType]) /* pizza was computed ? */
    {
        ...
        {
            if (prv_highSpeedPortNumber[unitType] > PRV_CPSS_DXCH_PORT_HIGH_SPEED_PORT_NUM_CNS)
            {
