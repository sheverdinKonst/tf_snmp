...
*       DxCh1; DxCh1_Diamond; DxCh2; DxCh3; Lion; Lion2; Lion3; xCat; xCat2; Bobcat2_A0.
*
* INPUTS:
...
*
* NOT APPLICABLE DEVICES:
*       DxCh1; DxCh1_Diamond; DxCh2; DxCh3; Lion; Lion2; Lion3; xCat; xCat2.
*
* INPUTS:
*       devNum      - system device number
*       portNum     - physical port number
...
*
* NOT APPLICABLE DEVICES:
*        DxCh1; DxCh1_Diamond; DxCh2; DxCh3; xCat; xCat3; Lion; xCat2; Bobcat2.
*
*
* INPUTS:
*       devNum    - physical device number
...
)
{
    ...
    rc = prvCpssDxChHwsPortsParamsCfgGet(devNum,
...
*
*******************************************************************************/
static GT_STATUS prvCpssDxChPortBcat2PtpInterfaceWidthSelect
(
    IN  GT_U8                           devNum,
    IN  GT_U32                          portMacNum,
    IN  CPSS_PORT_SPEED_ENT             speed
...
)
{
    GT_STATUS               rc; /* return code */
    GT_U32                  i;  /* iterator */
    PRV_CPSS_DXCH_PORT_MAP_SHADOW_STC *portMapShadowPtr; /* ptr to ports entry in
                                                            mapping shadow DB */
...
*
*******************************************************************************/
static GT_STATUS prvCpssDxChPortBcat2InterlakenTxDmaEnableSet
(
    IN  GT_U8   devNum,
    IN  GT_BOOL enable
)
...
*
*******************************************************************************/
static GT_STATUS prvCpssDxChPortBcat2FcaBusWidthSet
(
    IN  GT_U8                           devNum,
    IN  GT_PHYSICAL_PORT_NUM            portNum,
    IN  CPSS_PORT_INTERFACE_MODE_ENT    ifMode,
...
)
{
    ...
    else
    {
        ...
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EGF_eft.egrFilterConfigs.
                                            physicalPortLinkStatusMask[portMacNum/16];
        if (prvCpssDrvHwPpPortGroupSetRegField(devNum,
                                               CPSS_PORT_GROUP_UNAWARE_MODE_CNS,
                                               regAddr, (portMacNum%16)*2, 2,
                                               ((powerUp) ? 0 : 2)) != GT_OK)
        {
            return GT_HW_ERROR;
        }
...
{
    return cpssDxChPtpPortUnitResetSet(
        devNum, portNum, reset /*resetTxUnit*/, reset /*resetRxUnit*/);
}
...
*
*******************************************************************************/
static GT_STATUS prvCpssDxChLedPortTypeConfig
(
    IN  GT_U8                           devNum,
    IN  GT_PHYSICAL_PORT_NUM            portNum
)
...
)
{
    ...
    for(portNum = 0; portNum < CPSS_MAX_PORTS_NUM_CNS; portNum++)
    {
        ...
        rc = prvCpssDxChPortPtpReset(devNum, portNum, GT_TRUE);
        if (rc != GT_OK)
        {
            return rc;
        }
        ...
        *portSpeedPtr = CPSS_PORT_SPEED_NA_E;
        ...
        if(!powerUp)
        {
            ...
            rc = prvCpssDxChPortPtpReset(devNum, portNum, GT_FALSE);
            if (rc != GT_OK)
            {
                return rc;
            }
    ...
    for(i = 0; i < PRV_CPSS_PP_MAC(devNum)->numOfPorts; i++)
    {
        ...
        else
        {
            if(PRV_CPSS_PORT_FE_E == PRV_CPSS_DXCH_PORT_TYPE_MAC(devNum,i))
                return GT_NOT_SUPPORTED; /* Fast Ethernet ports have no serdes,
...
*
* NOT APPLICABLE DEVICES:
*       DxCh1; DxCh1_Diamond; DxCh2; DxCh3; xCat; Lion; xCat2; Bobcat2.
*
* INPUTS:
*       devNum   - physical device number
...
)
{
    ...
    MV_HWS_PORT_STANDARD portMode; /* interface modes of port which use this
                                        configuration in Black Box format */
    ...
    else
    {
        ...
        for(portMode = SGMII; portMode <= SGMII2_5; portMode++)
        {
            CPSS_LOG_INFORMATION_MAC("Calling: hwsPortExtendedModeCfg(devNum[%d], portGroup[%d], phyPortNum[%d], portMode[%d], extendedMode[%d])", devNum, portGroupId, localPort, portMode, enable);
            rc = hwsPortExtendedModeCfg(devNum,portGroupId,localPort,portMode,enable);
            if(rc != GT_OK)
            {
                CPSS_LOG_INFORMATION_MAC("Hws return code is %d", rc);
                return rc;
        ...
        CPSS_LOG_INFORMATION_MAC("Calling: hwsPortExtendedModeCfg(devNum[%d], portGroup[%d], phyPortNum[%d], portMode[%d], extendedMode[%d])", devNum,portGroupId,localPort,_10GBase_KR,enable);
        rc = hwsPortExtendedModeCfg(devNum,portGroupId,localPort,_10GBase_KR,enable);
        if(rc != GT_OK)
        {
            CPSS_LOG_INFORMATION_MAC("Hws return code is %d", rc);
            return rc;
        ...
        CPSS_LOG_INFORMATION_MAC("Calling: hwsPortExtendedModeCfg(devNum[%d], portGroup[%d], phyPortNum[%d], portMode[%d], extendedMode[%d])", devNum,portGroupId,localPort,_10GBase_SR_LR,enable);
        rc = hwsPortExtendedModeCfg(devNum,portGroupId,localPort,_10GBase_SR_LR,enable);
        if(rc != GT_OK)
        {
            CPSS_LOG_INFORMATION_MAC("Hws return code is %d", rc);
            return rc;
        ...
        /* because HWS package for now supports just global configuration of extended
            mode for local ports 9 and 11, run over all GOP's */
...
#define PRV_CPSS_DXCH_PORT_MAC_MODE_SELECT_HGL_CNS 7
    ...
    {
        CPSS_PORT_INTERFACE_MODE_ENT   evenNeighborIfMode;
        GT_U32      serdes; /* serdes number */
        GT_U32      serdesNum;  /* serdes quantity */

        if((rc = lionIfModeGet(devNum,portNum-1,&evenNeighborIfMode)) != GT_OK)
            return rc;

        if((rc = prvCpssDxChPortIfModeSerdesNumGet(devNum, portNum-1,
                                                   evenNeighborIfMode,
                                                   &serdes, &serdesNum)) != GT_OK)
        {
            return rc;
        }
        if(serdesNum > 2)
...
*
* DESCRIPTION:
*       Function to wrap hwsPortsParamsCfgGet.
*
* APPLICABLE DEVICES:
*        Lion2; Bobcat2; Caelum; Bobcat3.
*
...
*
* INPUTS:
*       devNum   - physical device number, the function will convert it to
*                       MV_HWS_DEV_TYPE devType
*       macNum      - port MAC number
*       portMode - port interface in HWS format (MV_HWS_PORT_STANDARD)
*
* OUTPUTS:
*       macNumPtr       - number of MAC used by port for this interface
*       pcsNumPtr       - number of PCS used by port for this interface
*       sdVecSizePtr    - number of serdeses used by port for this interface
*       sdVectorPtrPtr  - list of serdeses used by port for this interface
*
* RETURNS:
*       GT_OK                    - on success
*       GT_BAD_PARAM             - on wrong port number or device
*       GT_BAD_PTR               - one of the parameters is NULL pointer
*       GT_NOT_INITIALIZED       - HWS port feature not intialized yet.
*
* COMMENTS:
*
*******************************************************************************/
GT_STATUS prvCpssDxChHwsPortsParamsCfgGet
(
   GT_U8                devNum,
   GT_U32   macNum,
   GT_U32               portMode,
   ...
   GT_U32               *pcsNumPtr,
   GT_U32               *sdVecSizePtr,
   GT_U32               **sdVectorPtrPtr
)
{
    GT_STATUS rc;
    CPSS_NULL_PTR_CHECK_MAC(macNumPtr);
    CPSS_NULL_PTR_CHECK_MAC(pcsNumPtr);
    CPSS_NULL_PTR_CHECK_MAC(sdVecSizePtr);

    CPSS_LOG_INFORMATION_MAC("Calling: hwsPortsParamsCfgGet(devNum[%d], portGroup[%d], portNum[%d], portMode[%d], *macNum, *pcsNum, *sdVecSize, **sdVector)", devNum, 0, macNum, portMode);
    rc = hwsPortsParamsCfgGet(devNum, 0, macNum, portMode, macNumPtr, pcsNumPtr,
                                sdVecSizePtr, sdVectorPtrPtr);
    if(rc != GT_OK)
    {
        CPSS_LOG_INFORMATION_MAC("Hws return code is %d", rc);
    }
...
*
* NOT APPLICABLE DEVICES:
*       DxCh1; DxCh1_Diamond; DxCh2; DxCh3; xCat; Lion; xCat2; Lion2; Bobcat2; Lion3.
*
* INPUTS:
*       devNum    - physical device number
*       portNum   - physical port number (or CPU port)
...
)
{
    ...
    {
        PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[portNum].portRefClock.enableOverride = overrideEnable;
        /* invalidate entry */
        PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[portNum].portRefClock.portRefClockSource = CPSS_PORT_REF_CLOCK_SOURCE_LAST_E;
    }
    else
    {
        ...
        {
            PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[portNum].portRefClock.enableOverride = overrideEnable;
            PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[portNum].portRefClock.portRefClockSource = refClockSource;
        }
        else
        {
            return GT_BAD_PARAM;
...
)
{
    ...
    *overrideEnablePtr = PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[portNum].portRefClock.enableOverride;
    *refClockSourcePtr = PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[portNum].portRefClock.portRefClockSource;
