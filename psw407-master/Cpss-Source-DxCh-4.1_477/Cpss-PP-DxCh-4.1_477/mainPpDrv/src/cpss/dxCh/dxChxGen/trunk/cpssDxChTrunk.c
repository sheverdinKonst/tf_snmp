...
/* Lion3,Bc2 :  the DB about the fields in the CRC hash mask entry.
    index to this table is according to fields in
    CPSS_DXCH_TRUNK_LBH_CRC_MASK_ENTRY_STC */
...
)
{
    ...
    */
    if (PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_MAC(devNum))
    {
        ...
        devTrunkInfoPtr->validityMask.port   = 0xFF;/* 8 bits (also for bobk) */
    }
    else
    {
...
{
    GT_U8  *hashDataPtr = NULL;
    GT_U8  *saltDataPtr = NULL;
    GT_U8  *crc32DataPtr = NULL;
    GT_U32 hwEntry[HASH_MASK_CRC_ENTRY_NUM_WORDS_CNS] = {0};
    GT_U32 maskIndex = 0;
    GT_U8 mask = 0;
    GT_U8 pattern = 0;
    GT_U32 offset = 0;
    GT_U32 startBit = 0;
    GT_U32 numOfBitsInHash = 0;
    GT_U8  saltValue = 0;
    GT_U32 i,j;
    GT_BOOL carry = 0;
    GT_U32 finalInputHash = 0;
    GT_U32 crcSeed;
    ...
    hashDataPtr = cpssOsMalloc(sizeof(GT_U8)*560);
    if(hashDataPtr == NULL)
    {
        return GT_OUT_OF_CPU_MEM;
    }
    cpssOsMemSet(hashDataPtr,0,sizeof(GT_U8)*560);
    saltDataPtr = cpssOsMalloc(sizeof(GT_U8)*560);
    if(saltDataPtr == NULL)
    {
        cpssOsFree(hashDataPtr);
        return GT_OUT_OF_CPU_MEM;
    }
    cpssOsMemSet(saltDataPtr,0,sizeof(GT_U8)*560);

    crc32DataPtr = cpssOsMalloc(sizeof(GT_U8)*560);
    if(crc32DataPtr == NULL)
    {
        cpssOsFree(hashDataPtr);
        cpssOsFree(saltDataPtr);
        return GT_OUT_OF_CPU_MEM;
    }
    cpssOsMemSet(crc32DataPtr,0,sizeof(GT_U8)*560);
    ...
    case CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_L3_ECMP_E:
        ...
        rc  = cpssDxChIpEcmpHashNumBitsGet(devNum, &startBit, &numOfBitsInHash);
        if (rc != GT_OK)
        {
            cpssOsFree(hashDataPtr);
            cpssOsFree(saltDataPtr);
    ...
    for ( i = 0; i < 16; i++ )
    {
        mask = U32_GET_FIELD_MAC(hwEntry[0],offset/8 + i/8,1);
        pattern = U32_GET_FIELD_MAC(hashInputDataPtr->l4DstPortArray[i/8], i%8 ,1);
        hashDataPtr[i] = pattern & mask;
    }
    offset = 16;
    ...
    {
        mask = U32_GET_FIELD_MAC(hwEntry[0],offset/8 + i/8,1);
        pattern = U32_GET_FIELD_MAC(hashInputDataPtr->l4SrcPortArray[i/8], i%8 ,1);
    ...
    for ( i = 0; i < 20; i++ )
    {
        ...
        pattern = U32_GET_FIELD_MAC(hashInputDataPtr->ipv6FlowArray[i/8], i%8 ,1);
        hashDataPtr[offset+i] = pattern & mask;
    }
    offset = 52;
    ...
    {
        mask = U32_GET_FIELD_MAC(hwEntry[0],offset/8 + i/8,1);
        pattern = U32_GET_FIELD_MAC(hashInputDataPtr->ipDipArray[i/8], i%8 ,1);
        hashDataPtr[offset+i] = pattern & mask;
    }
    offset = 184;
    /* IP SIP - 311:184 */
    ...
    for ( i = 0; i < 128; i++ )
    {
        ...
        mask = U32_GET_FIELD_MAC(hwEntry[j],(offset/8 + i/8)%32,1);
        pattern = U32_GET_FIELD_MAC(hashInputDataPtr->ipSipArray[i/8], i%8 ,1);
        hashDataPtr[offset+i] = pattern & mask;
    }
    /* MAC DA  - 359:312 */
    offset = 312;
    for ( i = 0; i < 48; i++ )
    {
        mask = U32_GET_FIELD_MAC(hwEntry[1],(offset/8 + i/8)%32,1);
        pattern = U32_GET_FIELD_MAC(hashInputDataPtr->macDaArray[i/8], i%8 ,1);
        hashDataPtr[offset+i] = pattern & mask;
    }
    /* MAC SA - 407:360 */
    offset = 360;
    ...
    {
        mask = U32_GET_FIELD_MAC(hwEntry[1],(offset/8 + i/8)%32,1);
        pattern = U32_GET_FIELD_MAC(hashInputDataPtr->macSaArray[i/8], i%8 ,1);
        hashDataPtr[offset+i] = pattern & mask;
    }
    ...
    {
        mask = U32_GET_FIELD_MAC(hwEntry[1],(offset/8 + i/8)%32,1);
        pattern = U32_GET_FIELD_MAC(hashInputDataPtr->mplsLabel0Array[i/8], i%8 ,1);
        hashDataPtr[offset+i] = pattern & mask;
    }
    offset = 428;
    ...
    {
        mask = U32_GET_FIELD_MAC(hwEntry[1],(offset/8 + i/8)%32,1);
        pattern = U32_GET_FIELD_MAC(hashInputDataPtr->mplsLabel1Array[i/8], i%8 ,1);
        hashDataPtr[offset+i] = pattern & mask;
    }
    offset = 452;
    ...
    {
        mask = U32_GET_FIELD_MAC(hwEntry[1],(offset/8 + i/8)%32,1);
        pattern = U32_GET_FIELD_MAC(hashInputDataPtr->mplsLabel2Array[i/8], i%8 ,1);
        hashDataPtr[offset+i] = pattern & mask;
    }
    ...
    {
        mask = U32_GET_FIELD_MAC(hwEntry[1],(offset/8 + i/8)%32,1);
        pattern = U32_GET_FIELD_MAC(hashInputDataPtr->localSrcPortArray[i/8], i%8 ,1);
        hashDataPtr[offset+i] = pattern & mask;
    }
    ...
    for ( i = 0; i < 72; i++ )
    {
        ...
        mask = U32_GET_FIELD_MAC(hwEntry[j],(offset/8 + i/8)%32,1);
        pattern = U32_GET_FIELD_MAC(hashInputDataPtr->udbsArray[i/8 + 14], i%8 ,1);
        hashDataPtr[offset+i] = pattern & mask;
    }
