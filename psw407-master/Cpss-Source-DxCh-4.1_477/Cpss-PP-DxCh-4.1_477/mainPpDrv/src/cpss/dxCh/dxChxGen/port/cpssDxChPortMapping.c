...
#define PRV_CPSS_MAPPING_DEBUG_CNS           0
    ...
    #define PRV_CPSS_MAPPING_LION3_SUPPORT_CNS   0
...
| 2   | Cut Through TX DMA map of Local Device Physical Port to RX DMA number | Phys --> TxDMA  | Lion3
...
| 8   | Local Device Target Physical to hemisphere map                        | Phys -> TxQ (?) | Lion3
...
+------+----------------------------------------------------------------------+-----------+-----------+-----------+-----------+-----------+
| 2    | Cut Through TX DMA map of Local Device Physical Port to RX DMA number|    x      |    x      |    x      |    x      |    x      | Lion3  only
+------+----------------------------------------------------------------------+-----------+-----------+-----------+-----------+-----------+
...
#define PRV_CPSS_DXCH_RXDMA_2_PHYS_MAX_PHYS_PORT_CNS   255

#define PRV_CPSS_DXCH_RXDMA_2_PHYS_OFFS_CNS 0
#define PRV_CPSS_DXCH_RXDMA_2_PHYS_LEN_CNS  8
...
)
{
    ...
    if (physPort > PRV_CPSS_DXCH_RXDMA_2_PHYS_MAX_PHYS_PORT_CNS)
    {
        return GT_BAD_PARAM;
    }
...
#define PRV_CPSS_DXCH_CutTrough_TXDMA_2_PHYS_MAX_TXDMA_PORT_CNS 255
...
#if (PRV_CPSS_MAPPING_LION3_SUPPORT_CNS == 1)
    /*******************************************************************************
    * prvCpssDxChPortMappingCutThroughTxDMAOfLocalPhys2TxDmaSet
    *
    * DESCRIPTION:
    *       configure phys 2 cut through TxDMA port mapping
    *
    * APPLICABLE DEVICES:
    *       none.
    *
    ...
    *
    *******************************************************************************/
    static GT_STATUS prvCpssDxChPortMappingCutThroughTxDMAOfLocalPhys2TxDmaSet
    (
        IN GT_U8  devNum,
        IN GT_U32 portGroupId,
        IN GT_U32 physPort,
    ...
    )
    {
        ...
        PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_CH1_E | CPSS_CH1_DIAMOND_E
                                                | CPSS_CH2_E | CPSS_CH3_E | CPSS_XCAT_E | CPSS_XCAT3_E
                                                | CPSS_LION_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_BOBCAT3_E | CPSS_XCAT2_E);


        devFamily = PRV_CPSS_PP_MAC(devNum)->devFamily;

        if (devFamily != CPSS_PP_FAMILY_DXCH_LION3_E)
        {
            return GT_NOT_APPLICABLE_DEVICE;
        }

        if (physPort > PRV_CPSS_DXCH_CutTrough_TXDMA_2_PHYS_MAX_PHYS_PORT_CNS)
        {
            return GT_BAD_PARAM;
        }

        if (txDmaNum > PRV_CPSS_DXCH_CutTrough_TXDMA_2_PHYS_MAX_TXDMA_PORT_CNS)
        {
            return GT_BAD_PARAM;
        }

        regAddr = 0;
        regOffset = (physPort%4);

        rc = prvCpssHwPpPortGroupSetRegField(devNum, portGroupId, regAddr,
                                                regOffset, 8, txDmaNum);
        if (GT_OK != rc )
        {
            return rc;
        }
    ...
    /*******************************************************************************
    * prvCpssDxChPortMappingCutThroughTxDMAOfLocalPhys2TxDmaGet
    *
    * DESCRIPTION:
    *       configure phys 2 cut through RxDMA port mapping
    *
    * APPLICABLE DEVICES:
    *       none.
    *
    ...
    *
    * OUTPUTS:
    *       rxDmaPortPtr - pointer to txDMA port
    *
    * RETURNS:
    *       GT_OK           - on success
    *       GT_BAD_PARAM    - wrong devNum
    ...
    *
    *******************************************************************************/
    static GT_STATUS prvCpssDxChPortMappingCutThroughTxDMAOfLocalPhys2TxDmaGet
    (
        IN GT_U8  devNum,
        IN GT_U32 portGroupId,
        IN GT_U32 physPort,
        IN GT_U32 *txDmaNumPtr
    )
    {
        GT_STATUS rc;
        GT_U32    regAddr;            /* register's address */
        ...
        CPSS_NULL_PTR_CHECK_MAC(txDmaNumPtr);

        regAddr = 0;
        regOffset = physPort%4;
        rc = prvCpssHwPpPortGroupGetRegField(devNum, portGroupId, regAddr,
                                                regOffset, 8, txDmaNumPtr);
...
/* 3. BMA map of Local Device Physical Port to RX DMA number                   */
/*-----------------------------------------------------------------------------*/
#define PRV_CPSS_DXCH_BMA_PHYS_2_RXDMA_MAX_PHYS_PORT_CNS 255
...
*
*******************************************************************************/
static GT_STATUS prvCpssDxChPortMappingBMAMapOfLocalPhys2RxDMASet
(
    IN GT_U8  devNum,
    IN GT_U32 portGroupId,
    IN GT_U32 physPort,
...
)
{
    ...
    if (physPort > PRV_CPSS_DXCH_BMA_PHYS_2_RXDMA_MAX_PHYS_PORT_CNS)
    {
        return GT_BAD_PARAM;
    }
...
*
*******************************************************************************/
static GT_STATUS prvCpssDxChPortMappingBMAMapOfLocalPhys2RxDMAGet
(
    IN GT_U8  devNum,
    IN GT_U32 portGroupId,
    IN GT_U32 physPort,
...
)
{
...
#if (PRV_CPSS_MAPPING_LION3_SUPPORT_CNS == 1)
    /*-----------------------------------------------------------------------------*/
    /* 8 Local Device Target Physical to hemisphere map                            */
    /*-----------------------------------------------------------------------------*/
    /*******************************************************************************
    * prvCpssDxChPortMappingLocalDeviceTargetPhysPort2HemisphereMapSet
    *
    * DESCRIPTION:
    *       configure Phys port mapping to hemisphere
    *
    * APPLICABLE DEVICES:
    *       none.
    *
    ...
    *
    *******************************************************************************/
    static GT_STATUS prvCpssDxChPortMappingLocalDeviceTargetPhysPort2HemisphereMapSet
    (
        IN GT_U8  devNum,
        IN GT_U32 portGroupId,
        IN GT_U32 physPort,
    ...
    /*******************************************************************************
    * prvCpssDxChPortMappingLocalDeviceTargetPhysPort2HemisphereMapGet
    *
    * DESCRIPTION:
    *       configure Phys port mapping to hemisphere
    *
    ...
    *
    * OUTPUTS:
    *       hemispherePtr  - hemisphere number
    *
    * RETURNS:
    *       GT_OK           - on success
    *       GT_BAD_PARAM    - wrong devNum
    ...
    *
    *******************************************************************************/
    static GT_STATUS prvCpssDxChPortMappingLocalDeviceTargetPhysPort2HemisphereMapGet
    (
        IN GT_U8  devNum,
        IN GT_U32 portGroupId,
        IN  GT_U32 resId,
        OUT GT_U32 *hemispherePtr
    )
    {
        PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
        PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_CH1_E | CPSS_CH1_DIAMOND_E
        ...
        if (PRV_CPSS_PP_MAC(devNum)->devFamily != CPSS_PP_FAMILY_DXCH_LION3_E) /* currently only Bobcat2 is supported */
        {
            return GT_NOT_APPLICABLE_DEVICE;
        }

        CPSS_NULL_PTR_CHECK_MAC(hemispherePtr);
...
static PRV_DXCH_CPSS_MAPPING_SET_GET_FUN_STC prv_MappingFunList[] =
{
    /* 1  RX DMA to Local Device Physical Source Port */
    {
        ...
        /*  GT_U32                             applicableDevice; */ CPSS_LION3_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_BOBCAT3_E,
    ...
    {
        #if (PRV_CPSS_MAPPING_DEBUG_CNS == 1)
        /* GT_CHAR                            stringMapType[20];*/  "2 CutTroughTxDMA-phys2txDMA",
        #endif
        ...
        /*  GT_U32                             applicableDevice; */ CPSS_LION3_E,
        ...
        /*  PRV_DXCH_CPSS_MAPPING_SET_FUN_PTR  setFun;           */ prvCpssDxChPortMappingCutThroughTxDMAOfLocalPhys2TxDmaSet,
        /*  PRV_DXCH_CPSS_MAPPING_GET_FUN_PTR  getFun;           */ prvCpssDxChPortMappingCutThroughTxDMAOfLocalPhys2TxDmaGet
    },
#endif
    /* 3 BMA map of Local Device Physical Port to RX DMA number */
    {
    ...
    {
        #if (PRV_CPSS_MAPPING_DEBUG_CNS == 1)
        /* GT_CHAR                            stringMapType[20];*/  "8 phys2hemisphere",
        #endif
        /*  GT_U32                             mapingTypeBmp;    */ PRV_MAPTYPE_ETHERNET_MAC_E,
        /*  GT_U32                             applicableDevice; */ CPSS_LION3_E,
        /*  PRV_CPSS_DXCH_PORT_TYPE_ENT        srcPortType;      */ PRV_CPSS_DXCH_PORT_TYPE_PHYS_E,
        /*  PRV_CPSS_DXCH_PORT_TYPE_ENT        dstPortType;      */ PRV_CPSS_DXCH_PORT_TYPE_TXQ_E,
        /*  PRV_DXCH_CPSS_MAPPING_SET_FUN_PTR  setFun;           */ prvCpssDxChPortMappingLocalDeviceTargetPhysPort2HemisphereMapSet,
        /*  PRV_DXCH_CPSS_MAPPING_GET_FUN_PTR  getFun;           */ prvCpssDxChPortMappingLocalDeviceTargetPhysPort2HemisphereMapGet
    },
#endif
    /* 9 CPU TxQ port number  */
    {
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_CH1_E | CPSS_CH1_DIAMOND_E
                                            ...
                                            | CPSS_LION_E | CPSS_LION2_E | CPSS_XCAT2_E);
    devFamily = PRV_CPSS_PP_MAC(devNum)->devFamily;
    if (devFamily != CPSS_PP_FAMILY_DXCH_LION3_E && !PRV_CPSS_SIP_5_CHECK_MAC(devNum))
    {
        return GT_NOT_APPLICABLE_DEVICE;
    }
...
/*-----------------------------------------------------*/
#define PRV_CPSS_DXCH_PORT_NUM_MAPPING_MAX_PORT_CNS 256
           ...
           if(CPSS_DXCH_PORT_MAPPING_TYPE_ETHERNET_MAC_E == portMapPtr->mappingType)
           {
               switch((portMapPtr->interfaceNum)/32)
               {
                    ...
                    case 2: regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->
                            ...
                            break;
                    default: return GT_OUT_OF_RANGE;
               }
       ...
       if (GT_TRUE == portMapPtr->tmEnable)
       {
           ...
           switch(portMapPtr->mappingType)
           {
              case CPSS_DXCH_PORT_MAPPING_TYPE_ETHERNET_MAC_E:
                  ...
                  tableType = PRV_CPSS_DXCH_LION3_TABLE_TM_FCU_ETH_DMA_TO_TM_PORT_MAPPING_E;
                  break;

              case CPSS_DXCH_PORT_MAPPING_TYPE_ILKN_CHANNEL_E:
                  U32_SET_FIELD_MAC(regData, 7, 1, 1);
                  tableType = PRV_CPSS_DXCH_LION3_TABLE_TM_FCU_ILK_DMA_TO_TM_PORT_MAPPING_E;
                  break;
...
(
    ...
    OUT PRV_CPSS_DXCH_DETAILED_PORT_MAP_STC  *portMapPtr
)
{
    GT_STATUS rc;
...
)
{
    ...
    PRV_CPSS_DXCH_DETAILED_PORT_MAP_STC detailedPortMap;
    CPSS_DXCH_PORT_MAP_STC * portMapPtr;
