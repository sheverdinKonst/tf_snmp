...
#define LION3_MAC_ENTRY_SIZE_CNS  32/*alignment*/ /* Lion3: MAC table entry size in bytes  */
    ...
    if (bank0Size > 0)
    {
        ...
        if(GT_FALSE ==
            prvCpssDuplicatedMultiPortGroupsGet(devNum,CPSS_PORT_GROUP_UNAWARE_MODE_CNS,
             bank0Start,&portGroupsBmp,&isAdditionalRegDupNeeded,&additionalRegDup))
        {
...
)
{
    ...
    if(GT_FALSE ==
        prvCpssDuplicatedMultiPortGroupsGet(devNum,portGroupId,
             regAddr,&portGroupsBmp,&isAdditionalRegDupNeeded,&additionalRegDup))
    {
        portGroupsBmp = CPSS_PORT_GROUP_UNAWARE_MODE_CNS;
    }
...
)
{
    ...
    if(GT_FALSE ==
        prvCpssDuplicatedMultiPortGroupsGet(
            ...
            testBase,&portGroupsBmp,&isAdditionalRegDupNeeded,&additionalRegDup))
...
)
{
    ...
    if (PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
    {
        ...
        regsArray[1] = PRV_DXCH_REG1_UNIT_TXQ_DQ_MAC(devNum).shaper.tokenBucketUpdateRate;
        ...
        regsArray[7] = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->IPvX.routerGlobalCtrl.routerGlobalCtrl0;
    }
    else
    {
        if ((PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_LION_E)||
...
)
{
        ...
        || (PRV_CPSS_SIP_5_CHECK_MAC(devNum)))
    {
        ...
        rc = mvHwsSerdesTestGenStatus(devNum, portGroupId, serdesIndex,
                                      ...
                                      0 /* txPattern not used */, GT_FALSE /* clear on read mode */, &status);
        if (rc != GT_OK)
        {
            return rc;
        }
...
*                        Lion2 - (APPLICABLE RANGES: -142..228)
*                        Lion3 and Bobcat2 - (APPLICABLE RANGES: -277..198)
*
* OUTPUTS:
*       None.
*
...
)
{
    ...
    {    /* PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_XCAT3_E */
        ...
        *temperaturePtr = (GT_32)(hwValue * 10000 - 5847500) / 21445;
    }
...
)
{
    ...
    if(GT_FALSE ==
        prvCpssDuplicatedMultiPortGroupsGet(devNum,CPSS_PORT_GROUP_UNAWARE_MODE_CNS,
            memBase,&validPortGroupsBmp,&isAdditionalRegDupNeeded,&additionalRegDup))
    {
    ...
    else if (memType == CPSS_DIAG_PP_MEM_BUFFER_DRAM_E)
    {
        /* NOTE:
           getMemBaseFromType(...) function already knows that :
            Lion2,Lion3,bobcat2 are not supported
        */
