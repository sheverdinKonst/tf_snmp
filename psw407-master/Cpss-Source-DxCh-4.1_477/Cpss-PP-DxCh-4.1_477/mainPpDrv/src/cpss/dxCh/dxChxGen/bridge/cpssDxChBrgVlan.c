...
#define LION3_L2I_INGRESS_VLAN_PORT_MEMBERS_SIZE_CNS  256
/* number of words in the Bobcat2; Caelum; Bobcat3 and above ingress vlan entry */
#define VLAN_INGRESS_ENTRY_WORDS_NUM_CNS     BITS_TO_WORDS_MAC(LION3_L2I_INGRESS_VLAN_PORT_MEMBERS_SIZE_CNS)
/* number of words in the Lion and above Egress vlan entry */
#define VLAN_EGRESS_ENTRY_WORDS_NUM_CNS      (9)

/* number of words in the Bobcat2; Caelum; Bobcat3 and above EGF_QAG Egress vlan entry */
#define EGF_QAG_EGRESS_VLAN_ENTRY_WORDS_NUM_CNS      (24) /* this support 768 bits */
...
prvCpssDxChLion3L2iIngressVlanTableFieldsFormat - the lion3; Bobcat2 vlan entry format
...
/* NOTE: the macro uses next 'implicit' fields:
hwDataArr - the buffer
prvCpssDxChLion3EgfShtEgressEVlanTableFieldsFormat - the lion3; Bobcat2 EGF_SHT egress vlan entry format
...
)
{
    ...
    for (ii = 0; (ii < PRV_CPSS_PP_MAC(devNum)->numOfPorts); ii++)
    {
        ...
        else
        {
            ...
            {
                /* supported by Lion3 and above only */
                return GT_BAD_PARAM;
            }
...
)
{
    ...
    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr ,offset, 16, &value);
    if(rc != GT_OK)
    {
        return rc;
    }
    ...
    *etherTypePtr = (GT_U16)value;
    ...
    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, offset, 12, &value);
    if(rc != GT_OK)
    {
        return rc;
    }
...
*       devNum   - device number
*       mruIndex - MRU profile index (APPLICABLE RANGES: 0..7)
*       mruValue - MRU value in bytes (APPLICABLE RANGES: 0..0xFFFF)
* OUTPUTS:
*       None
*
* RETURNS:
...
)
{
    ...
    /* Set Maximum Receive Unit MRU profile value for the index */
    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, fieldOffset,
                                     16, &value);
    if(GT_OK != rc)
    {
        return rc;
...
)
{
    ...
    GT_U32      efgQag_egressHwData[EGF_QAG_EGRESS_VLAN_ENTRY_WORDS_NUM_CNS];/*Lion3: EGF_QAG info*/
...
)
{
    ...
    GT_U32  regValEArch;/* register Lion3 and Bobcat2 */
...
)
{
    ...
    /*Note:in Lion3 the configuration is 'per physical port'*/
...
/*******************************************************************************
* internal_cpssDxChBrgVlanPortEgressVlanFilteringEnableSet
*
* DESCRIPTION:
...
*
*******************************************************************************/
static GT_STATUS internal_cpssDxChBrgVlanPortEgressVlanFilteringEnableSet
(
    IN GT_U8            devNum,
    IN GT_PORT_NUM      portNum,
    IN GT_BOOL          enable
...
*
*******************************************************************************/
GT_STATUS cpssDxChBrgVlanPortEgressVlanFilteringEnableSet
(
    IN GT_U8            devNum,
    IN GT_PORT_NUM      portNum,
    IN GT_BOOL          enable
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChBrgVlanPortEgressVlanFilteringEnableSet);

    CPSS_API_LOCK_MAC(0,0);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, enable));

    rc = internal_cpssDxChBrgVlanPortEgressVlanFilteringEnableSet(devNum, portNum, enable);
...
/*******************************************************************************
* internal_cpssDxChBrgVlanPortEgressVlanFilteringEnableGet
*
* DESCRIPTION:
...
*
*******************************************************************************/
static GT_STATUS internal_cpssDxChBrgVlanPortEgressVlanFilteringEnableGet
(
    IN  GT_U8            devNum,
    IN  GT_PORT_NUM      portNum,
    OUT GT_BOOL          *enablePtr
...
*
*******************************************************************************/
GT_STATUS cpssDxChBrgVlanPortEgressVlanFilteringEnableGet
(
    IN  GT_U8            devNum,
    IN  GT_PORT_NUM      portNum,
    OUT GT_BOOL          *enablePtr
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChBrgVlanPortEgressVlanFilteringEnableGet);

    CPSS_API_LOCK_MAC(0,0);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, enablePtr));

    rc = internal_cpssDxChBrgVlanPortEgressVlanFilteringEnableGet(devNum, portNum, enablePtr);
