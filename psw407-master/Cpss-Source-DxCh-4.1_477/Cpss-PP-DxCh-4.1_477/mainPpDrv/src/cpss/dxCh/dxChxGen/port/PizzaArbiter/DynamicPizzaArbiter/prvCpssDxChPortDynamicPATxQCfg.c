...
#define MIN_INTERSLICE_GAP_APPR_ALGO_CNS        4
...
typedef struct
{
    ...
    GT_U32               highSpeedPortArr[PRV_CPSS_DXCH_PORT_HIGH_SPEED_PORT_NUM_CNS];
...
)
{
    ...
    *numberOfSlicesPtr = numberOfSlices;
    ...
    /*--------------------------------------*/
    portList->highSpeedPortNum = 0;
    if (portList->portNum <= PRV_CPSS_DXCH_PORT_HIGH_SPEED_PORT_NUM_CNS)  /* theretically only high speed ports */
    {
        if (portList->arr[0].numberOfSlicesToAllocate > 1)
        {
            for (portList->highSpeedPortNum = 0 ; portList->highSpeedPortNum < portList->portNum; portList->highSpeedPortNum++)
            {
                ...
                if (interSliceGap >= MIN_INTERSLICE_GAP_OPTIMAL_DISTR_CNS ) /* gap 3 is accceptable : slice : 0 1 2 3  */
                {                                             /*                         port : 0 x x 0  */
                    break;
                }
            }
    ...
    else
    {
        ...
        {
            for (portList->highSpeedPortNum = 0 ; portList->highSpeedPortNum < PRV_CPSS_DXCH_PORT_HIGH_SPEED_PORT_NUM_CNS; portList->highSpeedPortNum++)
            {
                interSliceGap = numberOfSlices / portList->arr[portList->highSpeedPortNum].numberOfSlicesToAllocate;
                if (interSliceGap >= MIN_INTERSLICE_GAP_OPTIMAL_DISTR_CNS )
                {
                    break;
                }
            }
    ...
    if (portList->portNum <= PRV_CPSS_DXCH_PORT_HIGH_SPEED_PORT_NUM_CNS)  /* theretically only high speed ports */
    {
        if (portList->arr[0].numberOfSlicesToAllocate > 1)
        {
            for (portList->highSpeedPortNum = 0 ; portList->highSpeedPortNum < portList->portNum; portList->highSpeedPortNum++)
            {
                ...
                if (interSliceGap_Q20 >= MIN_INTERSLICE_GAP_APPR_ALGO_CNS*FIXP_MUL) /* gap 4 is accceptable : slice : 0 1 2 3 4 */
                {                                                               /*                         port : 0 x x x 0 */
                    break;
                }
            }
...
(
    ...
    OUT  GT_U32       highSpeedPortsArr[PRV_CPSS_DXCH_PORT_HIGH_SPEED_PORT_NUM_CNS]
)
{
    GT_U32  i;
    GT_STATUS rc;
...
*/
    ...
    {
        rc = configPizza_OptimalDistribution(&portList,maxPipeSizeMbps,minSliceResolutionMpbs, pizzaArraySize, /*OUT*/pizzaArray, /*OUT*/&numberOfSlices);
        if (rc != GT_OK)
        {
            return rc;
        }
    ...
    {
        rc = configPizza(&portList,maxPipeSizeMbps,minSliceResolutionMpbs, pizzaArraySize, /*OUT*/pizzaArray, /*OUT*/&numberOfSlices);
        if (rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    rc = prvBuildTxQPizzaDistribution(portsConfigArrayPtr,
                                              portsConfigArraySize,
                                                                          maxPipeSizeMbps,
                                                                          minSliceResolutionMpbs,
                                      /*OUT*/pizzaArraySize,&pizzaArray[0], numberOfSlicesPtr,
                                      /*OUT*/numberOfHighSpeedPortsPtr,highSpeedPortsArr);
    if (rc != GT_OK)
    {
        return rc;
    }
...
GT_STATUS setTxQ64BW(GT_U32 bw)
{
    ...
    GT_U32  g_highSpeedPortsArr[PRV_CPSS_DXCH_PORT_HIGH_SPEED_PORT_NUM_CNS];
