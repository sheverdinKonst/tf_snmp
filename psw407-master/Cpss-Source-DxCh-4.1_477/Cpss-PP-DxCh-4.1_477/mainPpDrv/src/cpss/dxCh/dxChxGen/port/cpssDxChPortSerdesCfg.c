...
)
{
            ...
            (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_XCAT3_E))
    {
        ...
        else
        {
            ...
            rc = prvCpssDxChHwsPortsParamsCfgGet(devNum, localPort, portMode, &macNum,
...
)
{
    ...
    rc = prvCpssDxChHwsPortsParamsCfgGet(devNum,
                              localPort, portMode, &macNum, &pcsNum, &sdVecSize,
                              &sdVectorPtr);
    if(rc != GT_OK)
    {
        return rc;
...
*
* NOT APPLICABLE DEVICES:
*   DxCh1; DxCh1_Diamond; DxCh2; DxCh3,Lion2; Lion3: Bobcat2.
*
* INPUTS:
*       devNum      - system device number
*       portNum     - number of physical port all serdeses occupied by it, in
...
)
{
    ...
    if(rc != GT_OK)
    {
        cpssOsPrintf("prvCpssDxChPortInterfaceModeHwGet fail:rc=%d", rc);
        return rc;
    }
    ...
    if(rc != GT_OK)
    {
        cpssOsPrintf("prvCpssDxChPortSpeedHwGet fail:rc=%d", rc);
        return rc;
    }
...
*
* NOT APPLICABLE DEVICES:
*        DxCh1; DxCh1_Diamond; DxCh2; DxCh3; xCat; Lion; xCat3; xCat2; Lion3; Bobcat2.
*
* INPUTS:
...
)
{
	...
	CPSS_LOG_INFORMATION_MAC("Calling: mvHwsPortEnhanceTuneMaxDelaySet(devNum[%d], portGroup[%d], serdesNum[%d], serdesType[%d], *results)", (GT_U8)devNum, (GT_U32)portGroup, (GT_U32)phyPortNum, (MV_HWS_PORT_STANDARD)hwsPortMode, (GT_U8)maxDelay);
    rc = mvHwsPortEnhanceTuneMaxDelaySet(
        (GT_U8)                   devNum,
        (GT_U32)                  portGroup,
        (GT_U32)                  phyPortNum,
...
)
{
    ...
    CPSS_API_LOCK_MAC(0,0);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, maxDelay));

    rc = internal_cpssDxChPortSerdesEnhancedAutoTune(devNum, portNum, maxDelay);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portNum, maxDelay));
    CPSS_API_UNLOCK_MAC(0,0);
