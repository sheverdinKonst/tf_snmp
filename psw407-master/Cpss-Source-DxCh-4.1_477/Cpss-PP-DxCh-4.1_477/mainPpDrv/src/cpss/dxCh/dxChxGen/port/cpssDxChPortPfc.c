...
)
{
    ...
    GT_U32 portTxqMap; /* number of Txq mapped to this physical port */

    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_CH1_E | CPSS_CH1_DIAMOND_E |
                CPSS_CH2_E | CPSS_CH3_E | CPSS_XCAT_E | CPSS_XCAT3_E | CPSS_XCAT2_E);

    PRV_CPSS_DXCH_PORT_NUM_CHECK_AND_TXQ_PORT_GET_MAC(devNum, portNum, portTxqMap);
    ...
    if(PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_MAC(devNum) == GT_TRUE)
    {
        regAddr = PRV_DXCH_REG1_UNIT_TXQ_PFC_MAC(devNum).PFCPortProfile[(portTxqMap >> 3)];
        fieldOffset = (portTxqMap%8)*3;
    }
    else
    {
...
)
{
    ...
    rc = prvCpssHwPpGetRegField(devNum, regAddr, 0, 16, receivedCntPtr);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    return prvCpssHwPpGetRegField(devNum, regAddr, 0, 16, droppedCntPtr);
}
