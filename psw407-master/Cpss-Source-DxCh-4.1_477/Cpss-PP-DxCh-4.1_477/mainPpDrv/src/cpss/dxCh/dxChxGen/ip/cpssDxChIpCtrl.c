...
static CPSS_DXCH_IP_EXCPTION_CMD_REG_INFO_STC dxCh3AndAboveExceptions[] =
{
 ...
 /* exceptions entries that apply to Lion3 and Bobcat2 devices */
            ...
            case CPSS_DXCH_IP_CNT_SET_INTERFACE_MODE_E:
                ...
                switch (interfaceModeCfgPtr->portTrunkCntMode)
                {
                    ...
                    case CPSS_DXCH_IP_PORT_CNT_MODE_E:
                        ...
                        else/* CPSS_PP_FAMILY_DXCH_LION3_E */
                        {
                            data2 |= ((interfaceModeCfgPtr->portTrunk.port & 0x7FFF) << 0);
                            data2 |= ((interfaceModeCfgPtr->hwDevNum & 0x3FF) << 15);
                        }
...
)
{
    ...
    return prvCpssDrvHwPpSetRegField(devNum, regAddr, 0, 24, mtu);
}
...
)
{
    ...
    rc = prvCpssDrvHwPpWriteRegBitMask(devNum, regAddr1, mask1, value1);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    rc = prvCpssDrvHwPpWriteRegBitMask(devNum, regAddr2, mask2, value2);
    if (rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    rc = prvCpssDrvHwPpReadRegister(devNum, regAddr1, &value1);
    if (rc != GT_OK)
    {
        return rc;
    }
    rc = prvCpssDrvHwPpReadRegister(devNum, regAddr2, &value2);
    if (rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    rc = prvCpssDrvHwPpSetRegField(devNum, regAddr1, 27, 4, (tokenBucketIntervalSlowUpdateRatio-1));
    if(rc != GT_OK)
        return rc;

    rc = prvCpssDrvHwPpSetRegField(devNum, regAddr2, 0, 4, tokenBucketUpdateInterval);
    if(rc != GT_OK)
        return rc;

     rc = prvCpssDrvHwPpSetRegField(devNum, regAddr2, 8, 1, value);
    if(rc != GT_OK)
        return rc;
...
)
{
    ...
    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr1, 27, 4, &value);
    if(rc != GT_OK)
        return rc;

    *tokenBucketIntervalSlowUpdateRatioPtr = value + 1;

    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr2, 0, 4, &value);
    if(rc != GT_OK)
        return rc;

    *tokenBucketUpdateIntervalPtr = value;

    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr2, 8, 1,  &value);
    if(rc != GT_OK)
        return rc;
