 ...
 *      if packet is FCoE then  byte 0 is set to 0 and the rest (bytes 1,2,3)are FC D_ID.
 *      if packet is IPv4 then  first 4 bytes (0,1,2,3) hold IPv4 DIP.
 *      if packet is IPv6 then  first 4 bytes (0,1,2,3) hold IPv6 DIP[31:0].
 *      if packet is not IP then first 4 bytes(0,1,2,3)  are set to 0.
 *      if packet is IPv6 then last 12 bytes (4 - 15) hold IPv6 DIP[127:32].
 *      if packet is not IPv6 these bytes contains UD0-UDB11.
 *
 *    ipSip      - IPv6/v4 SIP  : 16 bytes
 *      if packet is FCoE then  byte 0 is set to 0 and the rest (bytes 1,2,3)are FC S_ID.
 *      if packet is IPv4 then  first 4 bytes (0,1,2,3) hold IPv4 SIP.
 *      if packet is IPv6 then  first 4 bytes (0,1,2,3) hold IPv6 SIP[31:0].
 *      if packet is not IP then first 4 bytes(0,1,2,3)  are set to 0.
 *      if packet is IPv6 then last 12 bytes (4 - 15) hold IPv6 SIP[127:32].
 *      if packet is not IPv6 these bytes contains UDB23-UDB34.
 *
