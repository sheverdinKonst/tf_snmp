 ...
 *      (priority = 5) with min shaper with shaper ignore for propagated priority = 3
 ...
 *      PPA - Propagated priority according to RevA . i.e. - only propagated priority 0...3 are supported.
 ...
 *                                  (APPLICABLE RANGES: 1..84479)
 *      quantum                   - Queue DWRR Quantum in CPSS_TM_NODE_QUANTUM_UNIT_CNS
 *                                  (APPLICABLE RANGES: 0x40..0x3FFF) in 256Bytes units
 *      dropProfileInd            - Index of Queue Drop profile
 *                                 (APPLICABLE RANGES: 0..2047)
 *      eligiblePrioFuncId        - Eligible Priority Function pointer
 *                                 (APPLICABLE RANGES: 0..17, 63)
 *
 ******************************************************************************/
typedef struct
{
 ...
 *      cbs                       - CBS in Kbytes
 *      ebs                       - EBS in Kbytes
 *      quantumArr[8]             - Queue DWRR Quantum in CPSS_TM_PORT_QUANTUM_UNIT_CNS
 *                                  (APPLICABLE RANGES: 0x10..0x1FF) in 64Bytes units
 *      schdModeArr[8]            - RR/DWRR Priority for B-Node Scheduling
 *                                  (APPLICABLE RANGES: 0..1)
 *      dropProfileInd            - Index of Drop profile
 *                                  (APPLICABLE RANGES: 0..191)
 *      eligiblePrioFuncId        - Eligible Priority Function pointer
 *                                  (APPLICABLE RANGES: 0..31, 63)
 *      numOfChildren             - Number of children nodes
...
typedef struct
{
	...
	GT_U32   old;
    GT_U32   new;
    struct CPSS_TM_TREE_CHANGE_STC *nextPtr;
