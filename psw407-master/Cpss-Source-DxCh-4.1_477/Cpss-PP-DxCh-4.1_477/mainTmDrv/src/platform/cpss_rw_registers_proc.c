...
#ifdef WRITE_REGISTERS_DUMP
	fregistersDump=fopen("c:/temp/dtemp/write_registers_dump.txt","a");
	if (fregistersDump)
	{
		fprintf(fregistersDump,"%6d  address=0x%x %x     value=0x%08x%08x\n",stringIndex,regAddr[0],regAddr[1],regValue[1],regValue[0]);
		stringIndex++;
		fclose(fregistersDump);
	}
#endif
	if (burstMode)
	{
...
/*		fRWerrorLog=fopen("c:/temp/dtemp/rw_errors_log.txt","a");
		...
		{
*/
		fprintf(stderr,"address=0x%x %x   written_value=0x%08x%08x   read_value=0x%08x%08x \n",regAddr[0],regAddr[1],regValue[1],regValue[0],readValue[1],readValue[0]);
/*			fclose(fRWerrorLog);
		}
*/
#endif
