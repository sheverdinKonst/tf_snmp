...
int tm_lib_sw_init(tm_handle hEnv, tm_handle *htm, struct tm_lib_init_params *init_params_PTR)
{
   ...
   /* Get general parameters : LAD frequency,  dwrr params  - they are readed from H/W resources*/
    ...
    ctl->tm_b_lvl_drop_prof_ptr = tm_malloc(sizeof(uint8_t) * ctl->tm_total_b_nodes);
    ...
    for (i=0 ; i < ctl->tm_total_ports; i++)			set_sw_port_default(&(ctl->tm_port_array[i]),1);
    for (i=0 ; i < ctl->tm_total_c_nodes; i++)			set_sw_c_node_default(&(ctl->tm_c_node_array[i]),1);
    for (i=0 ; i < ctl->tm_total_b_nodes; i++)			set_sw_b_node_default(&(ctl->tm_b_node_array[i]),1);
    for (i=0 ; i < ctl->tm_total_a_nodes; i++)			set_sw_a_node_default(&(ctl->tm_a_node_array[i]),1);
    for (i=0 ; i < ctl->tm_total_queues; i++)			set_sw_queue_default(&(ctl->tm_queue_array[i]),1);

    for (i=0; i < TM_NUM_QUEUE_DROP_PROF; i++)			set_sw_drop_profile_default(ctl->tm_q_lvl_drop_profiles, i);
    for (i=0; i < TM_NUM_A_NODE_DROP_PROF; i++)			set_sw_drop_profile_default(ctl->tm_a_lvl_drop_profiles, i);
    for (i=0; i < TM_NUM_B_NODE_DROP_PROF; i++)			set_sw_drop_profile_default(ctl->tm_b_lvl_drop_profiles, i);
    for (j=0; j < TM_WRED_COS; j++)
	{
        for (i=0; i < TM_NUM_C_NODE_DROP_PROF; i++)		set_sw_drop_profile_default(ctl->tm_c_lvl_drop_profiles[j], i);
    }
    for (i=0; i < ctl->tm_total_ports; i++)				set_sw_drop_profile_default(ctl->tm_p_lvl_drop_profiles, i);
    for (j=0; j < TM_WRED_COS; j++)
    {
        for (i=0; i < ctl->tm_total_ports; i++) 		set_sw_drop_profile_default(ctl->tm_p_lvl_drop_profiles_cos[j], i);
    }
    
    for (i=0; i < TM_NUM_WRED_QUEUE_CURVES; i++) 		set_sw_wred_curve_default(ctl->tm_wred_q_lvl_curves, (uint16_t)i);
    for (i=0; i < TM_NUM_WRED_A_NODE_CURVES; i++)		set_sw_wred_curve_default(ctl->tm_wred_a_lvl_curves, (uint16_t)i);
    for (i=0; i < TM_NUM_WRED_B_NODE_CURVES; i++)		set_sw_wred_curve_default(ctl->tm_wred_b_lvl_curves, (uint16_t)i);
    for (j=0; j < TM_WRED_COS; j++)
	{
        for (i=0; i < TM_NUM_WRED_C_NODE_CURVES; i++)	set_sw_wred_curve_default(ctl->tm_wred_c_lvl_curves[j], (uint16_t)i);
    }
    for (i=0; i < TM_NUM_WRED_PORT_CURVES; i++)			set_sw_wred_curve_default(ctl->tm_wred_ports_curves, (uint16_t)i);
    for (j=0; j < TM_WRED_COS; j++)
    {
        for (i=0; i < TM_NUM_WRED_PORT_CURVES; i++) 	set_sw_wred_curve_default(ctl->tm_wred_ports_curves_cos[j], (uint16_t)i);
    }
    
    for (i=0; i < ctl->tm_total_queues; i++)        ctl->tm_q_lvl_drop_prof_ptr[i] = TM_NO_DROP_PROFILE;
    for (i=0; i < ctl->tm_total_a_nodes; i++)       ctl->tm_a_lvl_drop_prof_ptr[i] = TM_NO_DROP_PROFILE;
    for (i=0; i < ctl->tm_total_b_nodes; i++)       ctl->tm_b_lvl_drop_prof_ptr[i] = TM_NO_DROP_PROFILE;
    for (i=0; i < TM_WRED_COS; i++)
    {
        for (j=0; j < ctl->tm_total_c_nodes; j++)   ctl->tm_c_lvl_drop_prof_ptr[i][j] = TM_NO_DROP_PROFILE;
    }
            
    for (i=0; i < ctl->tm_total_ports; i++)         ctl->tm_p_lvl_drop_prof_ptr[i] = TM_NO_DROP_PROFILE;
    for (i=0; i < TM_WRED_COS; i++)
    {
        for (j=0; j < ctl->tm_total_ports; j++)     ctl->tm_p_lvl_drop_prof_ptr_cos[i][j] = TM_NO_DROP_PROFILE;
    }
