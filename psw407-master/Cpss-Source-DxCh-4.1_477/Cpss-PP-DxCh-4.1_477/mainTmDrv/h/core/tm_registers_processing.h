...
#define	 TM_REG_ROW_SET(field_name,shift,width,default_value, ...) \
	else if (strcmp(DUMMY_MACRO(PARAM_1_OF_3(__VA_ARGS__)),#field_name)==0) \
	{\
...
#define	 TM_REG_ROW_GET(field_name,shift,width,default_value, ...) \
	else if (strcmp(DUMMY_MACRO(PARAM_1_OF_4(__VA_ARGS__)),#field_name)==0)\
	{\
...
/* if use register description using shifts: */
	...
	{\
		...
		else	{ 	fprintf(stderr,"%s line:%d unknown parameter %s used in assignment. aborting.\n",__FILE__,__LINE__,#param_name);	exit(1); } \
	}
	...
	{\
		...
		else	{ 	fprintf(stderr,"%s line:%d unknown parameter %s used in read request. aborting.\n",__FILE__,__LINE__,#param_name);	exit(1); } \
	}

	#define	TM_REGISTER_RESET(register) \
	{\
		memset(TM_REGISTER_VAR_ADDR(register),0,sizeof(char)*8);\
