...
{
    GT_U32 numEntries = *numEntriesPtr;
    PRV_CPSS_LOG_API_HISTORY_ENTRY_STC * currentHistoryEntryPtr = *entryPtrPtr;
    ...
    if (currentHistoryEntryPtr->entryHistoryFile)
    {
        /* Close file */
...
)
{
    ...
    for (entryIndex = 0; entryIndex < numEntries; entryIndex++)
    {
        if (currentHistoryEntryPtr->entryHistoryFile)
        {
            ...
            if (currentHistoryEntryPtr->entryHistoryFile == 0)
            {
...
)
{
    ...
    /* log error message */
    PRV_CPSS_LOG_AND_HISTORY_MAC(PRV_CPSS_LOG_PHASE_ERROR_E, contextLib, CPSS_LOG_TYPE_ERROR_E, "ERROR %s in function: %s, file %s, line[%d]. %s\n", enumStringPtr, functionName, fileName, line, errorMsgPtr);
}
...
)
{
    ...
    PRV_CPSS_LOG_AND_HISTORY_MAC(PRV_CPSS_LOG_PHASE_INFO_E, contextLib, CPSS_LOG_TYPE_INFO_E, "Information: in function %s, file %s, line[%d]. %s\n", functionName, fileName, line, buffer);
}
...
* RETURNS :
*       GT_OK           - on success
*       GT_BAD_PARAM    - bad parameter size
*
* COMMENTS:
*       None.
*
...
)
{
    PRV_CPSS_LOG_ZERO_VALUE_LOG_CHECK_MAC(numValue);
    prvCpssLogTabs(contextLib, logType);
    PRV_CPSS_LOG_AND_HISTORY_MAC(PRV_CPSS_LOG_PHASE_INFO_E, contextLib, logType, "%s = %p\n", fieldName, numValue);
}
...
)
{
    ...
    {
        PRV_CPSS_LOG_AND_HISTORY_MAC(PRV_CPSS_LOG_PHASE_ERROR_E, contextLib, CPSS_LOG_TYPE_ERROR_E, "ERROR: Null pointer array %s = NULL\n", namePtr);
        return;
    }
    ...
    {
        cpssOsSprintf(buffer1,"%s[%d]", namePtr, i);
        logFunc(contextLib, logType, buffer1, (GT_VOID *)currPtr);
...
)
{
    ...
    for (i = 0; i < rowNum; i++)
    {
        for (j = 0; j < rowNum; j++)
        {
            ...
            cpssOsSprintf(buffer1, "%s[%d][%d]", namePtr, i, j);
            logFunc(contextLib, logType, buffer1, (GT_VOID *)currPtr);
            /* Increment pointer address */
            currPtr += elementSize;
        }
