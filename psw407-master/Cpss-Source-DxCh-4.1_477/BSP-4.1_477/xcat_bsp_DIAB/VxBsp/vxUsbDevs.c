...
#include "examples/disk.h"
#include "examples/mouse.h"
...
#endif 
    return mvOsIoUncachedFree(pDev, size, (void *)phyAddr, pVirtAddr, NULL);
} 
...
static void    mvUsbDevIsr(int dev);
...
static int    mvUsbDevLoad(int dev)
{
    ...
    if(mvUsbGppMode != 0)
    {
        ...
        if(mvUsbDevice[dev].gppNo != (MV_U8)N_A)
        {
            if(mvUsbGppMode == 2)
            {
                /* Interrupt mode */
				int gppNo, group;
				gppNo = mvUsbDevice[dev].gppNo % 32;
				group = mvUsbDevice[dev].gppNo / 32;
                status = vxGppIntConnect(gppNo, group, mvUsbGppIsr, dev, 0);
                if (status != OK)
                {
                ...
                status = vxGppIntEnable(gppNo, group);
                if (status != OK)
                {
    ...
    status = intConnect(INUM_TO_IVEC(INT_LVL_USB_CNT(dev)), mvUsbDevIsr, (int)dev);
    if (status != OK)
    {
...
static void    mvUsbDevUnload(int dev)
{
    ...
    if(mvUsbDevice[dev].semId != NULL)
    {
        if (semDelete(mvUsbDevice[dev].semId) == ERROR)
        {
            printf("%s:%d: semDelete failed.\n", __func__, __LINE__);
            return;
        }
        mvUsbDevice[dev].semId = NULL;
    }
