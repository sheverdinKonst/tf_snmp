 ...
 )
{
  ...
  if (pciIf >= mvCtrlPciMaxIfGet()) /* is PEX interface? */
  {
    if (mvPexIsPowerUp(pciIf - mvCtrlPciMaxIfGet()) == MV_FALSE)
      return ERROR;
  }
...
{          
  ...
  {
    DB1(mvOsPrintf("sysPciHostIfGet: Illegal bus number=%d \n",bus));
    return ILLEGAL_INTERFACE;
  }
  DB1(if (bus2Pci[bus] == ILLEGAL_INTERFACE) \
 ...
 )
{
  ...
  pciIf = pciIf - mvCtrlPciMaxIfGet();  /* pex counter */
...
void pciShow()
{
  ...
  for (pciIf=0 ; pciIf < (mvCtrlPciIfMaxIfGet()); pciIf++ )
  {        
 ...
 )
{
  ...
  pPciList = pciAutoListCreate1 (pSystem, &listSize);
  DB(mvOsPrintf("pciAutoCfgFunc1: pciAutoListCreate1 return pPciList=0x%x\n",pPciList));
