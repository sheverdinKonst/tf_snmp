...
#ifdef INCLUDE_XOR_COPY
void mvXorMemCopy (UINT *source, UINT *destination, UINT nlongs);
#endif
...
#define ROM_OFFSET(adr) (((UINT)adr - (UINT)romInit) + romInitAddr)
...
void romStart (FAST int startType)
{
...
# define copyLongs romcopyLongs
    ...
    unsigned int romInitAddr = _romInitAddr();

    {
    extern unsigned int _G_xCatRevision;

    if (_G_xCatRevision > 2)
    {
...
#  if (CPU_FAMILY == MIPS)
     ...
     * copy back caches
     */
    ((FUNCPTR)ROM_OFFSET(copyLongs)) (romInitAddr, (UINT)K0_TO_K1(romInit),
        ROM_COPY_SIZE / sizeof (long));
#  else   /* CPU_FAMILY == MIPS */
    ((FUNCPTR)ROM_OFFSET(copyLongs)) (romInitAddr, (UINT)romInit,
        ROM_COPY_SIZE / sizeof (long));
...
#  else   /* CPU_FAMILY == MIPS */
    ((FUNCPTR)ROM_OFFSET(copyLongs)) (romInitAddr, (UINT)romInit,
        ((UINT)binArrayEnd - (UINT)romInit)/ sizeof (long));
    ((FUNCPTR)ROM_OFFSET(copyLongs))
           ((UINT *)(romInitAddr + ((UINT)BINARRAYEND_ROUNDOFF -
...
# elif   (CPU_FAMILY == I80X86) || (CPU_FAMILY == ARM) || (CPU_FAMILY == COLDFIRE)
    ...
    /* __asm("mcr p15, 0, %0, c7, c6, 0" : : "r" (0)); */
    if ((absUncompress) ((UCHAR *)(binArrayStart),
    ...
    UINT nlongs;        /* number of longs to copy       */
{
    ...
    mvXorMemCopy(source, destination, nlongs);
#else
    FAST UINT nchunks;
...
void mvXorMemCopy (UINT *source, UINT *destination, UINT nlongs)
{
    ...
    MV_CRC_DMA_DESC *copyDesc;
    int chan = 0;
    copyDesc = (MV_CRC_DMA_DESC*)((((int)arr) + 64) & (~0x1f));

    copyDesc->status = 0x80000000; /* set ownership to XOR */
    copyDesc->nextDescPtr = (unsigned int)NULL;
    copyDesc->byteCnt = (unsigned int)(nlongs * 4);
    copyDesc->destAdd = (unsigned int)destination;
    copyDesc->srcAdd0 = (unsigned int)source;

    /* read configuration register and mask the operation mode field */
    temp = MV_REG_READ(XOR_CONFIG_REG(XOR_UNIT(chan),XOR_CHAN(chan)));
    temp &= ~(XEXCR_OPERATION_MODE_MASK | XEXCR_SRC_BURST_LIMIT_MASK);
    temp |= (XEXCR_OPERATION_MODE_DMA | (2 << XEXCR_SRC_BURST_LIMIT_OFFS));
    ...
    MV_REG_WRITE(XOR_NEXT_DESC_PTR_REG(XOR_UNIT(chan),XOR_CHAN(chan)),
                 (unsigned int)copyDesc);

    __asm("mcr p15, 0, %0, c7, c10, 1" : : "r" ((unsigned int)copyDesc));

    /* Clean cause register */
    MV_REG_WRITE(XOR_CAUSE_REG(chan), 0);
    ...
    while(!(MV_REG_READ(XOR_CAUSE_REG(chan)) & (3 << chan)));

    /* TO DO Add invalidate to data cache */
