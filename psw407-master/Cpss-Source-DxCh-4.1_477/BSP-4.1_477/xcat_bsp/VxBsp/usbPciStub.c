...
#include "drv/pci/pciIntLib.h"      /* VxWorks PCI funcs */
...
#define MV_USB_PCI_IO_OFFSET    0x0
#define MV_USB_PCI_MEMIO_OFFSET 0x0
#define MV_USB_PCI_MEM_OFFSET   0x0
...
#define USB_INT_CONNECT(intNo, func, param)         \
    pciIntConnect( (VOIDFUNCPTR*)INUM_TO_IVEC(intNo), (VOIDFUNCPTR)func, (int)param)

#define USB_INT_ENABLE(i)   sysPciIntEnable(i)
#define USB_INT_DISABLE(i)  sysPciIntDisable(i)
...
#define USB_PCI_IN_BYTE(a)      PCI_IN_BYTE(a)      
#define USB_PCI_IN_WORD(a)      PCI_IN_WORD(a) 
#define USB_PCI_IN_DWORD(a)     PCI_IN_LONG(a)      
#define USB_PCI_OUT_BYTE(a,v)   PCI_OUT_BYTE(a,v)   
#define USB_PCI_OUT_WORD(a,v)   PCI_OUT_WORD(a,v)       
#define USB_PCI_OUT_DWORD(a,v)  PCI_OUT_LONG(a,v)   
...
UINT32  usb32bitSwap(UINT32 value)
{
    UINT32  swappedValue = MV_BYTE_SWAP_32BIT(value);
    
    return swappedValue;
}

int  usbVirtToPhysAddrMap(void* pVirtAddr)
{
    return (UINT32)(pVirtAddr);
}

STATUS    usbNonPciIntConnect(FUNCPTR func, INT32 param, int intNo)
{
    STATUS  status;

    status = intConnect( INUM_TO_IVEC(intNo), (VOIDFUNCPTR)func, (int)param);
    if(status != OK)
        return ERROR;
...
STATUS    usbNonPciIntDisconnect(FUNCPTR func, INT32 param, int intNo)
{
    intDisable(intNo);
...
LOCAL UINT32 usbDevId = PCI_VENDOR_ID_MARVELL;
/***************************************************************************
*
...
)
{
    ...
    int intFuncNo;
/*
    printf("usbPciClassFind: pciClass=0x%02x, subClass=0x%02x, pgmIf=0x%02x, idx=%d\n",
            pciClass, subClass, pgmIf, index);
*/
    /* Check for Non-PCI USB controllers */
    if(pgmIf == (EHCI_PGMIF | USB_IF_NON_PCI))
    {
        int                 i, usb_host_num=0;
        USB_HCD_BUS_INFO*	pHcdBusInfo = (USB_HCD_BUS_INFO*)pBusNo;

        for(i=0; i<mvCtrlUsbMaxGet(); i++)
        {
            if(mvUsbCtrlIsHost[i] == MV_FALSE)
            ...
            if(index == (usb_host_num-1))
                break;
        }
        if(i >= mvCtrlUsbMaxGet() )
            return FALSE;

#ifdef MV_CPU_BE
        pHcdBusInfo->pFuncDataSwap = (FUNCPTR)usb32bitSwap;
#else
        pHcdBusInfo->pFuncDataSwap = NULL;
#endif

        pHcdBusInfo->pFuncBufferSwap = NULL;
        pHcdBusInfo->pFuncCpuToBus = usbVirtToPhysAddrMap;
        pHcdBusInfo->pFuncBusToCpu = NULL;
        pHcdBusInfo->pFuncIntConnect = usbNonPciIntConnect;
        pHcdBusInfo->pFuncIntDisconnect = usbNonPciIntDisconnect;

        pHcdBusInfo->pBaseAddress = (UINT8*)(INTER_REGS_BASE | MV_USB_CORE_CAP_LENGTH_REG(i));
        pHcdBusInfo->irq = INT_LVL_USB_CNT(i);

        /* Update USB core revision: 0 - Initial, 1 - with Underrun fixed */
        usbDevId |= (MV_USB_VERSION << 16);

        pHcdBusInfo->pDeviceSpecificData = &usbDevId;
        printf("Non-PCI USB device found: pgmIf=0x%02x, idx=%d (%d), baseAddr=%p, irq=%d, swapFunc=%p\n", 
                pgmIf, index, i, pHcdBusInfo->pBaseAddress, 
                pHcdBusInfo->irq, pHcdBusInfo->pFuncDataSwap);
    ...
    printf("PCI USB device found: pgmIf=0x%02x, idx=%d, bus=%d, dev=%d, func=%d\n", 
                    pgmIf, index, intBusNo, intDeviceNo, intFuncNo);
    return TRUE;
}
...
)
{
    UINT8 value;
/*
    printf("%s: busNo=%d, deviceNo=%d, funcNo=%d, regOffset=%d\n", 
            __FUNCTION__, busNo, deviceNo, funcNo, regOffset);

    if( (busNo == 255) && (deviceNo == 255) && (funcNo == 255) ) 
        return 0;
*/
...
)
{
    ...
    printf("%s: busNo=%d, deviceNo=%d, funcNo=%d, pCfgHdr=%p\n", 
            __FUNCTION__, busNo, deviceNo, funcNo, pCfgHdr);
...
/*
    if( (busNo == 255) && (deviceNo == 255) && (funcNo == 255) ) 
        return;
*/
    pciConfigOutByte (busNo, deviceNo, funcNo, regOffset, data);
}
...
/*
    if( (busNo == 255) && (deviceNo == 255) && (funcNo == 255) ) 
        return;
*/
    pciConfigOutWord (busNo, deviceNo, funcNo, regOffset, data);
}
...
/*
    if( (busNo == 255) && (deviceNo == 255) && (funcNo == 255) ) 
        return;
*/
    pciConfigOutLong (busNo, deviceNo, funcNo, regOffset, data);
}
...
{
    return MV_USB_PCI_MEMIO_OFFSET;
}
...
)
{
    ...
    return ((UINT32) pPhys) + MV_USB_PCI_MEM_OFFSET;
}
...
)
{
    return CACHE_DRV_PHYS_TO_VIRT (&cacheUserFuncs, 
                    (void *) (pciAdrs - MV_USB_PCI_MEM_OFFSET));
}
...
{
    printf("usbIntConnect: func=%p, intNo=%d, param=%p\n", 
			func, intNo, param);

    if (USB_INT_CONNECT ((int)intNo, (VOIDFUNCPTR)func, (int)param) != OK)
    {
        printf("USB_INT_CONNECT failed\n");
        return ERROR;
    }
    if (USB_INT_ENABLE (intNo) != OK)
    {
        printf("USB_INT_ENABLE failed\n");
        return ERROR;
    }
...
/*------------------------------------------------------------------------------*/
/*-----------------this routine is not necessary beacuse we do scan for all -----*/
/*-----------------PCI devices -------------------------------------------------*/
/*------------------------------------------------------------------------------*/
