...
MV_BOOL mgiStatsPolling = MV_TRUE;
...
LOCAL void      mgiTxDoneIsr (DRV_CTRL *pDrvCtrl);
LOCAL void      mgiRxReadyIsr(DRV_CTRL *pDrvCtrl);
LOCAL void      mgiRxReadyJob(DRV_CTRL *pDrvCtrl, MV_U32 cause);
LOCAL void      mgiRxErrorJob(DRV_CTRL *pDrvCtrl, MV_U32 cause);
LOCAL  int      mgiRxReady(DRV_CTRL *pDrvCtrl, int rxq, int quota);
LOCAL void      mgiLinkChangeJob(DRV_CTRL *pDrvCtrl, int isLinkUp);
LOCAL void      mgiMiscIsr(DRV_CTRL *pDrvCtrl);
...
static     void repairRxDescRing(ETH_PORT_CTRL* pPortCtrl, int queue);
...
static INLINE int mgiRxQueueGet(MV_U32 cause)
{
    ...
    {
        if (cause & (1 << rxq))
            return rxq - 2 /* Q0 = 2, Q1 = 3 etc. */;
    }
    return 0; /* default rx queue */
}

static INLINE int mgiRxErrorQueueGet(MV_U32 cause)
{
    int rxq = MV_ETH_RX_Q_NUM - 1;

    for (rxq = (MV_ETH_RX_Q_NUM - 1); rxq > 0; rxq--)
    {
        if (cause & (1 << ETH_CAUSE_RX_ERROR_BIT(rxq)))
          return rxq;
    }
    return 0; /* default rx queue */
}
...
*/
                ...
                {
                    MGI_DEBUG_CODE(MGI_DEBUG_FLAG_STATS, (mgiMblkClDoubleFreeCount++));
            ...
            else
            {
                ...
                MGI_DEBUG_CODE(MGI_DEBUG_FLAG_STATS, (mgiMblkClFreeRtnCount++));
            }
            pMblk = NULL;
        }
        else
        {
            MGI_DEBUG_CODE(MGI_DEBUG_FLAG_STATS, (mgiMblkClRefFreeCount++));
    ...
    {
        /* free the mBlk */
        MGI_DEBUG_CODE(MGI_DEBUG_FLAG_STATS, (mgiMblkFreeCount++));
...
END_OBJ* mgiEndLoad(char *initString, void* dummy)
{
    ...
    if(mgiIsFirstTime)
    {
        ...
        mvEthRandomizeMac(mgiUserDefMacAddr);
    ...
    /*
     * Note: xCat OOB doen't need FTDLL WA; xCat2-QFP doesn't have OOB port.
     */
    ...
    for(i=0; i<ETH_NUM_OF_TX_DESCR; i++)
    {
        ...
        pDrvCtrl->txPktInfo[i]->srcIdx = (char)-1;
...
/*  ******************* */
    ...
    pDrvCtrl->endObj.pMib2Tbl = m2IfAlloc(M2_ifType_ethernet_csmacd,
                                   (UINT8*)pDrvCtrl->macAddr, MV_MAC_ADDR_SIZE,
                                    mgiMtuSize, MV_NET_1GBS, DEV_NAME, pDrvCtrl->unit);
...
LOCAL MV_STATUS mgiEthPortInit(DRV_CTRL *pDrvCtrl)
{
    ...
    portInit.mru          = MGI_RX_BUF_DEF_SIZE;
...
LOCAL STATUS mgiSend(DRV_CTRL *pDrvCtrl, M_BLK *pMblkPkt)
{
           ...
           (pMblkPkt->mBlkHdr.mData != NULL) )
    {
        ...
        status = mBlkToPktInfo(pMblkPkt, pktInfo, MGI_MAX_BUF_PER_PKT);
        pDrvCtrl->debugStats.txMaxBufCount = pktInfo->numFrags;
        ...
        if (status == MV_OK)
        {
            ...
            status = mvEthPortSgTx(pDrvCtrl->pEthPortHndl, queue, pktInfo);
        }
...
LOCAL void mgiMiscIsr(DRV_CTRL *pDrvCtrl)
{
    ...
    if( cause & (1 << ETH_CAUSE_LINK_STATE_CHANGE_BIT) )
    {
        ...
        rc = netJobAdd( (FUNCPTR)mgiLinkChangeJob,
                        (int)pDrvCtrl,
...
LOCAL void mgiLinkChangeJob(DRV_CTRL *pDrvCtrl, int isLinkUp)
{
    ...
    if((MV_BOOL)isLinkUp == MV_TRUE)
    {
        ...
        {
            if (mvEthPortLinkCfgFromPhy(pDrvCtrl->pEthPortHndl) != MV_OK)
            {
...
LOCAL void mgiTxDoneIsr(DRV_CTRL *pDrvCtrl)
{
    ...
    MGI_DEBUG_CODE(MGI_DEBUG_FLAG_STATS, (pDrvCtrl->debugStats.txDoneIsrCount++));
...
LOCAL void mgiRxReadyIsr(DRV_CTRL *pDrvCtrl)
{
    ...
    /* check for rxerror */
    if( cause & (1 << ETH_CAUSE_RX_ERROR_SUM_BIT) )
    {
      /* schedule the mgiRxErrorJob in netTask context */
      rc = netJobAdd( (FUNCPTR)mgiRxErrorJob,
                      (int)pDrvCtrl,
                      cause, 0, 0, 0 );
      if(rc != OK)
        pDrvCtrl->debugStats.netJobAddError++;
    ...
    MGI_DEBUG_CODE(MGI_DEBUG_FLAG_STATS, (pDrvCtrl->debugStats.rxReadyIsrCount++));
...
LOCAL int mgiRxReady(DRV_CTRL *pDrvCtrl, int queue, int quota)
{
    ...
    while( count < quota)
    {
        ...
        mvOsCacheUnmap(NULL, pMblk->pClBlk->clNode.pClBuf,
...
LOCAL void mgiRxReadyJob(DRV_CTRL *pDrvCtrl, MV_U32 cause)
{
    ...
    MGI_DEBUG_CODE(MGI_DEBUG_FLAG_STATS, (pDrvCtrl->debugStats.rxReadyJobCount++));
...
/*******************************************************************************
* mgiRxErrorJob - Service task-level interrupts for rxError handling.
*
* DESCRIPTION:
*       This routine runs in netTask's context.  It handles Rx Error conditions.
*
* INPUT:
*       DRV_CTRL *pDrvCtrl       pointer to DRV_CTRL structure
*
...
LOCAL void mgiRxErrorJob(DRV_CTRL *pDrvCtrl, MV_U32 cause)
{
    ...
    MGI_DEBUG_PRINT(MGI_DEBUG_FLAG_TRACE, ("ENTER mgiRxErrorJob: "
                                           "ifId=%d, cause=0x%x\n",
                                           pDrvCtrl->unit, cause) );

    MGI_DEBUG_CODE(MGI_DEBUG_FLAG_STATS, (pDrvCtrl->debugStats.rxReadyJobCount++));
    queue = mgiRxErrorQueueGet(cause);
    mvOsPrintf("%s%d Interface encountered Rx errors in queue %d. "
               "Performing port repair procedure\n",                
               DEV_NAME, pDrvCtrl->unit, queue);
    
    mgiStop(pDrvCtrl);

    /* Fix RX queues: */
    for(queue=0; queue<MV_ETH_RX_Q_NUM; queue++)
      repairRxDescRing((ETH_PORT_CTRL*)(pDrvCtrl->pEthPortHndl) ,queue);

    mgiStart(pDrvCtrl);

    mvOsPrintf("%s%d Repair done.\n", DEV_NAME, pDrvCtrl->unit);
    ...
    while(cause != 0)
    {
        ...
        while(MV_TRUE)
        {
            ...
            if (pDrvCtrl->txStall)
            {
                ...
                rc = netJobAdd((FUNCPTR)muxTxRestart, (int)&pDrvCtrl->endObj, 0,0,0,0);
                if (rc != OK)
                    pDrvCtrl->debugStats.netJobAddError++;
...
LOCAL void mgiTxDoneJob(DRV_CTRL *pDrvCtrl, MV_U32 cause)
{
    ...
    MGI_DEBUG_CODE(MGI_DEBUG_FLAG_STATS, (pDrvCtrl->debugStats.txDoneJobCount++) );
...
MV_STATUS mgiRxRefill(DRV_CTRL *pDrvCtrl, int queue, int num)
{
    ...
    while(count < num)
    {
        if ((pBufferPtr = (char*)netClusterGet(pDrvCtrl->endObj.pNetPool,
...
static M_BLK_ID     mgiGenerateMblk(DRV_CTRL *pDrvCtrl, MV_PKT_INFO* pPktInfo, int rxQueue)
{
    ...
    if( !netClBlkJoin( pClBlk, pPktInfo->pFrags->bufVirtPtr, CL_BUFF_SIZE,
    ...
    switch (cmd)
    {
        ...
        case EIOCSADDR:
            ...
            else
            {
                ...
                bcopy ((MV_U8*) data,
                       (MV_U8*) MGI_HADDR (&pDrvCtrl->endObj),
        ...
        case EIOCGPOLLSTATS:
            MGI_DEBUG_PRINT(MGI_DEBUG_FLAG_IOCTL,
                            ("Ioctl: ifId=0x%x, cmd=EIOCGPOLLSTATS(0x%x), data=0x%x\n",
                              pDrvCtrl->unit, cmd, (int)data) );

            if( (data == NULL) || (mgiStatsPolling == MV_FALSE) )
                error = EINVAL;
            else
                {
        ...
        ((newFlags & IFF_PROMISC) == 0) )
    {
        ...
        while(pEtherMulti != NULL)
        {
            mvEthMcastAddrSet(pDrvCtrl->pEthPortHndl,
                            &pEtherMulti->addr[0], pDrvCtrl->rxMcastQ);
    ...
    if (retVal == ENETRESET)
    {
        ...
        else
        {
            ...
            if( END_FLAGS_ISSET(IFF_PROMISC) == MV_FALSE)
                mvEthMcastAddrSet(pDrvCtrl->pEthPortHndl, pAddr, pDrvCtrl->rxMcastQ);
        }
        retVal = OK;
    }
...
LOCAL STATUS mgiMCastAddrDel(DRV_CTRL *pDrvCtrl, char *pAddr)
{
    ...
    if (retVal == ENETRESET)
    {
        ...
        /* If Promiscous mode don't access Hardware */
        if( END_FLAGS_ISSET(IFF_PROMISC) == MV_FALSE)
            mvEthMcastAddrSet(pDrvCtrl->pEthPortHndl, pAddr, -1);
...
LOCAL char* macFindInBootline(char *macName)
{
    ...
    subStr = strstr(BOOT_LINE_ADRS, macName);
    if (subStr == NULL)
        return NULL;
    else
...
void    mgiPrintMcastTable(int port)
{
    ...
    while(pEtherMulti != NULL)
    {
        mvMacHexToStr(&pEtherMulti->addr[0], macStr);
...
static void repairRxDescRing(ETH_PORT_CTRL* pPortCtrl, int queue)
{
  ...
  pRxDescBase = (ETH_RX_DESC*)MV_ALIGN_UP((MV_ULONG)
  ...
  for (ix=0; ix<rxDescNum; ix++)
  {
    ...
    if(ix == (rxDescNum-1))
    {
      ...
      pRxPrevDesc->nextDescPtr = (MV_U32)ethDescVirtToPhy(pQueueCtrl, 
                                                          (void*)pRxDescBase);
    }
    else
    {
...
{
  /* 
     Verification tool to simulate RxError condition:
     Corrupt first "next descriptor" field of every rx queue. gc
  */
   ...
   pDrvCtrl = mgiDrvCtrl[0];
   pPortCtrl = (ETH_PORT_CTRL*)(pDrvCtrl->pEthPortHndl);
   
   for (q = 0; q < MV_ETH_RX_Q_NUM; q++)
   {
     pQueueCtrl = &(pPortCtrl->rxQueue[q]);
     pRxCurrDesc = pQueueCtrl->pCurrentDescr;
     pRxCurrDesc->nextDescPtr = 0;
     ETH_DESCR_FLUSH_INV(pPortCtrl, pRxCurrDesc);
   }
}
...
void    mgiPortStatus(int port)
{
...
#ifdef MGI_DEBUG
    {
        ...
        mvOsPrintf("\n");
        mvOsPrintf("RxReadyJob=%u, RxDone=%u\n",
                    pDrvCtrl->debugStats.rxReadyJobCount, pDrvCtrl->debugStats.rxDoneCount);
        ...
        mvOsPrintf("\n");
        mvOsPrintf("TxPkts=%u, TxMaxPkts=%u, TxMaxBuf=%d, TxDoneJob=%u\n",
                    pDrvCtrl->debugStats.txCount, pDrvCtrl->debugStats.txMaxPktsCount,
                    pDrvCtrl->debugStats.txMaxBufCount, pDrvCtrl->debugStats.txDoneJobCount);
