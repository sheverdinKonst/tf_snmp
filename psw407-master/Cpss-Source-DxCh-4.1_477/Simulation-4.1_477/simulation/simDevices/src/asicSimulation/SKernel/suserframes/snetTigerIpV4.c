...
static void snetTigerIpv4InFilter
(
    IN    SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN    SKERNEL_FRAME_DESCR_STC * descrPtr
);

static void snetTigerIpv4Stage2
(
    IN    SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN    SKERNEL_FRAME_DESCR_STC * descrPtr
);

static void snetTigerIpv4UcastStage2
(
    IN    SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN    SKERNEL_FRAME_DESCR_STC * descrPtr
);

static void snetTigerIpv4DefaultIpm
(
    IN    SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN    SKERNEL_FRAME_DESCR_STC * descrPtr
);

static void snetTigerIpv4Lpm
(
    IN    SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN    SKERNEL_FRAME_DESCR_STC * descrPtr
);

static GT_U32 snetTigerIpv4Lpm4IpmInit
(
    IN    SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN    SKERNEL_FRAME_DESCR_STC * descrPtr
);

static GT_U32 snetTigerIpv4NextPointGet
(
    IN GT_U32 bitVector,
...
static GT_U32 snetTigerLpmNextSearch
(
    IN    SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN    SKERNEL_FRAME_DESCR_STC * descrPtr,
...
static void snetTigerIpv4Egress
(
    IN    SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN    SKERNEL_FRAME_DESCR_STC * descrPtr
);

static GT_BOOL snetTigerIpv4Counters
(
    IN    SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN    SKERNEL_FRAME_DESCR_STC * descrPtr
);

static void snetTigerIpMcGroupGet
(
    IN    SKERNEL_DEVICE_OBJECT * devObjPtr,
...
static GT_U8 snetTigerLpmNextPtrGet
(
    IN    SKERNEL_DEVICE_OBJECT * devObjPtr,
...
static GT_VOID snetTigerUcRoutEntryGet
(
    IN    SKERNEL_DEVICE_OBJECT * devObjPtr,
...
static GT_VOID snetTigerMcRoutEntryGet
(
    IN    SKERNEL_DEVICE_OBJECT * devObjPtr,
...
)
{
    ...
    snetTigerIpv4InFilter(devObjPtr, descrPtr);
    snetTigerIpv4Stage2(devObjPtr, descrPtr);
    snetTigerIpv4Lpm(devObjPtr, descrPtr);
    snetTigerIpv4Egress(devObjPtr, descrPtr);

    return snetTigerIpv4Counters (devObjPtr, descrPtr);
}

/*******************************************************************************
*   snetTigerIpv4InFilter
*
* DESCRIPTION:
...
{
    DECLARE_FUNC_NAME(snetTigerIpv4InFilter);
    ...
    /* DSCP to CoS Table */
    __LOG(("DSCP to CoS Table"));
    regAddress = 0x02CA0000 + (descrPtr->dscp * 4);

    smemRegFldGet(devObjPtr, regAddress, 8, 3, &fldValue);
    ...
    smemRegFldGet(devObjPtr, regAddress, 11, 3, &fldValue);
...
{
    DECLARE_FUNC_NAME(snetTigerIpv4Stage2);
    ...
    {
        snetTigerIpv4UcastStage2(devObjPtr, descrPtr);
        return;
    }
    ...
    snetTigerIpMcGroupGet(devObjPtr, nextLookupAddr, 1, &groupEntry);
    ...
    if (invalid != 1)
    {
        ...
        __LOG(("Second level is accessed DIP[19:10]+pointer from the first level"));
        dipFld = (descrPtr->ipv4DIP >> 10) & 0x3FF;
        nextLookupAddr = groupEntry.level1_2.nextMcPtr + dipFld -
                        (groupEntry.level1_2.minimumPtr << 6);
        /* Multicast group entry */
        snetTigerIpMcGroupGet(devObjPtr, nextLookupAddr, 2, &groupEntry);
        ...
        if (invalid != 1)
        {
            ...
            __LOG(("Third level is accessed DIP[9:0]+pointer form the second level."));
            dipFld = descrPtr->ipv4DIP & 0x3FF;
            nextLookupAddr = groupEntry.level1_2.nextMcPtr + dipFld -
                            (groupEntry.level1_2.minimumPtr << 6);

            /* Multicast group entry */
            snetTigerIpMcGroupGet(devObjPtr, nextLookupAddr, 3, &groupEntry);
    ...
    if (invalid == 1 || groupEntry.level3.regular_bkt.nextMcPtr == 0)
    {
        ...
        snetTigerIpv4DefaultIpm(devObjPtr, descrPtr);
        return;
    }
    ...
    if (groupEntry.level3.next_hope.mcGroupType == IPV4_LPM_REGULAR_BUCKET_E)
    {
        ...
        __LOG(("Direct pointer to a Route Entry"));
...
{
    DECLARE_FUNC_NAME(snetTigerIpv4Lpm);
    ...
    {
        nextPointerAddr = snetTigerIpv4Lpm4IpmInit(devObjPtr, descrPtr);
    }
    /* First LPM Prefix Entry Get */
    __LOG(("First LPM Prefix Entry Get"));
    bucketType =
        snetTigerLpmNextPtrGet(devObjPtr, nextPointerAddr, &lpmNextEntry);
    ...
    while (bucketType != IPV4_LPM_NEXT_HOP_BUCKET_E)
    {
        if (++searchLevel < 4)
        {
            ...
            nextPointerAddr = snetTigerLpmNextSearch(devObjPtr, descrPtr,
                    lpmNextEntry, (GT_U8)ipOffset, bucketType);
            /* Next LPM Prefix Entry Get */
            bucketType = snetTigerLpmNextPtrGet(devObjPtr, nextPointerAddr,
                    &lpmNextEntry);
        }
        else
        {
...
)
{
    ...
    bitVector = smemTigerNsramRead(devObjPtr, TIGER_NSRAM_E, bitVectorAddr);

    nextBucketAddr = snetTigerIpv4NextPointGet(bitVector, bitVectorAddr,
...
)
{
    ...
    nextPointerAddr = snetTigerIpv4NextPointGet(bitVector, bitVectorAddr,
                                                bucketType, ipOctet,
                                                bitVectorLine);
...
{
    DECLARE_FUNC_NAME(snetTigerIpv4Egress);
    ...
    /* need to take the value from moduleCfgPtr->ipCfg.routeEntriesMemBase*/
    routEntryAddr = 0x0c000000
         + ((descrPtr->nextHopPtr + routEntryIndex) << 4);
    /* Get rout entry pointer word 0 */
    __LOG(("Get rout entry pointer word 0"));
    routEntryPtr = (GT_U32 *)smemMemGet(devObjPtr, routEntryAddr);
    /* Cmd */
    routEntryCmd = (GT_U8)SMEM_U32_GET_FIELD(routEntryPtr[0], 0, 2);
    ...
    if (routEntryCmd == IPV4_CMD_ROUT_E)
    {
        descrPtr->ttlDecrement =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 14, 1);
    }

    /* Management Counter set to update when packet hits this Route Entry */
    descrPtr->ipv4CounterSet =
        (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 12, 2);

    routForceCos =
        (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 3, 1);
    /* COS of the packet is taken from the route entry */
    if (routForceCos)
    {
        descrPtr->trafficClass =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 4, 3);
        descrPtr->dropPrecedence =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 7, 2);
        descrPtr->userPriorityTag =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 9, 2);
    }
    ...
    if (descrPtr->doIpmRout == 0)
    {
        snetTigerUcRoutEntryGet(devObjPtr, routEntryAddr, &ucRoutEntry);
        if (age == 1)
        {
            ucRoutEntry.age = 1;
        }
    ...
    {
        snetTigerMcRoutEntryGet(devObjPtr, routEntryAddr, &mcRoutEntry);
        if (age == 1)
        {
            ucRoutEntry.age = 1;
        }
        ...
        {
            /* this bit relevant for tigerd only */
...
{
    DECLARE_FUNC_NAME(snetTigerIpMcGroupGet);

    GT_U32 mcGrpEntryPtr;
    GT_U8 mcGroupType;

    /* Fetch Multicast group Entry */
    __LOG(("Fetch Multicast group Entry"));
    mcGrpEntryPtr = smemTigerNsramRead(devObjPtr, TIGER_NSRAM_E, nextLookupAddr);
...
{
    DECLARE_FUNC_NAME(snetTigerLpmNextPtrGet);

    GT_U32 lpmNextEntry;
    GT_U8 bucketType;

    /* Fetch LPM next pointer entry */
    __LOG(("Fetch LPM next pointer entry"));
    lpmNextEntry = smemTigerNsramRead(devObjPtr, TIGER_NSRAM_E, nextPtrAddr);
...
)
{
    ...
    ucRoutePtr->cmd =
        (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 0, 2);
    ucRoutePtr->forceCos =
        (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 3, 1);
    if (ucRoutePtr->forceCos)
    {
        ...
        ucRoutePtr->dp =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 7, 2);
        ucRoutePtr->vpt =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 9, 3);
    }
    ...
    ucRoutePtr->enableDecTtl =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 14, 1);
    ucRoutePtr->age =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 16, 1);
    ucRoutePtr->mtu =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 31, 1) |
    /* Word 1 */
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[1], 0, 13) << 1;

    ucRoutePtr->ucNextHopEntry.outLifType =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[1], 13, 1);
    if (ucRoutePtr->ucNextHopEntry.outLifType == 0)
    {
        ucRoutePtr->ucNextHopEntry.out_lif.link_layer.LL_OutLIF =
            (GT_U32) SMEM_U32_GET_FIELD(routEntryPtr[1], 14, 18) |
    /* Word 2 */
            (GT_U32) SMEM_U32_GET_FIELD(routEntryPtr[2], 0, 12) << 18;
        ucRoutePtr->ucNextHopEntry.out_lif.link_layer.arpPtr =
            (GT_U32) SMEM_U32_GET_FIELD(routEntryPtr[2], 12, 14);
    }
    else
    {
        ucRoutePtr->ucNextHopEntry.out_lif.tunnel_outlif.tunnelOutLif =
            (GT_U32) SMEM_U32_GET_FIELD(routEntryPtr[1], 14, 18) |
    /* Word 2 */
            (GT_U32) SMEM_U32_GET_FIELD(routEntryPtr[2], 0, 14) << 18;
    }
    /* Word 2 */
    ucRoutePtr->greTunnelEnd =
        (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[2], 27, 1);
    ucRoutePtr->ipTunnelEnd =
        (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[2], 28, 1);
    ucRoutePtr->tunnelCpyDscp =
        (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[2], 29, 1);
    ucRoutePtr->pushMpls =
        (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[2], 30, 1);

    if (ucRoutePtr->pushMpls == 1)
    {
        ucRoutePtr->mplsInfo.expSet =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[2], 31, 1);
    /* Word 3 */
        ucRoutePtr->mplsInfo.label =
            (GT_U32)SMEM_U32_GET_FIELD(routEntryPtr[3], 0, 1) |
            (GT_U32)SMEM_U32_GET_FIELD(routEntryPtr[3], 1, 19) << 1;
        ucRoutePtr->mplsInfo.exp =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[3], 20, 3);
        ucRoutePtr->mplsInfo.ttl =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[3], 23, 8);
    }
}

/*******************************************************************************
*   snetTigerMcRoutEntryGet
*
* DESCRIPTION:
...
{
    DECLARE_FUNC_NAME(snetTigerMcRoutEntryGet);
    ...
    mcRoutePtr->enableDecTtl =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 14, 1);
    mcRoutePtr->l2Ce =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 15, 1);
    mcRoutePtr->age =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 16, 1);
    mcRoutePtr->mtu =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[0], 31, 1) |
    /* Word 1 */
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[1], 0, 13) << 1;
    mcRoutePtr->rpfCmd =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[1], 14, 1);
    mcRoutePtr->checkRpf =
            (GT_U8) SMEM_U32_GET_FIELD(routEntryPtr[1], 15, 1);
    mcRoutePtr->rpfInLif =
            (GT_U16)SMEM_U32_GET_FIELD(routEntryPtr[1], 16, 16);
    /* Word 2 */
    mcRoutePtr->mll =
            (GT_U16)SMEM_U32_GET_FIELD(routEntryPtr[2], 0, 16);
    mcRoutePtr->devVidx =
            (GT_U16)SMEM_U32_GET_FIELD(routEntryPtr[2], 16, 14);
