...
*
* DESCRIPTION:
*      This is a external API definition for HA unit (part of egress processing)
*
* DEPENDENCIES:
*       None.
*
...
/*
 * Typedef: enum SNET_CHT_HA_MODIFY_COMMAND_ENT
 *
 * Description: sip5 transit tunnels parsing mode
 *
 * Enumerations:
 *
 *     SNET_CHT_HA_MODIFY_COMMAND_MODIFY_SIP_PREFIX_E     - Replace the SIP prefix with the NAT<Address>.
 *     SNET_CHT_HA_MODIFY_COMMAND_MODIFY_DIP_PREFIX_E     - Replace the DIP prefix with the NAT<Address>
 *     SNET_CHT_HA_MODIFY_COMMAND_MODIFY_SIP_ADDRESS_E    - Replace the SIP address with NAT<Address>
 *     SNET_CHT_HA_MODIFY_COMMAND_MODIFY_DIP_ADDRESS_E    - Replace the DIP address with NAT<Address>
 */
typedef enum{
...
#define SMEM_LION3_HA_NAT44_TABLE_FIELDS_NAME                         \
    ...
    ,STR(SMEM_LION3_HA_NAT44_TABLE_FIELDS_RESERVED                 )\
...
/*SMEM_LION3_HA_NAT44_TABLE_FIELDS_NEW_TCP_UDP_SRC_PORT     */
    STANDARD_FIELD_MAC(16),
/*SMEM_LION3_HA_NAT44_TABLE_FIELDS_RESERVED         */
...
/*******************************************************************************
* snetChtHaEgressEPortEntryGet
*
* DESCRIPTION:
*       function of the HA unit
...
/*******************************************************************************
* snetChtHaEgressPhyPortEntryGet
*
* DESCRIPTION:
*       function of the HA unit
...
*        devObjPtr - pointer to device object.
*        descrPtr  - Cht frame descriptor
*        egressPort - physical egress port -- global port
*                    (referred as <LocalDevTrgPhyPort> in documentation ,
*                     but it is not part of the 'descriptor' !)
*
* OUTPUTS:
...
/*******************************************************************************
*   haExtendedTagIsReturnToSrcInterface
*
* DESCRIPTION:
*       HA - part of build BPE tag (6/8) bytes
...
/*******************************************************************************
*   haExtendedTagGetPushVidOfSrcEport
*
* DESCRIPTION:
*       HA - part of build BPE tag (6/8) bytes
...
/*******************************************************************************
*   snetChtHaEgressBuild6BytesTag
*
* DESCRIPTION:
*       HA - build 6 Bytes v-Tag
...
*                    (referred as <LocalDevTrgPhyPort> in documentation ,
*                     but it is not part of the 'descriptor' !)
*       haInfoPtr->evbBpeTagPtr     - pointer to EVB/BPE tags
*        etherType - etherType
* OUTPUTS:
*       haInfoPtr->evbBpeTagPtr     - pointer to EVB/BPE tags , filled with info
*       haInfoPtr->evbBpeTagLength  - EVB/BPE tags length (0/4/8/6)
*       haInfoPtr->evbBpeIsPassenger   - is EVB on passenger in case of TS.
...
)
{
    ...
    if(descrPtr->useVidx)
    {
        __LOG(("6-bytes-Tag : Build 6-bytes-Tag (6 bytes) for multi destination \n"));
    }
    else
    {
        __LOG(("6-bytes-Tag : Build 6-bytes-Tag (6 bytes) for single destination \n"));
    }

    pBit = descrPtr->useVidx;
    __LOG_PARAM_WITH_NAME("6-bytes-Tag : pBit from :", descrPtr->useVidx);
...
/*******************************************************************************
*   snetChtHaEgressEPortEvbBpeTag
*
* DESCRIPTION:
*       HA - check if need to build EVB extra tag
...
)
{
    ...
    */
    ...
    snetXCatHaEgressTagEtherTypeByTpid(devObjPtr,descrPtr,tpId,&etherType,&tagExtendedSize);
...
/*******************************************************************************
*   snetChtHaRouteMacSaModeGet
*
* DESCRIPTION:
*        HA - get the route MAC SA mode
* INPUTS:
*        devObjPtr          - pointer to device object.
*        descrPtr           - pointer to the frame's descriptor.
...
*
* OUTPUTS:
*        routeMacSaModePtr  - pointer to the route MAC SA mode
*
*
* COMMENTS:
*
*******************************************************************************/
static void snetChtHaRouteMacSaModeGet
(
    IN  SKERNEL_DEVICE_OBJECT           *devObjPtr,
    IN  SKERNEL_FRAME_CHEETAH_DESCR_STC *descrPtr,
    IN  GT_U32                          egressPort,
...
/*******************************************************************************
*   snetChtHaCreateAnalyzerVlanTag
*
* DESCRIPTION:
*       HA - Create remote analyzer VLAN tag added to packet forwarded to
...
*
* OUTPUTS:
*       vlanTagPtr      - pointer to remote analyzer VLAN tag
*       analyzerVlanTagLengthPtr  - pointer to analyzer VLAN tag length
*
* RETURNS:
*
* COMMENTS:
*       Only network ports can be set to add the remote analyzer VLAN tag.
...
/*******************************************************************************
*   snetLion3HaCreateTimestampTag
*
* DESCRIPTION:
*       Create (if needed) the egress Timestamp Tag.
*
* INPUTS:
*       devObjPtr       -  pointer to device object.
*       descrPtr        - pointer to the frame's descriptor.
*
* OUTPUTS:
*       timestampTagPtr         - pointer to TST
*       timestampTagLengthPtr   - pointer to TST length
*
* RETURNS:
*
* COMMENTS:
...
)
{
    ...
    regAddr = SMEM_LION2_HA_PHYSICAL_PORT_2_ATTRIBUTES_TBL_MEM(
        devObjPtr, descrPtr->localDevSrcPort);
    regPtr = smemMemGet(devObjPtr, regAddr);
    fieldData = snetFieldValueGet(regPtr, 39, 1);
...
/*******************************************************************************
*   snetChtHaIsMtagPacketChanged
*
* DESCRIPTION:
*        HA - Check if the DSA tagged packet is changed by the ingress pipeline
* INPUTS:
*        descrPtr        - pointer to the frame's descriptor.
*
* OUTPUTS:
*
* RETURNS: GT_TRUE - if packet was changed
*
* COMMENTS:
*
*******************************************************************************/
...
/*******************************************************************************
* snetChtHaIsPacketChanged
*
* DESCRIPTION:
*        HA - Check if the packet is changed by the ingress pipeline
* INPUTS:
*        descrPtr        - pointer to the frame's descriptor.
*        destVlanTagged  - support tr101 tagging modes
*        vlanEtherType   - egress VLAN EtherType for Tag0
*        vlanEtherType1  - egress VLAN EtherType for Tag1
*
* OUTPUTS:
*
* RETURNS: GT_TRUE - if packet is changed
*
* COMMENTS:
*
*******************************************************************************/
...
/*******************************************************************************
*   snetLion3HaDSATrgInterfaceRemoteFieldsReAssignment
*
* DESCRIPTION:
*        The 8B/16B DSA tag target interface fields assignment
...
* OUTPUTS:
*        descrPtr    - pointer to the frame's descriptor.
*        dsaFieldsPtr- pointer to dsa targed field values
*
* RETURNS:
*       GT_BOOL      - values were remapped (GT_TRUE), or not (GT_FALSE)
*
* COMMENTS:
*
*       If (<Remote Physical Port Map Enable>==1) AND (Egress DSA tag is 16B eDSA)
*       {
...
/*******************************************************************************
*   convertDsaTagWordsToPacketBytes
*
* DESCRIPTION:
*      convert DSA tag format in little endian (in words) and convert to stream
*      of bytes in network order
* INPUTS:
*        devObjPtr       -  pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
*        dsaType         - dsa tag type
*        mtag[]          - 4 words for the build of the DSA tag
* OUTPUTS:
...
/*******************************************************************************
*   snetChtHaCreate4WordsMarvellToCpu
*
* DESCRIPTION:
*      HA - Create 4 words DSA tag with TO_CPU command
* INPUTS:
*        devObjPtr       -  pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
*        dsaType         - dsa tag type
*        mtag[]          - 4 words for the build of the DSA tag
*        srcDevTrgDev    - src dev / trg dev
*        srcPortTrgPort  - src port / trg port
*        vidToUse        - the vid to use in the DSA tag
* OUTPUTS:
*        mtag[]          - 4 words for updated
* RETURNS:
*
* COMMENTS:
*
...
/*******************************************************************************
*   snetChtHaCreateMarvellTagToCpuToAnalyzerVidCalx
*
* DESCRIPTION:
*        HA - calc TO_CPU/TO_ANALYZER vid
* INPUTS:
*        devObjPtr       -  pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
*        mtag[]          - (pointer to)4 words for the build of the DSA tag
*        useOrigVlanTagInfo - do we use the original vlan tag info
* OUTPUTS:
*        mtag[]          - (pointer to)4 words for the build of the DSA tag
*        vidToUsePtr     - (pointer to)the vid to use in the dsa
* RETURNS:
*
* COMMENTS:
*
*******************************************************************************/
static void snetChtHaCreateMarvellTagToCpuToAnalyzerVidCalx
(
    IN  SKERNEL_DEVICE_OBJECT           *devObjPtr,
    IN  SKERNEL_FRAME_CHEETAH_DESCR_STC *descrPtr,
    INOUT GT_U32                         mtag[/*4*/],
...
{
    DECLARE_FUNC_NAME(snetChtHaCreateMarvellTagToCpuToAnalyzerVidCalx);
    ...
    if(useOrigVlanTagInfo == GT_FALSE)
    {
        ...
        {
            __LOG_PARAM_WITH_NAME("use vid from 'current evid'",descrPtr->eVid);
        ...
        else  /* this part of fix bugs# 91490 , 91072 */
        {
            ...
            {
                __LOG_PARAM_WITH_NAME("use vid from 'ingress pipe'",descrPtr->ingressPipeVid);
                vidToUse = descrPtr->ingressPipeVid;
            }
        }
    }
    else
...
/*******************************************************************************
*   snetChtHaCreateMarvellTagToCpu
*
* DESCRIPTION:
*        HA - Create DSA tag with TO_CPU command
* INPUTS:
*        devObjPtr       -  pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
*        egressPort      - egress port
...
)
{
    ...
    snetChtHaCreateMarvellTagToCpuToAnalyzerVidCalx(devObjPtr,descrPtr,mtag,
    ...
    if (descrPtr->egrMarvellTagType >= MTAG_EXTENDED_E )
    {
        ...
        else
        {
            ...
            if ((descrPtr->marvellTagged) &&
                (descrPtr->incomingMtagCmd == SKERNEL_MTAG_CMD_TO_CPU_E))
            {
                __LOG(("FEr#89: pktOrigBC is packet's byte count but not ingress DSA pktOrigBC"));
            }
...
/*******************************************************************************
*   snetChtHaEDsaTpidGet_from_cpu
*
* DESCRIPTION:
*      HA - build of the EDSA FROM_CPU - get the TPID that is needed.
* INPUTS:
*        devObjPtr       - pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
*        egressPort      - egress port
* OUTPUTS:
*        tpidIndexPtr - pointer to the TPID index
* RETURNS:
*
* COMMENTS:
*
...
/*******************************************************************************
*   snetChtHaCreate4WordsMarvellFromCpu
*
* DESCRIPTION:
*      HA - Create 4 words DSA tag with FROM_CPU command
* INPUTS:
*        devObjPtr       - pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
*        mtag[]          - 4 words for the build of the DSA tag
...
)
{
    ...
    /*eVLAN[15:12]*/
    SMEM_U32_SET_FIELD(mtag[3], 27 , 4,
        SMEM_U32_GET_FIELD(descrPtr->eVid,12,4));
...
/*******************************************************************************
*   snetChtHaCreateMarvellTagFromCpu
*
* DESCRIPTION:
*      HA - Create DSA tag with FROM_CPU command
* INPUTS:
*      devObjPtr       - pointer to device object.
*      descrPtr        - pointer to the frame's descriptor.
*
...
)
{
    ...
    SMEM_U32_SET_FIELD(mtag[0], 29, 1, descrPtr->trgTagged);
    ...
    SMEM_U32_SET_FIELD(mtag[0] , 0, 12, descrPtr->eVid);/* bit [0:11] */
    ...
    if (descrPtr->egrMarvellTagType >= MTAG_EXTENDED_E)
    {
        ...
        SMEM_U32_SET_FIELD(mtag[1], 28, 1, SMEM_U32_GET_FIELD(descrPtr->tc,2,1));/* bit [27] &0x4 */
        if (SKERNEL_IS_CHEETAH3_DEV(devObjPtr))
        {
    ...
    if (descrPtr->egrMarvellTagType == MTAG_4_WORDS_E)
    {
        snetChtHaCreate4WordsMarvellFromCpu(devObjPtr, descrPtr, mtag);
    }
...
/*******************************************************************************
*   snetChtHaCreate4WordsMarvellToAnalyzer
*
* DESCRIPTION:
*      HA - Create 4 words DSA tag with TO_ANALYZER DSA tag
* INPUTS:
*        devObjPtr       -  pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
*        mtag[]          - 4 words for the build of the DSA tag
...
/*******************************************************************************
*   snetChtHaCreateMarvellTagToAnalyzer
*
* DESCRIPTION:
*      HA - Create DSA tag with TO_ANALYZER command
* INPUTS:
*        devObjPtr       - pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
*
...
)
{
    ...
    if ( descrPtr->rxSniff )
    {
        __LOG_PARAM_WITH_NAME("bit[29] SrcTagged/TrgTagged" , descrPtr->tagSrcTagged[SNET_CHT_TAG_0_INDEX_CNS]);

        SMEM_U32_SET_FIELD(mtag[0], 29, 1, descrPtr->tagSrcTagged[SNET_CHT_TAG_0_INDEX_CNS]);/* bit [29]&0x1*/
        srcDevTrgDev = descrPtr->srcDev;
        srcPortTrgPort = descrPtr->localDevSrcPort;
    }
    else
    {
        __LOG_PARAM_WITH_NAME("bit[29] SrcTagged/TrgTagged" , descrPtr->trgTagged);
...
/*******************************************************************************
*   lion3HaEgressVlanTagsForDsaInfoGet
*
* DESCRIPTION:
*       (sip5) get vlan tag 0,1 info for DSA build -
...
*
* OUTPUTS:
*       tag0ExistPtr - pointer to is tag 0 exists (in final egress packet)
*       tag1ExistPtr - pointer to is tag 1 exists (in final egress packet)
*       outerVlanIsTag1Ptr - pointer to indication that tag 1 it the outer tag(in final egress packet)
*
* RETURN:
*       None
*
...
/*******************************************************************************
*   lion3HaFinalDsaUpdate_farward
*
* DESCRIPTION:
*      HA - update FORWARD DSA fields with info that was not known during build of the DSA.
* INPUTS:
*        devObjPtr       - pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
*        egressPort      - egress port
...
/*******************************************************************************
*   lion3HaPrepareDsaAndTagsInfo
*
* DESCRIPTION:
*      HA - prepare values needed in HA when build DSA and vlan tags.
* INPUTS:
*        devObjPtr       - pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
* OUTPUTS:
...
/*******************************************************************************
*   snetChtHaCreate4WordsMarvellForward
*
* DESCRIPTION:
*      HA - Create 4 words DSA tag with FORWARD command
* INPUTS:
*        devObjPtr       - pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
*        mtag[]          - 4 words for the build of the DSA tag
...
)
{
    ...
    /**********/
    /*eVLAN[15:12]*/
    __LOG_PARAM_WITH_NAME("eVLAN[15:12]", SMEM_U32_GET_FIELD(descrPtr->eVid,12,4));
    SMEM_U32_SET_FIELD(mtag[3], 27 , 4,
        SMEM_U32_GET_FIELD(descrPtr->eVid,12,4));
    ...
    if(descrPtr->useVidx)
    {
        ...
        {
            __LOG_PARAM_WITH_NAME("origSrcPhyPort (8 bits)", descrPtr->eArchExtInfo.origSrcPhyPortTrunk);
            /* port */
            SMEM_U32_SET_FIELD(mtag[3],7,8,descrPtr->eArchExtInfo.origSrcPhyPortTrunk);
        }
    ...
    {
        /*TRGePort[19:0]*/
        __LOG_PARAM_WITH_NAME("TRGePort[19:0]", descrPtr->trgEPort);
        SMEM_U32_SET_FIELD(mtag[3], 7 , 17, descrPtr->trgEPort);
...
/*******************************************************************************
*   snetChtHaCreateMarvellForward_setCfiVptVid
*
* DESCRIPTION:
...
)
{
    ...
    /* set cfi, vpt, vid */
    snetChtHaCreateMarvellForward_setCfiVptVid(devObjPtr, descrPtr, haInfoPtr, mtag);
...
/*******************************************************************************
* snetChtHaVlanTranslate
*
* DESCRIPTION:
*        HA - Egress VLAN Translation logic.
*        supported from ch3
*
* INPUTS:
*       devObjPtr    - pointer to device object.
...
)
{
    ...
    else
    {
        ...
        if(regValue)
        {
            __LOG(("Enable Egress VLAN Translation \n"));
            ...
            if (descrPtr->outGoingMtagCmd == SKERNEL_MTAG_CMD_TO_TRG_SNIFFER_E &&
                descrPtr->rxSniff == 0 &&   /* egress mirrored packet */
                (SKERNEL_IS_MATCH_DEVICES_MAC(descrPtr->trgDev, descrPtr->ownDev,
                                                          devObjPtr->dualDeviceIdEnable.ha)))
            {
                translateVid = GT_TRUE;
        ...
        if(SMEM_CHT_IS_SIP5_10_GET(devObjPtr))
        {
            ...
            */
                                   ...
                                   Assign the 12 LS-bits of the eVLAN<Service-ID> to VID1. */
            {
                ...
                __LOG(("TRANSLATION of VID and VID1 according to ServiceID [0x%6.6x] (using 12 MSBits[0x%3.3x]) to VID[%4.4x] , VID1[%3.3x]\n",
                    ServiceID,/*(24 bits)*/
                    (ServiceID >> 12),/*12 MSbits*/
                    descrPtr->eVid,
                    descrPtr->vid1));
    ...
    {
        __LOG(("no VID translation \n"));
    }

}

/*******************************************************************************
*   snetChtHaDsaForwardToFromCpuManipulation
*
* DESCRIPTION:
*       HA - It is required to support a mode per physical port, where a unicast FWD DSA packet is turned into a
...
*        dsaOrVlanTag - DSA TAG 8 words -- the DSA tag may require 4 words
*
* OUTPUTS:
*        dsaOrVlanTag - updated DSA TAG 8 words -- the DSA tag may require 4 words
* RETURNS:
*
* COMMENTS:
*
...
/*******************************************************************************
*   snetChtHaUpdateIPTotalLength
*
* DESCRIPTION:
*        updates ip total length packet field,
...
*
* COMMENTS:
*      function snetChtHaTsArpPointerInfo(...) could not know if passenger
...
*        descrPtr        - pointer to the frame's descriptor.
*        egressPort      - the local egress port (not global port)
*        haInfoPtr       - pointer to internal ha info
* OUTPUTS:
*        mrvlTagPtr      - pointer to DSA tag
*
* RETURNS:
...
/*******************************************************************************
*   snetLion3HaOamChannelTypeProfile
*
* DESCRIPTION:
*       HA OAM Channel Type Profile support
*
* INPUTS:
*       devObjPtr           - pointer to device object.
...
/*******************************************************************************
*   haBuildPacketNonChanged
*
* DESCRIPTION:
*        build packet that is not changed by the HA
*
* INPUTS:
*        devObjPtr       -  pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
*        egressBufferPtr - the egress buffer
* OUTPUTS:
...
/*******************************************************************************
*   haBuildPacketWithOrigInfoFromIngressToCpuOrFromIngressToAnalyzer
*
* DESCRIPTION:
*        build packet that is not changed by the HA
...
)
{
    ...
    GT_U8    extractFromDSADueToForceNewDsaToCpuTag[8];
    GT_U32  ingressL2Len;
    ...
    if(descrPtr->tag0Ptr)
    {
        if(extractFromDSADueToForceNewDsaToCpuTagLength && tag0Outer)
        {
            /* the tag0 will jump over the DSA */
            descrPtr->tag0Ptr += extractFromDSADueToForceNewDsaToCpuTagLength;
        }
    ...
    if(descrPtr->tag1Ptr)
    {
        if(extractFromDSADueToForceNewDsaToCpuTagLength && tag1Outer)
        {
            /* the tag1 will jump over the DSA */
            descrPtr->tag1Ptr += ingressL2FirstTagLen;
        }
    ...
    payloadLen = descrPtr->frameBuf->actualDataSize - (payloadPtr - descrPtr->origInfoBeforeTunnelTermination.originalL2Ptr);
    ...
    if(descrPtr->egrMarvellTagType != MTAG_TYPE_NONE_E)
    {
        /* If the packet is DSA tagged, then the Timestamp tag is immediately after the DSA tag */
        SET_FORCE_MAC(
            descrPtr->haToEpclInfo.timeStampTagPtr, egressBufferPtr /*+ dsaOrVlanTagLength*/);
    }
    ...
    if((payloadLen + dsaOrVlanTagLength + macDaSaLength +
        extractFromDSADueToForceNewDsaToCpuTagLength +
    ...
    *frameDataPtrPtr = devObjPtr->egressBuffer;
    ...
    if (descrPtr->outGoingMtagCmd == SKERNEL_MTAG_CMD_TO_CPU_E)
    {
        if ( (descrPtr->truncated) && (totalLength > 128) )
        {
            ...
            __LOG(("frame TO_CPU needs to be truncated --> to 128 bytes "));
        }
    }
...
/*******************************************************************************
*   chtHaIpHeaderModifications
*
* DESCRIPTION:
*        HA - do ipv4/6 header modifications (tunnel start or passenger)
*
* INPUTS:
*        devObjPtr       -  pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
...
*        l3HeaderChangedBuffer - pointer to ipv4/6 header
*                           relevant only when doTunnelHeader = GT_FALSE
*        haInfoPtr       - pointer to HA info
*                           relevant only when doTunnelHeader = GT_TRUE
*        l4HeaderChangedBuffer - pointer to TCP/UDP header
*                           relevant only when not NULL and l4HeaderChangedLen != 0
*        l4HeaderChangedLen - number of bytes in l4HeaderChangedBuffer
* OUTPUTS:
*
* RETURNS:
*
...
/*******************************************************************************
*   snetHaEgressVlanTag1Removed
*
* DESCRIPTION:
*       HA - update tag state to support Remove Vlan Tag1 If Zero
*
* INPUTS:
*       devObjPtr   - pointer to device object
*       descrPtr    - pointer to the frame's descriptor.
...
*
* COMMENTS:
*       This feature enables Tag1 removal from the egress port Tag State
...
/*******************************************************************************
*   chtHaCalcTr101FinalState
*
* DESCRIPTION:
*        HA - calcl tr101 final tag state of tags 0,1
*
* INPUTS:
*        devObjPtr       -  pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
*        egressPort      - egress port.
*        destVlanTagged  - pre HA needed tag state
*        passengerDestVlanTagged  - tag states for the passenger (when TS)
*
* OUTPUTS:
*
* RETURNS:
...
/*******************************************************************************
*   chtHaTargetAnalyzerTagModeGet
*
* DESCRIPTION:
*        HA - to sniffer analyzer tags info
*
* INPUTS:
*        devObjPtr       -  pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
...
/*******************************************************************************
*   snetBc2B0HaNatIpv4CheckException
*
* DESCRIPTION:
*       NAT: ipv4 check exception
*
...
*
* RETURN:
*       isNatException - whether NAT exception happened
*
*******************************************************************************/
...
/*******************************************************************************
*   snetBc2B0HaNatIpv4L4ChecksumPreparePseudoHeader
*
* DESCRIPTION:
*       NAT: Prepare pseudo-header for L4 checksum computation (IPv4)
*
*                     +--------+--------+--------+--------+
*                     |           Source Address          |
*                     +--------+--------+--------+--------+
...
*       devObjPtr   - pointer to device object.
*       descrPtr    - pointer to packet descriptor
*       l3StartOffsetPtr - pointer to start of L3
*       l4StartOffsetPtr - pointer to start of L4
*       pseudoHeaderArr - pointer to pseudo-header data of only + start of L4:
*                       1. sip,dip (8B)
*                       2. 0 (1B)
*                       3. ipProt (1B)
*                       4. length (L4 length) (2B)
*                       ==========12 bytes
*                       5. 16 bytes of L4
*                       6. 2 bytes of checksum
*                       ==========total 12+16+2 = 30B
*                   NOTE: the rest of L4 bytes are NOT part of this buffer.
*                      to avoid copy of long buffers !
*       dataLenPtr  - pointer to len of the data (L4 length)
*
*******************************************************************************/
...
/*******************************************************************************
*   snetBc2B0HaNatIpv4L4ChecksumUpdate
*
* DESCRIPTION:
*       NAT: ipv4 L4 checksum update
*
...
*       devObjPtr           - pointer to device object.
*       descrPtr            - pointer to packet descriptor
*       ingPseudoHeaderArr  - pointer to old header data
*       egrPseudoHeaderArr  - len old header data
*       checksumBuffPtr     - pointer to buffer
*
* OUTPUTS:
*       descrPtr    - pointer to packet descriptor
*
...
*
* DESCRIPTION:
*    Calculate incremental 16bit TCP/UDP checksum based on original header and new header.
*
* INPUTS:
*     origPrefixPtr - (pointer to) original prefix
*     newPrefixPtr  - (pointer to) new prefix
*     origChecksum  - original checksum
*
* OUTPUTS:
...
* RETURN:
*
*     Reference: tools.ietf.org/html/rfc1624
*         HC' = ~(C + (-m) + m')
*             = ~(~HC + ~m + m')
*
...
/*******************************************************************************
*   haNatCheckEntryValidity
*
* DESCRIPTION:
*       NAT: ipv4/6 check NAT entry type validity.
*
...
*       devObjPtr   - pointer to device object.
*       descrPtr    - pointer to packet descriptor
*       natEntryType  - the nat entry type
*
* OUTPUTS:
*       None
*
...
/*******************************************************************************
*   haProcessNat44L3L4HeaderModify
*
* DESCRIPTION:
*        HA - NAT : do ipv4 and L4 header modifications.
*
* INPUTS:
*        devObjPtr       -  pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
...
/*******************************************************************************
*   snetBobKHaNatIpv6L3ChecksumUpdate
*
* DESCRIPTION:
*       NAT: ipv6 L3 checksum update
*
...
*       devObjPtr   - pointer to device object.
*       descrPtr    - pointer to packet descriptor
*       tunnelStartActionPtr - pointer to the NAT entry memory
*       oldAddress  - the old sip/dip address
*
* OUTPUTS:
*       newAddress - the updated address.
*
*******************************************************************************/
...
/*******************************************************************************
*   haProcessNat66L3HeaderModify
*
* DESCRIPTION:
*        HA - NAT : do ipv6 header modifications.
*
* INPUTS:
*        devObjPtr       -  pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
*        l3HeaderChangedBuffer - pointer to ipv6 header
* OUTPUTS:
*
* RETURNS:
*
...
#define SIP5_15_NAT_FIELD_GET_MAC(field)\
    ...
    case SNET_CHT_HA_MODIFY_COMMAND_MODIFY_SIP_PREFIX_E:
        ...
        /* packet SIP applied to buffer here, and not in chtHaUnit */
         snetBobKHaNatIpv6L3ChecksumUpdate(devObjPtr,descrPtr,tunnelStartActionPtr,&descrPtr->l3StartOffsetPtr[IPV6_L3_SIP_OFFSET],&l3HeaderChangedBuffer[IPV6_L3_SIP_OFFSET]);
    ...
    case SNET_CHT_HA_MODIFY_COMMAND_MODIFY_DIP_PREFIX_E:
        ...
        /* packet DIP applied to buffer here, and not in chtHaUnit */
         snetBobKHaNatIpv6L3ChecksumUpdate(devObjPtr,descrPtr,tunnelStartActionPtr,&descrPtr->l3StartOffsetPtr[IPV6_L3_DIP_OFFSET],&l3HeaderChangedBuffer[IPV6_L3_DIP_OFFSET]);
...
)
{
    ...
    if (descrPtr->isMplsLsr)
    {
        ...
        case SKERNEL_XCAT_TTI_MPLS_POP_3_E:      /*new in sip5 */
            offset = (descrPtr->numOfLabels+1) >= 3 ?
                    12 : /*jump 3 labels*/
                    (descrPtr->numOfLabels + 1) * 4 ;/* do not jump more then existing labels */
            break;
    ...
    */
    ...
    if(SKERNEL_IS_XCAT_REVISON_A1_DEV(devObjPtr))
    {
        ...
        snetXCatHaEgressTagEtherType(devObjPtr, descrPtr,egressPort , &vlanEtherType,
    ...
    if (egressPort == SNET_CHT_EMBEDDED_CPU_PORT_CNS &&
        devObjPtr->supportEArch == 0)
    {
        /* treat this interface to the embedded CPU to always be with extended DSA tag */
        descrPtr->egrMarvellTagType = MTAG_EXTENDED_E;
    ...
    if (egressPort == SNET_CHT_CPU_PORT_CNS) /* send packet to cpu */
    {
        ...
        __LOG(("check if SDMA bus is enabled ?"));
    ...
    else
    {
        ...
        if (destVlanTagged)
        {
            ...
            {
                snetChtHaEgressTagDataExtGet((GT_U8)descrPtr->up, (GT_U16)descrPtr->eVid,
       ...
       haInfo.tunnelStart == 0)/* the TS get it's tagging regardless to tr101 of the vlan entry */
    {
        ...
        snetXCatHaEgressTagBuild(devObjPtr, descrPtr, vlanEtherType, vlanEtherType1,
    ...
    {
        tag0OffsetInTag = destVlanTagged ? 0 : SMAIN_NOT_VALID_CNS;/*if tag 0 exists it is first in the tag (no tag1) */
    }
    ...
    if(haInfo.tunnelStart)
    {
        ...
        if(descrPtr->tunnelStartType == SKERNEL_FRAME_TUNNEL_START_TYPE_CAPWAP_E)
        {
            /*build tunnel start CAPWAP header -- in buffer
              set length of TS header*/
            payloadLen = 0;/* pay load is already part of "tunnel header"
                              due to UDP header checksum calculations */
        }
    ...
    if(totalLength < SGT_MIN_FRAME_LEN)
    {
        ...
        __LOG(("Consider Padding of packet length [%d] (shorter then 64 bytes) \n",
            totalLength));
    ...
    if(paddingZeroForLess64BytesLength)
    {
        ...
        SET_FORCE_MAC(descrPtr->haToEpclInfo.paddingZeroForLess64BytesLength,
...
/*******************************************************************************
*   snetlion3HaInterruptEvent
*
* DESCRIPTION:
*       Setting interrupt for HA unit.
*
* INPUTS:
*       devObjPtr       - pointer to device object.
...
/*******************************************************************************
*   snetLion3HaOutgoingTimestampTag
*
* DESCRIPTION:
*        Setting the type of the outgoing timestamp tag.
*
* INPUTS:
*       devObjPtr   - pointer to device object.
*       descrPtr    - frame data buffer Id
...
)
{
    ...
    regAddr = SMEM_LION2_HA_PHYSICAL_PORT_2_ATTRIBUTES_TBL_MEM(devObjPtr, descrPtr->localDevSrcPort);
    regPtr = smemMemGet(devObjPtr, regAddr);
    ...
    regAddr = SMEM_LION2_HA_PHYSICAL_PORT_2_ATTRIBUTES_TBL_MEM(devObjPtr, egressPort);
    regPtr = smemMemGet(devObjPtr, regAddr);

    portTstMode = snetFieldValueGet(regPtr, 35, 3);
...
/*******************************************************************************
*   snetLion3HaPtpExtractionAndAction
*
* DESCRIPTION:
*        PTP Field Extraction and Timestamp Action Resolution
*
* INPUTS:
*       devObjPtr   - pointer to device object.
*       descrPtr    - frame data buffer Id
...
)
{
    ...
    /* PTP Offset profile from Timestamp Tag */
    regAddr = SMEM_LION2_HA_PHYSICAL_PORT_2_ATTRIBUTES_TBL_MEM(devObjPtr,descrPtr->localDevSrcPort);
    regPtr = smemMemGet(devObjPtr, regAddr);

    if( (descrPtr->oamInfo.timeStampEnable == 0) &&
        (snetFieldValueGet(regPtr, 39, 1)) &&
    ...
    /* Timestamp Selection */
    if( snetFieldValueGet(regPtr, 39, 1) &&
        (descrPtr->timestampTagged[SMAIN_DIRECTION_INGRESS_E] !=
          SKERNEL_TIMESTAMP_TAG_TYPE_UNTAGGED_E) )
    {
...
/*******************************************************************************
*   lion3HaInfoForEgressPipe
*
* DESCRIPTION:
*        HA - add info for the EPCL , and other units to use down the egress pipe.
*
* INPUTS:
*        devObjPtr       -  pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
...
/*******************************************************************************
*   lion3HaUnitInitInfoGet
*
* DESCRIPTION:
*        HA - get some init info
*
* INPUTS:
*        devObjPtr       -  pointer to device object.
*        descrPtr        - pointer to the frame's descriptor.
...
)
{
    ...
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr) && (oversizeTags > 20/*5 VLAN tags*/))
    {
        ...
        __LOG(("WARNING : The HA recognized too many vlan tags ([%d] tags --> more then 5) so packet is dropped (will not egress the port) \n",
            (oversizeTags / 4)));
    ...
    if(simLogIsOpen())/* LOG indication about the pointers HA to EPCL */
    {
