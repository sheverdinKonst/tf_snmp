...
#define CHECK_PTR_MA(ptr) if (!ptr) { DEBUG_MSG(SNMP_DEBUG,"can not allocate memory, line: %d\r\n", __LINE__); return ERR_MEMORY_ALLOCATION; }
        ...
        switch (input[*pos]) {
            ...
            case BER_TYPE_SNMP_TRAP:
                ...
                *pos = *pos + 1;
                break;
            default:
            	DEBUG_MSG(SNMP_DEBUG,"unsupported BER type %02X\r\n", input[*pos]);
                return -1;
        }
    } else {
        DEBUG_MSG(SNMP_DEBUG,"unexpected end of the SNMP request (pos=%d, len=%d) [1]\r\n", *pos, len);
        return -1;
    }
    return 0;
}
    ...
    if (*pos < len) {
        ...
        if (!(input[*pos] & 0x80)) {
            ...
            *pos = *pos + 1;
            ...
            if (size_of_length > 2) {
                DEBUG_MSG(SNMP_DEBUG,"unsupported value of the length field occurs (must be up to 2 bytes)\r\n");
                return 1;
            }
            *length = 0;
            while (size_of_length--) {
                if (*pos < len) {
                    ...
                    *pos = *pos + 1;
                    ...
                    DEBUG_MSG(SNMP_DEBUG,"can't fetch length, unexpected end of the SNMP request [2]\r\n");
                    return -1;
                }
            }
        }
    } else {
        DEBUG_MSG(SNMP_DEBUG,"can't fetch length, unexpected end of the SNMP request [3]\r\n");
        return -1;
    }
    return 0;
}
...
s8t ber_decode_sequence(const u8t* const input, const u16t len, u16t* pos)
{
    ...
    if (type != BER_TYPE_SEQUENCE || length != (len - *pos)) {
        DEBUG_MSG(SNMP_DEBUG,"bad type or length value for an expected sequence: type %02X length %d\r\n", type, length);
        return -1;
    }
    return 0;
}
...
s8t ber_decode_sequence_length(const u8t* const input, const u16t len, u16t* pos, u16t* length)
{
    ...
    if (type != BER_TYPE_SEQUENCE) {
        DEBUG_MSG(SNMP_DEBUG,"bad type for an expected sequence: type %d length %d\r\n", type, *length);
        return -1;
    }
    return 0;
}
...
s8t ber_decode_integer(const u8t* const input, const u16t len, u16t* pos, s32t* value)
{
    ...
    if (type != BER_TYPE_INTEGER || length < 1) {
        DEBUG_MSG(SNMP_DEBUG,"bad type or length value for an expected integer: type %02X length %d\r\n", type, length);
        return -1;
    }
    ...
    if (*pos + length - 1 < len) {
        ...
        DEBUG_MSG(SNMP_DEBUG,"can't fetch an integer: unexpected end of the SNMP request\r\n");
        return -1;
    }
    return 0;
}
...
s8t ber_decode_unsigned_integer(const u8t* const input, const u16t len, u16t* pos, u32t* value)
{
    ...
    if (*pos + length - 1 < len) {
        *value = 0;
        ...
        DEBUG_MSG(SNMP_DEBUG,"can't fetch an unsigned integer: unexpected end of the SNMP request\r\n");
        return -1;
    }
    return 0;
}
...
s8t ber_decode_string(u8t* const input, const u16t len, u16t* pos, u8t** value, u16t* field_len)
{
    ...
    if ((type != BER_TYPE_OCTET_STRING)&& (type != BER_TYPE_IPADDRESS)) { // my
        DEBUG_MSG(SNMP_DEBUG,"SNMP string must be of type %02X, byt not %02X\r\n", BER_TYPE_OCTET_STRING, type);
        return -1;
    }
    if (*pos + *field_len - 1 < len) {
        ...
        *pos = *pos + *field_len;
        ...
        DEBUG_MSG(SNMP_DEBUG,"can't fetch an octet string: unexpected end of the SNMP input\r\n");
        return -1;
    }
    return 0;
}
...
s8t ber_decode_oid(u8t* const input, const u16t len, u16t* pos, ptr_t* o)
{
    ...
    if (type != BER_TYPE_OID || length < 1) {
        DEBUG_MSG(SNMP_DEBUG,"bad type or length of the OID: type %02X length %d\r\n", type, length);
        return -1;
    }
    ...
    if (*pos + length <= len) {
        ...
        *pos += length;
        ...
        DEBUG_MSG(SNMP_DEBUG,"can't fetch an oid: unexpected end of the SNMP request\r\n");
        return -1;
    }
    return 0;
}
...
s8t ber_decode_void(const u8t* const input, const u16t len, u16t* pos)
{
    ...
    if ((type == BER_TYPE_NULL && length != 0) || (type != BER_TYPE_NULL && length == 0)) {
        DEBUG_MSG(SNMP_DEBUG,"bad type of length of a void value: type %02X length %d\r\n", type, length);
        return -1;
    }
    if (*pos + length - 1 < len) {
        *pos = *pos + length;
    } else {
        DEBUG_MSG(SNMP_DEBUG,"can't fetch void: unexpected end of the SNMP request\r\n");
        return -1;
    }
    return 0;
}
...
s8t ber_decode_pdu(u8t* const input, const u16t len, u16t* pos, pdu_t* pdu)
{
    ...
    if (length != (len - *pos)) {
        DEBUG_MSG(SNMP_DEBUG,"the length of the PDU should be %d, got %d\r\n", (len - *pos), length);
        return -1;
    }
    ...
    DEBUG_MSG(SNMP_DEBUG,"request id: %lu\r\n", pdu->request_id);

    /* error-state */
    TRY(ber_decode_integer(input, len, pos, &tmp));
    pdu->error_status = (u8t)tmp;
    DEBUG_MSG(SNMP_DEBUG,"error-status: %d\r\n", pdu->error_status);

    /* error-index */
    TRY(ber_decode_integer(input, len, pos, &tmp));
    pdu->error_index = (u8t)tmp;
    DEBUG_MSG(SNMP_DEBUG,"error-index: %d\r\n", pdu->error_index);

    /* variable-bindings */
    pdu->varbind_index = *pos;
    DEBUG_MSG(SNMP_DEBUG,"varbind index %d\r\n", *pos);
