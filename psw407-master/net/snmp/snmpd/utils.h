...
#define CHECK_PTR(ptr) if (!ptr) { DEBUG_MSG(SNMP_DEBUG,"can not allocate memory, line: %d\n", __LINE__); return -1; }
/** \brief checks whether the pointer is 0, if so leaves the function. */
#define CHECK_PTR_U(ptr) if (!ptr) { DEBUG_MSG(SNMP_DEBUG,"can not allocate memory, line: %d\n", __LINE__); return 0; }

/** \brief */
#define TRY(c) if (c < 0) { DEBUG_MSG(SNMP_DEBUG,"exception line: %d\n", __LINE__); return FAILURE; }

#define DECN(pos, value) (*pos) -= value; if (*pos < 0) { DEBUG_MSG(SNMP_DEBUG,"too big message: %d\n", __LINE__); return -1;}
