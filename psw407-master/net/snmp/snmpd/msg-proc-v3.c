...
s8t prepareDataElements_v3(u8t* const input, const u16t input_len, u16t* pos, message_v3_t* request)
{
    ...
    DEBUG_MSG(SNMP_DEBUG,"msgId: %lu\r\n", request->msgId);

    /* msgMaxSize */
    TRY(ber_decode_integer(input, input_len, pos, &int_value));
    DEBUG_MSG(SNMP_DEBUG,"msgMaxSize: %lu\r\n", int_value);

    /* msgFlags */
    TRY(ber_decode_type_length(input, input_len, pos, &type, &length));
    if ((type != BER_TYPE_OCTET_STRING && type != BER_TYPE_IPADDRESS) || length != 1) {
        return FAILURE;
    }
    request->msgFlags = input[*pos];
    *pos = *pos + 1;
    DEBUG_MSG(SNMP_DEBUG,"msgFlags: %d\r\n", request->msgFlags);
    ...
    /* msgSecurityModel */
    TRY(ber_decode_integer(input, input_len, pos, &int_value));
    DEBUG_MSG(SNMP_DEBUG,"msgSecurityModel: %lu\r\n", int_value);
    ...
    switch (int_value) {
        case USM_SECURITY_MODEL:
            ...
            break;
        default:
            //DEBUG_MSG(SNMP_DEBUG,"unsupported security model [%d]\r\n", request->msgSecurityModel);
            return FAILURE;
    }
