...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  {  
    ...
    if (((RegValue & PHY_SPEED_DUPLEX_STATUS) == PHY1_FULL_10M) || ((RegValue & PHY_SPEED_DUPLEX_STATUS) == PHY1_FULL_100M))
    {
    ...
    if (((RegValue & PHY_SPEED_DUPLEX_STATUS) == PHY1_HALF_10M) || ((RegValue & PHY_SPEED_DUPLEX_STATUS) == PHY1_HALF_100M))
    {
...
__IO uint32_t timeout = 0;
  ...
  if(timeout == PHY_READ_TO)
  {
	  DEBUG_MSG(DEBUG_QD,"Read from register:ETH_ERROR Timeout\r\n");
	  taskEXIT_CRITICAL();
	  return (uint16_t)ETH_ERROR;
  }
  
  //DEBUG_MSG(DEBUG_QD,"Read from register: phyAddr 0x%x,  regAddr 0x%x\r\n",
  //		  PHYAddress,PHYReg);
...
uint32_t ETH_WritePHYRegister(uint16_t PHYAddress, uint16_t PHYReg, uint16_t PHYValue)
{
  ...
  {
	   DEBUG_MSG(DEBUG_QD,"Write from register:ETH_ERROR Timeout\r\n");
	   taskEXIT_CRITICAL();
	   return ETH_ERROR;
  }
  
  //DEBUG_MSG(DEBUG_QD,"Write from register: phyAddr 0x%x,  regAddr 0x%x, data 0x%x\r\n",
	 ...
	 DMARxDescToGet->Status = ETH_DMARxDesc_OWN;
	  ...
	  else /* Ring Mode */
	  {
	    if((DMARxDescToGet->ControlBufferSize & ETH_DMARxDesc_RER) != (uint32_t)RESET)
	    {
	      ...
	      DEBUG_MSG(ETH_DBG,"rx last descr\r\n");
	    }
	    else
	    {
	      /* Selects the next DMA Rx descriptor list for next buffer to read */
...
{
  //DEBUG_MSG(UIP_DEBUG,"ETH_HandleTxPkt %d\r\n",FrameLength);

//for(u16 i=0;i<FrameLength;i++){
//	  DEBUG_MSG(UIP_DEBUG,"0x%02x\r\n",ppkt[i]);
//}
// DEBUG_MSG(UIP_DEBUG,"\r\n");
  ...
  //printf("out %lx\r\n",HTONL(xHeader->dsa_tag));

  uint32_t offset = 0;
  //static xSemaphoreHandle NetSemaphore = NULL;
  ...
  //if (NetSemaphore==NULL)  vSemaphoreCreateBinary(NetSemaphore);
  //xSemaphoreTake( NetSemaphore, ( portTickType ) portMAX_DELAY );
