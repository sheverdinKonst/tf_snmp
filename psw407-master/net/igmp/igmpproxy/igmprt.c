...
igmp_rep_cleanup(igmp_group_t *gp,igmp_rep_t * rep){
	...
	DEBUG_MSG(IGMPV3_DEBUG, "igmp_rep_cleanup: %s\n", inet_ntoa(rep->igmpr_addr));
}
...
igmp_rep_print(
	igmp_rep_t* rep)
{
	DEBUG_MSG(IGMPV3_DEBUG("\t\tmembre:%-16s\r\n", inet_ntoa(rep->igmpr_addr)));
}
/*
	...
	igmp_src_t * src)
{
	...
	DEBUG_MSG(IGMPV3_DEBUG, "igmp_src_cleanup: %s\r\n", inet_ntoa(src->igmps_source));
}
...
igmp_src_print(
	igmp_src_t* src)
{
	DEBUG_MSG(IGMPV3_DEBUG,"\t\tsource:%-16s  %d\r\n",
	...
	igmp_group_t* gp)
{
	...
	DEBUG_MSG(IGMPV3_DEBUG, "igmp_group_cleanup: %s\n", inet_ntoa(gp->igmpg_addr));
}
...
igmp_interface_create(struct in_addr ifaddr,char *ifname,vifi_t index){
  ...
  short flags;
	...
	if (ifp->igmpi_socket == -1) {
		DEBUG_MSG(IGMPV3_DEBUG,"can't create socket \r\n");
 		free(ifp->igmpi_buf);
		free(ifp);
		return NULL;
	}
...
igmp_interface_cleanup(igmp_interface_t* ifp)
{
	...
	assert(ifp != NULL);
	DEBUG_MSG(IGMPV3_DEBUG, "igmp_interface_cleanup: %s\r\n", inet_ntoa(ifp->igmpi_addr));
		...
		if ((VALID_ADDR(mreq.imr_multiaddr)) && (ifp->igmpi_addr.s_addr != upstream)) {
		  ...
		  if (igmp_interface_group_lookup(upstream_interface,mreq.imr_multiaddr) == NULL) {
		    if (setsockopt(router->igmprt_up_socket, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void *) &mreq, sizeof(mreq)) < 0) { 
		    	DEBUG_MSG(IGMPV3_DEBUG,"setsockopt - IP_ADD_MEMBERSHIP\r\n");
		    	return NULL;
		    	//exit(1);
		    }
		  }
				    ...
				    int len)
{
	...
	for (i=0; i < numgrps;i++){
		...
		case 6: /* an BLOCK report */
		  ...
		  break;
		default:
		  DEBUG_MSG(IGMPV3_DEBUG, "igmp_interface_membership_report_v3: group record type undefined\r\n");
		  break;
		}
	}
	...
	igmp_interface_t* ifp)
{
	...
	DEBUG_MSG(IGMPV3_DEBUG," interface %s, %s ver=0x%x\r\n",
...
int igmprt_init(igmp_router_t* igmprt)
{
    ...
    if( igmprt->igmprt_up_socket < 0) {
       	DEBUG_MSG(IGMPV3_DEBUG,"can't open upstream socket\r\n");
       	return 0;
	}*/
	...
	if (igmprt->igmprt_socket < 0) 
		DEBUG_MSG(IGMPV3_DEBUG,"can't open igmp socket\r\n");
        forward_upstream = 0;
    */
	return 1;
}
 ...
 * Cleanup the router 
 */
void igmprt_destroy(igmp_router_t* igmprt)
{
	...
	DEBUG_MSG(IGMPV3_DEBUG, "destroy igmp router ...\r\n");
	igmprt_stop(igmprt);
...
igmprt_input(igmp_router_t* igmprt, igmp_interface_t* ifp)
{
	...
	case IGMP_MEMBERSHIP_QUERY:
		...
		if (query->igmpq_code == 0){
			/*version 1 query*/
			DEBUG_MSG(IGMPV3_DEBUG, "igmpv1 query \r\n");
			receive_membership_query(ifpi,query->igmpq_group,NULL,src.s_addr,0,srsp);
		}else if (igmplen == 8){
			/*version 2 query*/
			DEBUG_MSG(IGMPV3_DEBUG, "igmpv2 query \r\n");
			receive_membership_query(ifpi,query->igmpq_group,NULL,src.s_addr,0,srsp);
		}else if (igmplen >= 12){
			/*version 3 query*/
...
igmprt_input_thread(void* arg)
{
	...
	if (maxfd == 0) {
		DEBUG_MSG(IGMPV3_DEBUG, "error: no interfaces available to wait for input\r\n");
		return NULL;
	}
...
igmprt_start(igmp_router_t* igmprt)
{
	...
	if ((err = pthread_create(&igmprt->igmprt_thr_timer, NULL, 
		...
		DEBUG_MSG(IGMPV3_DEBUG,"couldn't start timer thread\r\n");
	...
	if ((err = pthread_create(&igmprt->igmprt_thr_input, NULL, 
		...
		DEBUG_MSG(IGMPV3_DEBUG,"couldn't start input thread\r\n");
...
igmprt_print(igmp_router_t* igmprt)
{
	...
	assert(igmprt != NULL);
	DEBUG_MSG(IGMPV3_DEBUG,"igmp router:\r\n");
	for (ifp = igmprt->igmprt_interfaces; ifp; ifp = ifp->igmpi_next)
...
void igmp_task(void *pvParameters){
	igmp_router_t router;
		...
		case 's':
			...
			for (gp=ifp->igmpi_groups;gp;gp=gp->igmpg_next)				
				if ((src=igmp_group_src_lookup(gp,srcaddr)) != NULL){
					DEBUG_MSG(IGMPV3_DEBUG,"interface : %s\n",inet_ntoa(ifp->igmpi_addr));
					DEBUG_MSG(IGMPV3_DEBUG,"\tgroup : %s\n",inet_ntoa(gp->igmpg_addr));
					igmp_src_print(src);
				}
		    return 1;
